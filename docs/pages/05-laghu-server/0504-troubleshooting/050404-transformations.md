...
Use `X-Laghu`, cache/CSP response headers, and authenticated metrics for cold/warm
state and failure classification.
Use dry-run mode to compare planned changes without serving them: `GET /path?laghu=preview`
returns the origin body unchanged plus `X-Laghu-Preview`, which reports original and
selected byte counts and delta bytes.
`X-Laghu-Preview` reports `changed`, `original-bytes`, `selected-bytes`,
`delta-bytes`, cache readiness, active policy/rewrite-mode, and the enabled
filter family plan.
