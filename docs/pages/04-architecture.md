...
The standalone `laghu` executable is the third adapter over the same transaction engine. Its transport supports one configured HTTP or verified HTTPS origin, one request per downstream connection, fixed-length request bodies up to 1 MiB, bounded response framing, and a fixed worker pool with a bounded accepted-connection queue. POSIX sockets and WinSock implement one internal transport contract. The proxy owns hop-by-hop and forwarding-header removal, peer-CIDR trust, TLS verification, deadlines, disconnect handling, and origin errors; it never retries a request. Captured responses are complete before engine output is committed, while malformed or truncated captures return a transport error without partial optimization headers.

The proxy reserves `/.laghu/` locally. Valid image, CSS, and JavaScript hashes use the transaction engine's immutable cache path, and fixed beacon scripts plus bounded same-origin POST ingestion remain adapter-owned. Health and readiness are local adapter routes that never contact the origin.
...
One immutable OpenSSL context owns origin trust configuration. Each HTTPS transaction creates a worker-local TLS session over the worker-owned origin socket; TLS never assumes descriptor ownership. The accept queue carries the binary peer address so forwarding trust is evaluated without DNS, global identity state, or client-address logging.
...
JavaScript parsing and transformation run only in `laghu-js-optimize`, a native Rust process built from the committed Cargo lockfile and pinned SWC crate graph. The C runtime publishes source bytes, classic/module kind, normalized Browserslist query, source and policy keys, and bounded metadata through queue protocol v7. The worker performs syntax lowering without polyfills or module conversion, safe compression, and function-local mangling, then publishes only valid, non-identical, strictly smaller output. Checksummed AST-derived metadata certifies URL independence and conservative classic-script concatenation safety; missing or incompatible metadata disables structural rewrites. The shared planner may then combine explicitly grouped classic scripts, inline ready small external scripts, or outline ready large inline scripts while accounting for the complete HTML-plus-external transfer. The worker performs no network access and exposes no Rust or SWC API to a server module.

Opt-in RUM instrumentation is a shared final HTML-planner stage with one deliberate, bounded growth exception for its fixed deferred tag. The planner fingerprints normalized path, bounded structure, ordered eligible script dependencies, policy, observation-provider digest, sample rate, and planner/instrumentation versions. It publishes a server-owned catalog of at most 64 opaque script keys. Adapters serve the immutable fixed script and validate same-origin beacon transport; the runtime validates catalog membership and updates only checksummed mobile/desktop aggregate histograms and counters. Template, dependency, policy, provider, or version changes naturally select a new aggregate record.

Each native server worker owns a bounded RUM memory engine. Request, planner, and beacon paths read or merge records only in that memory engine; they never open a snapshot or contact an external store. A worker-owned background synchronization thread rotates bounded deltas, restores and publishes portable checksummed records, and reconciles the resulting aggregate back into memory. Backend failure therefore affects durability or cross-worker convergence, not request handling, and the worker continues from its last in-memory state. `memory:` is process-lifetime only, `local:` coordinates workers through a separately locked atomic snapshot, and Redis or Valkey coordinates replicas through dynamically loaded hiredis. One versioned Lua contract validates and merges at most 64 encoded records atomically, updates TTLs, and commits an idempotent batch marker without a distributed lock or client-side read/merge cycle. All three adapters preserve the same memory-first boundary.
