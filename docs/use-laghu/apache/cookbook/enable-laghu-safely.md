...
Leave Laghu disabled until the cache and worker queue are writable. Enable it for one site, request known HTML, and retain the previous configuration for rollback. Set `Laghu On` in the intended server or virtual-host scope after the queue and cache are ready.
