...
Verify queue files exist with service-account ownership, then inspect worker logs and readiness. Stop policy changes and repair the affected worker before retrying. Set `laghu on;` in the intended scope after the queue and cache are ready.
