...
static void proxy_access_generate_trace_ids(proxy_access_log *access) {
  ...
  (void)snprintf(access->trace_id, sizeof(access->trace_id), "%016" PRIx64 "%016" PRIx64, trace_high, trace_low);
  (void)snprintf(access->span_id, sizeof(access->span_id), "%016" PRIx64, trace_high ^ trace_low);
}
...
void proxy_log_event(proxy_queue *queue, const char *event, const char *state) {
  ...
  laghu_log_lifecycle record = {.common = {(time_t)time(NULL), "standalone", "standalone", NULL, NULL},
                                .state = state == NULL ? "failed" : state,
                                .failure = event != NULL && strcmp(event, "startup_failure") == 0 ? "runtime" : "none"};
  if (laghu_log_render_lifecycle(&record, line, sizeof(line))) proxy_log_line(queue, line);
}

void proxy_log_startup_failure(proxy_queue *queue, const char *failure) {
  char line[LAGHU_LOG_LINE_SIZE];
  laghu_log_lifecycle record = {
      .common = {(time_t)time(NULL), "standalone", "standalone", NULL, NULL}, .state = "failed", .failure = failure == NULL ? "runtime" : failure};
  if (laghu_log_render_lifecycle(&record, line, sizeof(line))) proxy_log_line(queue, line);
}
...
void proxy_access_write(proxy_queue *queue, const laghu_proxy_options *options, const proxy_access_log *access) {
  ...
  (void)laghu_operational_registry_heartbeat(&queue->operational, (uint64_t)time(NULL), true, 0U, 0U);
  laghu_operational_decision operational_decision =
      access->output_bytes < access->original_response_bytes ? LAGHU_OPERATIONAL_DECISION_OPTIMIZED : LAGHU_OPERATIONAL_DECISION_ORIGINAL;
  if (access->decision != LAGHU_DECISION_PASS && access->decision != LAGHU_DECISION_IMAGE_HIT)
  ...
  if (options == NULL || !options->access_log) return;
  {
