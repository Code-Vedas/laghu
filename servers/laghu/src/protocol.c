...
proxy_header *proxy_find(proxy_header *headers, size_t count,
                         const char *name) {
  size_t index;
...
bool proxy_parse_request(proxy_request *request, size_t length) {
  ...
  *space2++ = '\0';
  if (*line == '\0' || *space1 == '\0' || *space2 == '\0' ||
      !laghu_base_string_copy(request->method, sizeof(request->method), line) ||
      !laghu_base_string_copy(request->target, sizeof(request->target),
                              space1) ||
      !laghu_base_string_copy(request->version, sizeof(request->version),
                              space2) ||
...
bool proxy_parse_response(proxy_response *response, size_t length) {
  ...
  *space2++ = '\0';
  ...
  if (strlen(space1) != 3U ||
      !laghu_base_parse_u64(space1, 100U, 599U, &parsed_status))
    return false;
  response->status = (unsigned int)parsed_status;
  if (*space2 == '\0' ||
      !laghu_base_string_copy(response->reason, sizeof(response->reason),
                              space2) ||
