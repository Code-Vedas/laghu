...
static bool status_schema_valid(const char *body, size_t length, bool stats) {
  ...
  return !stats || strstr(body, "\"schema\":\"laghu-cache-stats\"") != NULL;
}
...
static bool status_explain_schema_valid(const char *body, size_t length, char target[LAGHU_RUNTIME_PATH_SIZE], size_t target_capacity,
  ...
  if (!status_json_has_only_fields(body, length, explain_fields, sizeof(explain_fields) / sizeof(explain_fields[0])) ||
    ...
    return false;
  if (!status_json_text_field(body, "schema", schema, sizeof(schema)) || strcmp(schema, "laghu-explain") != 0 ||
...
typedef enum { STATUS_COMMAND_STATUS, STATUS_COMMAND_DOCTOR } status_command;

static int status_run(int argc, char **argv, status_command command) {
  status_origin origin;
  char token[257], ready[STATUS_BODY_LIMIT + 1U], stats[STATUS_BODY_LIMIT + 1U];
  const char *token_file = NULL, *ca_file = NULL;
  unsigned int timeout = 5U;
  bool json = false;
  SSL_CTX *context = NULL;
  int index, ready_status, stats_status;
  size_t ready_length, stats_length;
  ...
  if (ready_status == 503 || stats_status == 503) {
    fprintf(stderr, "laghu %s: service unavailable\n", name);
    return 6;
  }
  ...
  if (json && command == STATUS_COMMAND_STATUS) {
    printf("{\"schema\":\"laghu-status\",\"ready\":%s,\"stats\":%s}\n", ready, stats);
  } else if (json) {
    ...
    printf(
        "{\"schema\":\"laghu-doctor\",\"runtime\":\"%s\",\"cache\":\"%s\","
        "\"workers\":\"%s\",\"budgets\":\"%s\",\"policy\":\"%s\",\"stats\":%s}"
        "\n",
...
usage:
  fprintf(stderr,
          "Usage: laghu %s URL --token-file PATH [--timeout SECONDS] "
          "[--ca-file PATH] [--json]\n",
          name);
  return 2;
}
...
int laghu_purge_run(int argc, char **argv) {
  ...
  if (json)
    printf(
        "{\"schema\":\"laghu-purge\",\"status\":\"accepted\",\"matched_"
        "artifacts\":%llu}\n",
...
usage:
  fputs(
      "Usage: laghu purge URL --token-file PATH [--timeout SECONDS] [--ca-file "
      "PATH] [--json]\n",
      stderr);
  return 2;
}
...
int laghu_bench_run(int argc, char **argv) {
  ...
  if (ca_file != NULL && !origin.tls) goto usage;
  if (origin.tls) {
    context = SSL_CTX_new(TLS_client_method());
    if (context == NULL || !SSL_CTX_set_default_verify_paths(context) ||
        (ca_file != NULL && !SSL_CTX_load_verify_locations(context, ca_file, NULL))) {
  ...
  if (minimum_ms == UINT64_MAX) minimum_ms = 0ULL;
  {
    ...
    if (json) {
      printf(
          "{\"schema\":\"laghu-bench\",\"target\":\"%s\",\"requests\":%u,"
  ...
  if (malformed > 0U) return 5;
  if (connection_failures > 0U) return 4;
  if (status_200 != requests) return 7;
  return 0;
usage:
  fputs(
      "Usage: laghu bench URL [--requests N] [--timeout SECONDS] [--ca-file "
      "PATH] [--json]\n",
      stderr);
  return 2;
}
...
usage:
  fputs(
      "Usage: laghu explain URL --token-file PATH [--timeout SECONDS] "
      "[--ca-file PATH] [--json]\n",
      stderr);
  return 2;
}
