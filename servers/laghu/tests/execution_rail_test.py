...
def main():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        subprocess.run([sys.executable, str(root / "execution_rail.py"), "--contract", str(root / "parity-contract-v1.json"), "--output", str(output), "--architecture", "native", "--laghu-command", "printf laghu", "--nginx-command", "printf nginx", "--apache-command", "printf apache"], check=True)
        ...
        result = subprocess.run([sys.executable, str(root / "execution_rail.py"), "--contract", str(root / "parity-contract-v1.json"), "--output", str(failed), "--architecture", "native", "--laghu-command", "false", "--nginx-command", "true", "--apache-command", "true"])
