...
CSS rewrite evidence: the dependency-free tokenizer enforces 2 MiB, 262,144 token, 256 URL, 64 nesting-level, and 32 sprite-input bounds. Unit tests cover strings, comments, relative URLs, custom properties, `calc()`, malformed input, style attributes, cold/warm derivations, queue-v5 sprite serialization, and strict bundle gates. NGINX and Apache share the same cold-original derivation runtime, dependency ETags, stale entity-header removal, cache validation, and fail-open behavior. The shared markup planner also combines only whitespace- adjacent, compatible, fully ready stylesheet records in DOM order. Unit tests cover media and CSP precedence, group termination, deterministic immutable assets, source-map and relative-URL exclusion, cold publication, and warm delivery; both adapters exercise the same planner and hash-only CSS route. Import evidence: the shared parser records only contiguous leading imports and the runtime resolves ready same-origin stylesheet graphs without fetching. Unit tests cover quoted and `url()` grammar, media, cross-origin and modern- qualifier rejection, URL rebasing, cycles, cold publication, warm flattening, and ordered immutable links from inline style blocks. Graphs are bounded to 32 unique stylesheets, eight levels, and 2 MiB; dependency keys include ordered URLs, media, source/derived keys, policy, and capabilities. NGINX stable and Apache smoke fixtures populate catalogs through normal CSS responses, verify byte-identical cold responses, warm flattened CSS with dependency ETags, and warm import-to-link conversion through the hash-only CSS route. Head-placement tests cover bounded parsing, missing and adjacent heads, strict node eligibility, original-order movement, protected content, cold publication, byte-neutral warm delivery, and policy-gated executable-script crossing in both adapters. Critical-CSS evidence: the shared bounded planner fingerprints normalized paths, structural element/class/id sequences, stylesheet dependencies, policy, and planner/parser version into opaque mobile/desktop learning buckets. Its same-origin CSP-gated beacon reports bounded rule indexes only, uses monotonic union with a three-observation quorum and metadata-TTL expiry, and keeps cold, invalid, unsupported, or stale pages unchanged. Ready pages inline complete selected rule wrappers within `css_inline_limit`, conservatively retain font-face and keyframe dependencies, and move the unmodified complete stylesheet link before `</body>` with dependency-derived ETags. NGINX, Apache, and standalone smoke lanes exercise the shared script route and planner.

External-font evidence: adapters and the dedicated `laghu-resource-fetch` worker load the same strict provider file and bind queue jobs and cache records to provider ID, provider digest, normalized URL, and request-profile version. The installed file enables exact Google Fonts and Fontsource CDN stylesheet and asset paths and permits additional administrator-owned exact-host providers on reload. The worker uses verified HTTPS, fixed headers, public-address DNS checks, redirect revalidation, bounded responses, and strict font-only CSS validation; it never downloads font binaries. Shared planner tests cover cold-original publication, warm inline replacement, media preservation, CSP and attribute rejection, hostile asset rejection, fail-open behavior, and dependency ETags. Worker fixtures use an injected local CA and DNS endpoint and never contact live providers. NGINX, Apache, and standalone builds use the same planner and configuration contract.
...
JavaScript rewrite evidence: the native `laghu-js-optimize` Rust worker builds pinned SWC crates from `Cargo.lock` and consumes a dedicated bounded queue without Node, network fetching, or C/Rust FFI. It parses classic scripts and modules separately, resolves bounded Browserslist targets, lowers syntax without module conversion or polyfills, disables property and top-level mangling, retains directives and legal comments, and accepts only valid output that is strictly smaller than the 2 MiB source. Checksummed catalog metadata certifies program kind, URL independence, top-level declaration freedom, and concatenation safety. The shared C runtime owns eligibility, observed same-origin catalogs, immutable routes, CSP nonce/hash protection, dependency ETags, queue deduplication, cache validation, the 64-script document bound, and aggregate HTML-plus-JavaScript fail-open transfer-size selection. Independent inherited JavaScript thresholds default to 2 KiB for inlining and 8 KiB for outlining. The bounded planner combines only explicitly grouped runs of 2–16 compatible blocking classic scripts, inlines ready URL-independent external classic or module scripts, and outlines ready URL-independent inline scripts while preserving accepted attributes and CSP semantics. Rust and C tests cover target rejection, queue serialization, resolved-matrix records, classic/module behavior, URL-sensitive constructs, local-only mangling, catalog checksums, cold publication, warm selection, bundle ordering, and inline preservation. NGINX stable, Apache, and standalone smoke lanes exercise real observed-script cold/queue/warm cycles for combination, inlining, outlining, and immutable delivery.

Deferral and source-map evidence: the shared planner requires current RUM quorum, candidate coverage, an exact administrator-approved same-origin query-free path, a checksummed SWC deferral certificate, and a complete order-safe classic-script suffix before adding `defer`. Recommendations and rollback warnings are advisory, bounded, redacted, generation-separated, and exposed consistently without allowing approval to bypass stale evidence, changed dependencies, or fail-open checks. SWC emits Source Map v3 data with sanitized names and no embedded sources; external, outlined, and combined artifacts use immutable same-origin map routes, with ordered indexed sections for bundles. Map comments participate in strict smaller-output selection, while map generation or publication failure preserves valid optimized JavaScript without a map. Unit, Rust, NGINX, Apache, standalone, container, and packaging validation cover the completed capabilities.
...
Lifecycle evidence covers bounded POSIX signal drain, second-signal and deadline cancellation, queued-connection rejection, worker joins, local cache and optimizer readiness transitions, query/header/body-safe JSON records, and non-root execution with a read-only container filesystem.
