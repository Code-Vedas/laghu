...
static int laghu_html_refresh_publish_job(int argc, char **argv) {
  ...
  path_length = snprintf(job.request_path, sizeof(job.request_path), "%s", argv[4]);
  validator_length = snprintf(job.validator, sizeof(job.validator), "%s", argv[5]);
  if (path_length < 0 || (size_t)path_length >= sizeof(job.request_path) || validator_length < 0 || (size_t)validator_length >= sizeof(job.validator))
    return 1;
...
static int laghu_html_refresh_assert_cached(int argc, char **argv) {
  ...
  if (!laghu_runtime_cache_lookup_variant(argv[2], key, &entry) || entry.length != expected_length ||
      (body = malloc(entry.length == 0U ? 1U : entry.length)) == NULL)
    return 1;
  if (laghu_runtime_cache_read(&entry, body, entry.length) && memcmp(body, argv[5], expected_length) == 0) result = 0;
  free(body);
  return result;
}

int main(int argc, char **argv) {
  if (argc > 1 && strcmp(argv[1], "--job") == 0) return laghu_html_refresh_publish_job(argc, argv);
  if (argc > 1 && strcmp(argv[1], "--cached") == 0) return laghu_html_refresh_assert_cached(argc, argv);
  fputs(
