...
static bool ngx_http_laghu_same_origin(ngx_http_request_t *request) {
  ...
  for (index = 0U;; ++index) {
    ...
    if (headers[index].hash != 0U && headers[index].key.len == sizeof("Sec-Fetch-Site") - 1U &&
        ngx_strncasecmp(headers[index].key.data, (u_char *)"Sec-Fetch-Site", sizeof("Sec-Fetch-Site") - 1U) == 0 &&
        headers[index].value.len == sizeof("same-origin") - 1U &&
        ngx_strncasecmp(headers[index].value.data, (u_char *)"same-origin", sizeof("same-origin") - 1U) == 0)
      return true;
  }
}

static ngx_int_t ngx_http_laghu_send_beacon_script(ngx_http_request_t *request, const unsigned char *script, size_t script_length) {
  ngx_buf_t *buffer;
  ngx_chain_t output;
...
static ngx_int_t ngx_http_laghu_read_beacon(ngx_http_request_t *request) {
  time_t now = ngx_time();
  if (request->headers_in.content_type == NULL || request->headers_in.content_type->value.len < sizeof("application/json") - 1U ||
      ngx_strncasecmp(request->headers_in.content_type->value.data, (u_char *)"application/json", sizeof("application/json") - 1U) != 0 ||
      request->headers_in.content_length_n <= 0 || request->headers_in.content_length_n > 16384 || !ngx_http_laghu_same_origin(request))
  ...
  if (ngx_http_read_client_request_body(request, ngx_http_laghu_beacon_body) >= NGX_HTTP_SPECIAL_RESPONSE) return NGX_HTTP_BAD_REQUEST;
  return NGX_DONE;
}
...
ngx_int_t ngx_http_laghu_beacon_endpoint(ngx_http_request_t *request, ngx_http_laghu_loc_conf_t *conf) {
  ...
  laghu_buffer target = {(const unsigned char *)request->uri.data, request->uri.len};

  laghu_http_beacon_options_init(&options);
  options.image_enabled = conf->core.mode == LAGHU_MODE_ON && conf->core.image_beacon == LAGHU_MODE_ON;
  options.critical_css_enabled = conf->core.mode == LAGHU_MODE_ON && conf->core.critical_css_beacon == LAGHU_MODE_ON;
  options.instrumentation_enabled = conf->core.mode == LAGHU_MODE_ON && conf->core.instrumentation_beacon == LAGHU_MODE_ON;
  if (!laghu_http_beacon_plan_build(&plan, method, target, &options) || !plan.recognized) return NGX_DECLINED;
  ...
  switch (plan.route) {
    case LAGHU_HTTP_BEACON_ROUTE_IMAGE_SCRIPT: {
      ...
      return ngx_http_laghu_send_beacon_script(request, (const unsigned char *)script, strlen(script));
    }
