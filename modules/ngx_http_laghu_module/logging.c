...
void ngx_http_laghu_log_transaction(ngx_http_request_t *request, ngx_http_laghu_request_ctx_t *context, const laghu_http_transaction_result *result,
  ...
  method_length = request->method_name.len < sizeof(method) - 1U ? request->method_name.len : sizeof(method) - 1U;
  path_length = request->uri.len < sizeof(path) - 1U ? request->uri.len : sizeof(path) - 1U;
  ...
  {
    laghu_log_transaction record = {.common = {(time_t)ngx_time(), "nginx", "nginx", trace_id, span_id},
    ...
    if (laghu_log_render_transaction(&record, line, sizeof(line))) {
      ngx_log_error(NGX_LOG_NOTICE, request->connection->log, 0, "laghu_json=%s", line);
...
void ngx_http_laghu_log_defer_recommendation(ngx_http_request_t *request, const laghu_http_transaction_result *result) {
  time_t now;
  if (!result->javascript_defer_recommended && !result->javascript_defer_rollback_recommended) return;
  now = ngx_time();
  if (ngx_http_laghu_last_defer_recommendation != 0 && now - ngx_http_laghu_last_defer_recommendation < 3600) return;
  ...
  ngx_log_error(NGX_LOG_NOTICE, request->connection->log, 0,
                ...
                result->javascript_defer_rollback_recommended ? "javascript_defer_rollback_recommendation" : "javascript_defer_recommendation",
                result->javascript_defer_path, result->javascript_defer_template, (ngx_uint_t)result->javascript_defer_bucket,
                (uint64_t)result->javascript_defer_observations, result->javascript_defer_path);
}
