...
static bool ngx_http_laghu_html_cache_hit(const ngx_http_request_t *request) {
  ...
  for (index = 0U;; ++index) {
    ...
    if (headers[index].hash != 0U && headers[index].key.len == sizeof("X-Laghu-Cache") - 1U && headers[index].value.len == sizeof("hit") - 1U &&
        ngx_strncasecmp(headers[index].key.data, (u_char *)"X-Laghu-Cache", sizeof("X-Laghu-Cache") - 1U) == 0 &&
        ngx_strncasecmp(headers[index].value.data, (u_char *)"hit", sizeof("hit") - 1U) == 0)
      return true;
  }
  return false;
}

static void laghu_http_laghu_generate_trace_ids(const ngx_http_request_t *request, char trace_id[33U], char span_id[17U]) {
...
static bool ngx_http_laghu_html_cache_control_prohibits(laghu_buffer value) {
  ...
  while (cursor < value.length) {
    ...
    if (ngx_http_laghu_html_cache_token_equal(value.data + token_start, token_end - token_start, "private") ||
        ngx_http_laghu_html_cache_token_equal(value.data + token_start, token_end - token_start, "no-store") ||
        ngx_http_laghu_html_cache_token_equal(value.data + token_start, token_end - token_start, "no-cache"))
      return true;
    cursor = end + 1U;
  }
  return false;
...
static bool ngx_http_laghu_html_cache_response_safe(const ngx_http_laghu_request_ctx_t *context) {
  size_t index;
  bool html = false;
  if (context == NULL || context->response.status != NGX_HTTP_OK || context->request.method.length != 3U ||
  ...
  for (index = 0U; index < context->request.header_count; ++index)
    if ((context->request.headers[index].name.length == 6U &&
         ngx_strncasecmp((u_char *)context->request.headers[index].name.data, (u_char *)"Cookie", 6U) == 0) ||
        (context->request.headers[index].name.length == 13U &&
         ngx_strncasecmp((u_char *)context->request.headers[index].name.data, (u_char *)"Authorization", 13U) == 0))
      return false;
  for (index = 0U; index < context->response.header_count; ++index) {
    const laghu_http_header *header = &context->response.headers[index];
    if ((header->name.length == 10U && ngx_strncasecmp((u_char *)header->name.data, (u_char *)"Set-Cookie", 10U) == 0) ||
        (header->name.length == 4U && ngx_strncasecmp((u_char *)header->name.data, (u_char *)"Vary", 4U) == 0 &&
         !(header->value.length == 15U && ngx_strncasecmp((u_char *)header->value.data, (u_char *)"Accept-Encoding", 15U) == 0)) ||
        (header->name.length == 16U && ngx_strncasecmp((u_char *)header->name.data, (u_char *)"Content-Encoding", 16U) == 0))
      return false;
    if (header->name.length == 13U && ngx_strncasecmp((u_char *)header->name.data, (u_char *)"Cache-Control", 13U) == 0 &&
        ngx_http_laghu_html_cache_control_prohibits(header->value))
      return false;
    if (header->name.length == 12U && ngx_strncasecmp((u_char *)header->name.data, (u_char *)"Content-Type", 12U) == 0 &&
        header->value.length >= 9U && ngx_strncasecmp((u_char *)header->value.data, (u_char *)"text/html", 9U) == 0)
      html = true;
  }
  return html;
}

static void ngx_http_laghu_publish_html_cache(ngx_http_laghu_request_ctx_t *context, ngx_http_laghu_loc_conf_t *conf,
                                              const laghu_http_transaction_result *result) {
  laghu_buffer selected;
  if (result == NULL || result->dependencies_pending) return;
  selected = result->cache_selected.data == NULL ? result->selected : result->cache_selected;
  if (context == NULL || conf == NULL || conf->core.html_cache_origin[0] == '\0' || conf->core.html_cache_ttl == LAGHU_HTML_CACHE_TTL_UNSET ||
      !ngx_http_laghu_html_cache_response_safe(context))
    return;
  (void)laghu_html_cache_publish(conf->service.image_cache, conf->core.html_cache_origin, (const char *)context->request.normalized_path.data,
                                 (const char *)context->response.source_validator.data, selected, (uint64_t)ngx_time(), NULL);
}
...
ngx_int_t ngx_http_laghu_transaction_header_filter(ngx_http_request_t *request) {
  ...
  if (ngx_http_laghu_html_cache_hit(request)) return ngx_http_laghu_next_header_filter(request);
  if (request->uri.len >= sizeof("/.laghu/") - 1U && ngx_strncmp(request->uri.data, "/.laghu/", sizeof("/.laghu/") - 1U) == 0) {
    return ngx_http_laghu_next_header_filter(request);
  }
  administration_candidate = conf != NULL && ngx_http_laghu_administration_candidate(request, conf);
  ...
  if (!prepared || result.action == LAGHU_HTTP_ACTION_BYPASS) {
    ngx_http_laghu_log_transaction(request, context, &result, prepared ? "none" : "runtime");
    laghu_http_transaction_result_release(&result);
    return ngx_http_laghu_next_header_filter(request);
  }
  {
    laghu_cache_limits limits = {.size_limit = conf->service.cache_limits.size_limit,
    ...
    if (!laghu_cache_backend_register_path(conf->service.image_cache, &limits)) {
      ngx_log_error(NGX_LOG_WARN, request->connection->log, 0, "laghu file cache backend unavailable; serving origin");
      laghu_http_transaction_result_release(&result);
      return ngx_http_laghu_next_header_filter(request);
    }
  }
  ...
  ngx_http_set_ctx(request, context, ngx_http_laghu_module);
  laghu_http_transaction_result_release(&result);
  return context->header_deferred ? NGX_OK : ngx_http_laghu_next_header_filter(request);
}

ngx_int_t ngx_http_laghu_transaction_body_filter(ngx_http_request_t *request, ngx_chain_t *chain) {
  ngx_http_laghu_request_ctx_t *context = ngx_http_get_module_ctx(request, ngx_http_laghu_module);
  ngx_chain_t *current;
  ...
  for (current = chain; current != NULL; current = current->next) {
    ...
    if (length != 0U && ngx_buf_in_memory(buffer)) {
      ngx_memcpy(context->capture + context->capture_length, buffer->pos, length);
    } else if (length != 0U && buffer->in_file) {
      if (ngx_read_file(buffer->file, context->capture + context->capture_length, length, buffer->file_pos) != (ssize_t)length) {
        context->capture_enabled = false;
        break;
      }
    } else if (length != 0U) {
  ...
  if (!final_buffer) {
    return context->header_deferred ? NGX_OK : ngx_http_laghu_next_body_filter(request, chain);
  }
  if (context->capture_enabled) {
    ...
    laghu_operational_registry_budget(&ngx_http_laghu_operational, &context->transaction.budget,
                                      context->transaction.environment.config.transform_deadline_ms);
    laghu_operational_registry_lcp(&ngx_http_laghu_operational, result.lcp_decision, result.lcp_applied, result.lcp_profile_observations,
    ...
    if (context->header_deferred) {
      ...
      unsigned char *copy;
      result.not_modified = laghu_http_request_matches_result_etag(&context->request, &result);
      copy = result.not_modified ? NULL : ngx_pnalloc(request->pool, selected_length);
      if ((!result.not_modified && copy == NULL) || (!result.not_modified && ngx_http_laghu_send_early_hints(request, &result) != NGX_OK) ||
    ...
    if (!result.job_published && context->transaction.action == LAGHU_HTTP_ACTION_CAPTURE_IMAGE) {
      time_t now = ngx_time();
      if (ngx_http_laghu_last_queue_warning == 0 || now - ngx_http_laghu_last_queue_warning >= 60) {
        ngx_http_laghu_last_queue_warning = now;
        ngx_log_error(NGX_LOG_WARN, request->connection->log, 0, "laghu image queue is full; preserving the original");
      }
    }
    laghu_http_transaction_result_release(&result);
  }
