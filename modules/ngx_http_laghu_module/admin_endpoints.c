...
static ngx_int_t ngx_http_laghu_admin_write(ngx_http_request_t *request, const laghu_http_administrative_response *response, const char *body) {
  ngx_buf_t *buffer;
  ngx_chain_t output;
  ...
  if (response->content == LAGHU_HTTP_ADMINISTRATIVE_CONTENT_PROMETHEUS) {
    ngx_str_set(&request->headers_out.content_type, "text/plain; version=0.0.4; charset=utf-8");
...
static ngx_int_t ngx_http_laghu_admin_json(ngx_http_request_t *request, ngx_uint_t status, const char *json) {
  laghu_http_administrative_response response = {status, LAGHU_HTTP_ADMINISTRATIVE_CONTENT_JSON, strlen(json)};
  return ngx_http_laghu_admin_write(request, &response, json);
}

static bool ngx_http_laghu_admin_authorized(ngx_http_request_t *request, const ngx_http_laghu_loc_conf_t *conf) {
  ...
  if (conf->service.purge_token_file[0] == '\0' || !ngx_http_laghu_peer_matches(request, conf->service.purge_allow, conf->service.purge_allow_count))
    return false;
  for (header_index = 0U;; ++header_index) {
    ...
    if (headers[header_index].key.len == sizeof("X-Laghu-Purge-Token") - 1U &&
        ngx_strncasecmp(headers[header_index].key.data, (u_char *)"X-Laghu-Purge-Token", sizeof("X-Laghu-Purge-Token") - 1U) == 0) {
  ...
  if (token != NULL) {
    ...
    file.fd = ngx_open_file((u_char *)conf->service.purge_token_file, NGX_FILE_RDONLY, NGX_FILE_OPEN, 0U);
    length = file.fd == NGX_INVALID_FILE ? -1 : ngx_read_file(&file, expected, sizeof(expected), 0U);
    if (file.fd != NGX_INVALID_FILE) ngx_close_file(file.fd);
    while (length > 0 && (expected[length - 1] == '\n' || expected[length - 1] == '\r')) --length;
    maximum = length > 0 && (size_t)length > supplied ? (size_t)length : supplied;
    difference = (u_char)((length > 0 ? (size_t)length : 0U) ^ supplied);
    for (compare_index = 0U; compare_index < maximum; ++compare_index)
      difference |= (u_char)((compare_index < (size_t)(length > 0 ? length : 0) ? expected[compare_index] : 0U) ^
                             (compare_index < supplied ? token->value.data[compare_index] : 0U));
    return length >= 16 && length < (ssize_t)sizeof(expected) && difference == 0U;
  }
  return false;
}
...
bool ngx_http_laghu_administration_candidate(ngx_http_request_t *request, ngx_http_laghu_loc_conf_t *conf) {
  ...
  if ((request->method_name.len == 5U && ngx_strncasecmp((u_char *)request->method_name.data, (u_char *)"PURGE", 5U) == 0) ||
      (request->unparsed_uri.len >= 12U && request->unparsed_uri.len >= strlen("laghu=purge") &&
       ngx_strnstr(request->unparsed_uri.data, (char *)"laghu=purge", request->unparsed_uri.len) != NULL)) {
    return true;
  }
  ...
  method = (laghu_buffer){(const unsigned char *)request->method_name.data, request->method_name.len};
  target = (laghu_buffer){(const unsigned char *)request->unparsed_uri.data, request->unparsed_uri.len};
...
ngx_int_t ngx_http_laghu_admin_endpoint(ngx_http_request_t *request, ngx_http_laghu_loc_conf_t *conf) {
  laghu_http_administrative_options options;
  laghu_http_administrative_plan plan;
  laghu_cache_limits cache_limits = {.size_limit = conf->service.cache_limits.size_limit,
  ...
  laghu_buffer method = {(const unsigned char *)request->method_name.data, request->method_name.len};
  laghu_buffer target = {(const unsigned char *)request->unparsed_uri.data, request->unparsed_uri.len};
  ...
  options.purge_query_enabled = conf->service.purge_query;
  options.purge_query_get_only = false;
  if (!laghu_http_administrative_plan_build(&plan, method, target, &options) || !plan.recognized) return NGX_DECLINED;
  if (plan.status == NGX_HTTP_NOT_FOUND) return NGX_HTTP_NOT_FOUND;
  if (plan.requires_authorization && !ngx_http_laghu_admin_authorized(request, conf))
    return ngx_http_laghu_admin_json(request, NGX_HTTP_FORBIDDEN, "{\"status\":\"forbidden\"}");
  if (!laghu_cache_backend_register_path(conf->service.image_cache, &cache_limits))
    return ngx_http_laghu_admin_json(request, NGX_HTTP_SERVICE_UNAVAILABLE, "{\"status\":\"unavailable\"}");
  if (plan.status != 0U) {
    if (plan.status == NGX_HTTP_BAD_REQUEST) return ngx_http_laghu_admin_json(request, NGX_HTTP_BAD_REQUEST, "{\"status\":\"malformed\"}");
  ...
  if (plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_STATS) {
    ...
    char *json;
    if (!laghu_cache_backend_health_path(conf->service.image_cache, &stats)) return NGX_HTTP_SERVICE_UNAVAILABLE;
    json = ngx_pnalloc(request->pool, LAGHU_OPERATIONAL_RENDER_SIZE);
    if (json == NULL) return NGX_HTTP_INTERNAL_SERVER_ERROR;
    if (!laghu_http_administrative_render_stats(&cache_limits, &stats, json, LAGHU_OPERATIONAL_RENDER_SIZE, &response))
      return NGX_HTTP_INTERNAL_SERVER_ERROR;
    return ngx_http_laghu_admin_write(request, &response, json);
  }
  if (plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_CONSOLE || plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_HISTORY ||
    ...
    if (plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_CONSOLE || plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_EXPLAIN) {
      ...
      cache_ready = laghu_cache_backend_health_path(conf->service.image_cache, &stats);
    }
    if (!laghu_operational_registry_snapshot(&ngx_http_laghu_operational, &snapshot) ||
        !laghu_operational_readiness_evaluate(&snapshot, (uint64_t)ngx_time(), true, cache_ready, conf->service.readiness_strict, &readiness) ||
        !laghu_http_administrative_build_console_page_model(&plan, &stats, &readiness, &snapshot, &page) ||
        !laghu_http_administrative_render_console_page(&plan, &page, rendered, LAGHU_OPERATIONAL_RENDER_SIZE, &response)) {
      return NGX_HTTP_SERVICE_UNAVAILABLE;
    }
    return ngx_http_laghu_admin_write(request, &response, rendered);
  }
  if (plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_METRICS || plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_READINESS) {
    laghu_operational_snapshot *snapshot;
    laghu_http_administrative_response response;
    char *rendered;
    uint64_t now = (uint64_t)ngx_time();
    bool cache_ready = true;
    (void)laghu_operational_registry_heartbeat(&ngx_http_laghu_operational, now, true, 0U, 0U);
    snapshot = ngx_pcalloc(request->pool, sizeof(*snapshot));
    rendered = ngx_pnalloc(request->pool, LAGHU_OPERATIONAL_RENDER_SIZE);
    if (snapshot == NULL || rendered == NULL) return NGX_HTTP_INTERNAL_SERVER_ERROR;
    if (!laghu_operational_registry_snapshot(&ngx_http_laghu_operational, snapshot))
      return ngx_http_laghu_admin_json(request, NGX_HTTP_SERVICE_UNAVAILABLE, "{\"status\":\"unavailable\"}");
    ...
    if (!laghu_http_administrative_render_operational(&plan, snapshot, now, true, cache_ready, conf->service.readiness_strict, rendered,
  ...
  if (plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_PURGE) {
    ...
    if (rendered == NULL) return NGX_HTTP_INTERNAL_SERVER_ERROR;
    result = laghu_cache_backend_purge_url_path(conf->service.image_cache, purge_target, (uint64_t)ngx_time(), &matched);
    if (!laghu_http_administrative_render_purge(result, matched, rendered, LAGHU_OPERATIONAL_RENDER_SIZE, &response))
      return NGX_HTTP_INTERNAL_SERVER_ERROR;
    return ngx_http_laghu_admin_write(request, &response, rendered);
  }
  return NGX_HTTP_INTERNAL_SERVER_ERROR;
