...
static bool laghu_apache_admin_authorized(request_rec *request, laghu_apache_config *config) {
  const char *provided = apr_table_get(request->headers_in, "X-Laghu-Purge-Token");
  bool peer_allowed = false;
  bool token_allowed = false;
  peer_allowed = laghu_apache_peer_matches(request, config->service.purge_allow, config->service.purge_allow_count);
  if (provided != NULL && config->service.purge_token_file[0] != '\0') {
    FILE *token_file = fopen(config->service.purge_token_file, "rb");
    char expected[257U];
    size_t length = token_file == NULL ? 0U : fread(expected, 1U, sizeof(expected), token_file);
    ...
    for (token_index = 0U; token_index < maximum; ++token_index)
      difference |=
          (unsigned char)((token_index < length ? expected[token_index] : 0U) ^ (token_index < supplied ? (unsigned char)provided[token_index] : 0U));
    token_allowed = length >= 16U && length < sizeof(expected) && difference == 0U;
  }
  return peer_allowed && token_allowed;
}

static int laghu_apache_admin_write(request_rec *request, const laghu_http_administrative_response *response, const char *rendered) {
  ...
  return request->header_only || ap_rwrite(rendered, (int)response->length, request) >= 0 ? OK : HTTP_INTERNAL_SERVER_ERROR;
}
...
int laghu_apache_admin_endpoint(request_rec *request, laghu_apache_config *config) {
  ...
  if (request->method != NULL) method = (laghu_buffer){(const unsigned char *)request->method, strlen(request->method)};
  if (request->unparsed_uri != NULL) target = (laghu_buffer){(const unsigned char *)request->unparsed_uri, strlen(request->unparsed_uri)};
  ...
  if (!laghu_http_administrative_plan_build(&plan, method, target, &options) || !plan.recognized) return DECLINED;
  if (plan.status == HTTP_NOT_FOUND) return HTTP_NOT_FOUND;

  if (plan.requires_authorization && !laghu_apache_admin_authorized(request, config)) {
  ...
  apr_table_setn(request->headers_out, "Cache-Control", "no-store");
  ap_set_content_type(request, "application/json");
  if (!laghu_cache_backend_register_path(config->service.image_cache, &config->service.cache_limits)) {
  ...
  if (plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_STATS) {
    ...
    if (!laghu_cache_backend_health_path(config->service.image_cache, &stats)) return HTTP_SERVICE_UNAVAILABLE;
    if (!laghu_http_administrative_render_stats(&config->service.cache_limits, &stats, rendered, LAGHU_OPERATIONAL_RENDER_SIZE, &response))
  ...
  if (plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_CONSOLE || plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_HISTORY ||
    ...
    if (plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_CONSOLE || plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_EXPLAIN) {
      ...
      cache_ready = laghu_cache_backend_health_path(config->service.image_cache, &stats);
    }
    if (!laghu_operational_registry_snapshot(&laghu_apache_operational, &snapshot) ||
        !laghu_operational_readiness_evaluate(&snapshot, (uint64_t)apr_time_sec(apr_time_now()), true, cache_ready, config->service.readiness_strict,
                                              &readiness) ||
  ...
  if (plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_METRICS || plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_READINESS) {
    ...
    (void)laghu_operational_registry_heartbeat(&laghu_apache_operational, now, true, 0U, 0U);
    ...
    if (!laghu_operational_registry_snapshot(&laghu_apache_operational, snapshot)) return HTTP_SERVICE_UNAVAILABLE;
    if (plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_READINESS) {
      laghu_cache_stats stats = {0};
      cache_ready = laghu_cache_backend_health_path(config->service.image_cache, &stats);
      laghu_operational_registry_cache(&laghu_apache_operational, &stats);
    }
    if (!laghu_http_administrative_render_operational(&plan, snapshot, now, true, cache_ready, config->service.readiness_strict, rendered,
  ...
  if (plan.action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_PURGE) {
    ...
    if (rendered == NULL) return HTTP_INTERNAL_SERVER_ERROR;
    purge_result = laghu_cache_backend_purge_url_path(config->service.image_cache, purge_target, (uint64_t)apr_time_sec(apr_time_now()), &matched);
    if (!laghu_http_administrative_render_purge(purge_result, matched, rendered, LAGHU_OPERATIONAL_RENDER_SIZE, &response))
      return HTTP_INTERNAL_SERVER_ERROR;
    return laghu_apache_admin_write(request, &response, rendered);
  }
  return HTTP_INTERNAL_SERVER_ERROR;
