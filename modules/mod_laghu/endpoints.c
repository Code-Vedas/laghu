...
static bool laghu_apache_administration_candidate(request_rec *request, laghu_apache_config *config) {
  ...
  if (request == NULL || config == NULL || request->unparsed_uri == NULL || request->method == NULL) {
    return false;
  }
  if (strcmp(request->method, "PURGE") == 0 || strstr(request->unparsed_uri, "laghu=purge") != NULL) {
    return true;
  }
  ...
  method = (laghu_buffer){(const unsigned char *)request->method, strlen(request->method)};
  target = (laghu_buffer){(const unsigned char *)request->unparsed_uri, strlen(request->unparsed_uri)};
...
static void laghu_apache_enqueue_html_refresh(laghu_apache_config *config, const char *request_path, const laghu_html_cache_record *record) {
  laghu_runtime_queue *queue;
  laghu_runtime_job job = {0};
  size_t length;
  if (config == NULL || request_path == NULL || record == NULL || (length = strlen(request_path)) >= sizeof(job.request_path) ||
      !laghu_html_cache_key(config->core.html_cache_origin, request_path, job.index_key)) {
    return;
  }
  ...
  (void)laghu_apache_html_refresh_try_publish(config, &job, (uint64_t)apr_time_sec(apr_time_now()));
}

static int laghu_apache_html_cache_handler(request_rec *request, laghu_apache_config *config) {
  laghu_html_cache_record record;
  laghu_runtime_cache_entry encoded;
  laghu_precompressed_coding coding;
  const char *content_encoding = NULL;
  unsigned char *body;
  if (request == NULL || config == NULL || config->core.mode != LAGHU_MODE_ON || request->method_number != M_GET || request->unparsed_uri == NULL ||
      request->unparsed_uri[0] != '/' || strncmp(request->unparsed_uri, "/.laghu/", sizeof("/.laghu/") - 1U) == 0 ||
      apr_table_get(request->headers_in, "Authorization") != NULL || apr_table_get(request->headers_in, "Cookie") != NULL ||
      config->core.html_cache_origin[0] == '\0' || config->core.html_cache_ttl == LAGHU_HTML_CACHE_TTL_UNSET ||
      !laghu_html_cache_lookup(config->service.image_cache, config->core.html_cache_origin, request->unparsed_uri,
                               (uint64_t)apr_time_sec(apr_time_now()), config->core.html_cache_ttl, config->core.html_cache_stale_ttl, &record) ||
      record.state == LAGHU_HTML_CACHE_MISS || record.entry.length == 0U || record.entry.content_type[0] == '\0' || record.entry.length > INT_MAX) {
    return DECLINED;
  }
  if (record.state == LAGHU_HTML_CACHE_STALE) laghu_apache_enqueue_html_refresh(config, request->unparsed_uri, &record);
  body = apr_palloc(request->pool, record.entry.length);
  if (body == NULL || !laghu_runtime_cache_read(&record.entry, body, record.entry.length)) {
    return DECLINED;
  }
  if (laghu_precompressed_select(config->service.image_cache, (laghu_buffer){body, record.entry.length},
                                 apr_table_get(request->headers_in, "Accept-Encoding"), &encoded, &coding)) {
  ...
  if (content_encoding != NULL) {
    ...
    apr_table_setn(request->err_headers_out, "Content-Encoding", content_encoding);
  }
...
int laghu_apache_html_cache_entry_handler(request_rec *request) {
  ...
  server_config = ap_get_module_config(request->server->module_config, &laghu_module);
  directory_config = ap_get_module_config(request->per_dir_config, &laghu_module);
  config = laghu_apache_merge_config(request->pool, server_config, directory_config);
...
int laghu_apache_variant_handler(request_rec *request) {
  ...
  if (strncmp(request->uri, "/.laghu/", sizeof("/.laghu/") - 1U) != 0 && !administration_candidate) {
    return DECLINED;
  }
  ...
  if (config->service.cache_flush_file[0] != '\0')
    (void)laghu_cache_flush_file_poll(config->service.image_cache, config->service.cache_flush_file, (uint64_t)apr_time_sec(apr_time_now()), NULL);
  {
    int admin_status = laghu_apache_admin_endpoint(request, config);
    if (admin_status != DECLINED) return admin_status;
  }
  if (request->uri != NULL && strncmp(request->uri, "/.laghu/beacon/", sizeof("/.laghu/beacon/") - 1U) == 0) {
    int beacon_status = laghu_apache_beacon_endpoint(request, config);
    if (beacon_status != DECLINED) return beacon_status;
  }
  if (request->uri != NULL && (strncmp(request->uri, "/.laghu/image/", sizeof("/.laghu/image/") - 1U) == 0 ||
                               strncmp(request->uri, "/.laghu/css/", sizeof("/.laghu/css/") - 1U) == 0 ||
                               strncmp(request->uri, "/.laghu/js/", sizeof("/.laghu/js/") - 1U) == 0 ||
                               strncmp(request->uri, "/.laghu/media/", sizeof("/.laghu/media/") - 1U) == 0)) {
