...
void laghu_apache_log_transaction(request_rec *request, laghu_apache_context *context, const laghu_http_transaction_result *result,
  ...
  {
    laghu_log_transaction record = {.common = {(time_t)apr_time_sec(apr_time_now()), "apache", "apache", trace_id, span_id},
                                    ...
                                    .output_bytes = output,
                                    .duration_ms = context->log_started == 0 ? 0U : (uint64_t)((apr_time_now() - context->log_started) / 1000),
                                    .cache = cache,
                                    .job_published = job_published,
                                    .failure = failure == NULL ? "none" : failure};
    if (laghu_log_render_transaction(&record, line, sizeof(line))) {
      ap_log_rerror(APLOG_MARK, APLOG_NOTICE, 0, request, "laghu_json=%s", line);
...
void laghu_apache_log_defer_recommendation(request_rec *request, const laghu_http_transaction_result *result) {
  ...
  now = apr_time_now();
  if (laghu_apache_last_defer_recommendation != 0 && now - laghu_apache_last_defer_recommendation < apr_time_from_sec(3600)) return;
  ...
  ap_log_rerror(APLOG_MARK, APLOG_NOTICE, 0, request,
                ...
                result->javascript_defer_path, result->javascript_defer_template, result->javascript_defer_bucket,
                (unsigned long long)result->javascript_defer_observations, result->javascript_defer_path);
}
