...
static laghu_apache_queue_binding laghu_apache_queue_bindings[LAGHU_APACHE_QUEUE_CONFIG_LIMIT];
...
bool laghu_apache_queue_registry_add(const laghu_apache_config *parent, const laghu_apache_config *child, const laghu_service_config *service) {
  ...
  for (index = 0U; index < laghu_apache_queue_binding_count; ++index)
    if (laghu_apache_queue_bindings[index].parent == parent && laghu_apache_queue_bindings[index].child == child) return true;
  for (index = 0U; index < laghu_apache_queue_binding_count; ++index) {
    laghu_apache_queue_binding *existing = &laghu_apache_queue_bindings[index];
    if (existing->font_enabled == (service->font_providers != NULL) && strcmp(existing->image_queue_path, service->worker_queue) == 0 &&
  ...
  if (laghu_apache_queue_binding_count >= LAGHU_APACHE_QUEUE_CONFIG_LIMIT) return false;
  binding = &laghu_apache_queue_bindings[laghu_apache_queue_binding_count++];
  binding->parent = parent;
  binding->child = child;
  binding->font_enabled = service->font_providers != NULL;
  (void)snprintf(binding->image_queue_path, sizeof(binding->image_queue_path), "%s", service->worker_queue);
  (void)snprintf(binding->font_queue_path, sizeof(binding->font_queue_path), "%s", service->font_fetch_queue);
...
laghu_apache_queue_binding *laghu_apache_queue_binding_find_service(const laghu_service_config *service) {
  ...
  for (index = 0U; index < laghu_apache_queue_binding_count; ++index) {
    ...
    if (binding->font_enabled == font_enabled && strcmp(binding->image_queue_path, service->worker_queue) == 0 &&
...
static laghu_runtime_queue *laghu_apache_attached_queue(laghu_apache_queue_binding *binding, unsigned int kind) {
  ...
  if (kind == 1U && binding->font_enabled && apr_atomic_read32(&binding->font_attached) != 0U) return (laghu_runtime_queue *)&binding->font_queue;
...
bool laghu_apache_html_refresh_try_publish(laghu_apache_config *config, const laghu_runtime_job *job, uint64_t now) {
  ...
  for (index = 0U; index < LAGHU_APACHE_HTML_REFRESH_DEDUP; ++index) {
    if (binding->html_refresh_until[index] > now && memcmp(binding->html_refresh_keys[index], job->index_key, sizeof(job->index_key)) == 0) {
      goto done;
    }
    if (binding->html_refresh_until[index] < binding->html_refresh_until[candidate]) candidate = index;
  }
  if (laghu_runtime_queue_try_publish(queue, job)) {
    memcpy(binding->html_refresh_keys[candidate], job->index_key, sizeof(job->index_key));
...
static bool laghu_apache_attach_queue(laghu_runtime_queue *queue, volatile apr_uint32_t *attached, const char *path) {
...
static bool laghu_apache_attach_all_queues(void) {
  ...
  for (index = 0U; index < laghu_apache_queue_binding_count; ++index) {
    ...
    if (binding->html_refresh_queue_path[0] != '\0' &&
        !laghu_apache_attach_queue(&binding->html_refresh_queue, &binding->html_refresh_attached, binding->html_refresh_queue_path))
      complete = false;
...
static void *APR_THREAD_FUNC laghu_apache_queue_maintenance(apr_thread_t *thread, void *data) {
  unsigned int tick;
  (void)thread;
  (void)data;
...
static void laghu_apache_close_all_queues(void) {
  ...
  for (index = 0U; index < laghu_apache_queue_binding_count; ++index) {
    laghu_apache_queue_binding *binding = &laghu_apache_queue_bindings[index];
    if (apr_atomic_read32(&binding->image_attached) != 0U) laghu_runtime_queue_close(&binding->image_queue);
    if (apr_atomic_read32(&binding->font_attached) != 0U) laghu_runtime_queue_close(&binding->font_queue);
...
static bool laghu_apache_open_operational(uint64_t now) {
  ...
  for (attempt = 0U; attempt < 16U; ++attempt) {
    if (laghu_operational_registry_open(&laghu_apache_operational, laghu_apache_operational_cache, LAGHU_OPERATIONAL_SURFACE_APACHE,
...
void laghu_apache_child_init(apr_pool_t *pool, server_rec *server) {
  ...
  apr_pool_cleanup_register(pool, NULL, laghu_apache_rum_cleanup, apr_pool_cleanup_null);
  if (laghu_apache_operational_enabled && !laghu_apache_open_operational((uint64_t)apr_time_sec(apr_time_now())))
    ap_log_error(APLOG_MARK, APLOG_WARNING, 0, server, "Laghu operational registry unavailable; observability disabled");
  if (config != NULL && config->core.mode == LAGHU_MODE_ON &&
      !laghu_cache_backend_register_path(config->service.image_cache[0] != '\0' ? config->service.image_cache : LAGHU_APACHE_LIFECYCLE_CACHE,
                                         &config->service.cache_limits))
    ap_log_error(APLOG_MARK, APLOG_WARNING, 0, server, "Laghu file cache backend unavailable; serving origin");
  length = config != NULL && config->service.rum_snapshot_path[0] != '\0'
               ? snprintf(snapshot, sizeof(snapshot), "%s", config->service.rum_snapshot_path)
               : snprintf(snapshot, sizeof(snapshot), "%s/rum.snapshot", LAGHU_APACHE_LIFECYCLE_CACHE);
  if (length <= 0 || (size_t)length >= sizeof(snapshot)) return;
  options.snapshot_path = snapshot;
  if (config != NULL) {
    options.store_uri = config->service.rum_store;
    options.client_library = config->service.rum_client_library[0] == '\0' ? NULL : config->service.rum_client_library;
  ...
  if (laghu_apache_rum == NULL) {
    ap_log_error(APLOG_MARK, options.required ? APLOG_CRIT : APLOG_WARNING, 0, server, "Laghu RUM engine unavailable: %s", error);
    if (options.required) exit(APEXIT_CHILDFATAL);
    options.store_uri = "local:";
    options.client_library = NULL;
    options.required = false;
    laghu_apache_rum = laghu_rum_engine_create(&options, error, sizeof(error));
    if (laghu_apache_rum == NULL) ap_log_error(APLOG_MARK, APLOG_WARNING, 0, server, "Laghu local RUM fallback unavailable: %s", error);
  }
}
