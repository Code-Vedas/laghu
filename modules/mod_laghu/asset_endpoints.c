...
int laghu_apache_asset_endpoint(request_rec *request, laghu_apache_config *config) {
  ...
  css_asset =
      strlen(request->uri) == sizeof(css_prefix) - 1U + LAGHU_SHA256_HEX_LENGTH && strncmp(request->uri, css_prefix, sizeof(css_prefix) - 1U) == 0;
  javascript_asset = strlen(request->uri) == sizeof(javascript_prefix) - 1U + LAGHU_SHA256_HEX_LENGTH &&
                     strncmp(request->uri, javascript_prefix, sizeof(javascript_prefix) - 1U) == 0;
  javascript_map = strlen(request->uri) == sizeof(javascript_prefix) - 1U + LAGHU_SHA256_HEX_LENGTH + 4U &&
                   strncmp(request->uri, javascript_prefix, sizeof(javascript_prefix) - 1U) == 0 &&
                   strcmp(request->uri + strlen(request->uri) - 4U, ".map") == 0;
  media_asset = strlen(request->uri) == sizeof(media_prefix) - 1U + LAGHU_SHA256_HEX_LENGTH &&
                strncmp(request->uri, media_prefix, sizeof(media_prefix) - 1U) == 0;
  if (!css_asset && !javascript_asset && !javascript_map && !media_asset &&
      (strlen(request->uri) != sizeof(prefix) - 1U + LAGHU_SHA256_HEX_LENGTH || strncmp(request->uri, prefix, sizeof(prefix) - 1U) != 0)) {
                                                               ...
                                                               : sizeof(prefix) - 1U);
  {
    ...
    for (offset = 0U; offset < LAGHU_SHA256_HEX_LENGTH; ++offset) {
      if (!((key[offset] >= '0' && key[offset] <= '9') || (key[offset] >= 'a' && key[offset] <= 'f'))) {
        return HTTP_NOT_FOUND;
      }
    }
  }
  {
    ...
    for (index = 0U; index < result.header_operation_count; ++index) {
      ...
      if (strcmp(operation->name, "Content-Type") == 0 && operation->value != NULL) {
        apr_cpystrn(entry.content_type, operation->value, sizeof(entry.content_type));
      }
    }
    laghu_http_transaction_result_release(&result);
  }
