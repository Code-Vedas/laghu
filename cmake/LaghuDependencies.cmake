...
function(laghu_require_system_dependency id)
  ...
  set(system_include_directories ${${found_prefix}_INCLUDE_DIRS})
  ...
  if(id STREQUAL brotli)
    pkg_check_modules(LAGHU_SYSTEM_brotli_decoder QUIET libbrotlidec)
    if(NOT LAGHU_SYSTEM_brotli_decoder_FOUND)
      message(FATAL_ERROR "Laghu dependency gate failed: dependency=brotli rule=decoder_not_found")
    endif()
    if(LAGHU_SYSTEM_brotli_decoder_VERSION VERSION_LESS 1.0.9)
      message(FATAL_ERROR
        "Laghu dependency gate failed: dependency=brotli rule=decoder_version_too_old found=${LAGHU_SYSTEM_brotli_decoder_VERSION} required=1.0.9")
    endif()
    list(APPEND system_include_directories ${LAGHU_SYSTEM_brotli_decoder_INCLUDE_DIRS})
    if(system_LINK_MODE STREQUAL "STATIC")
      set(decoder_libraries ${LAGHU_SYSTEM_brotli_decoder_STATIC_LIBRARIES})
      set(decoder_library_directories ${LAGHU_SYSTEM_brotli_decoder_STATIC_LIBRARY_DIRS})
      laghu_prove_system_static_artifacts(
        "${id}" "${decoder_libraries}" "${decoder_library_directories}" decoder_libraries)
      list(APPEND system_libraries ${decoder_libraries})
    else()
      list(APPEND system_libraries ${LAGHU_SYSTEM_brotli_decoder_LINK_LIBRARIES})
    endif()
  endif()
  ...
  if(NOT system_TARGET STREQUAL "")
    ...
    target_include_directories("${system_TARGET}" SYSTEM INTERFACE ${system_include_directories})
...
function(laghu_acquire_vendored_cmake_dependency id private_target)
  ...
  if(id STREQUAL brotli)
    set(BROTLI_BUILD_TOOLS OFF CACHE BOOL "Build Brotli tools" FORCE)
    set(BROTLI_DISABLE_TESTS ON CACHE BOOL "Build Brotli tests" FORCE)
  endif()
  if(id STREQUAL zstd)
    set(ZSTD_BUILD_PROGRAMS OFF CACHE BOOL "Build Zstandard programs" FORCE)
    set(ZSTD_BUILD_TESTS OFF CACHE BOOL "Build Zstandard tests" FORCE)
  ...
  if(id STREQUAL brotli)
    target_link_libraries("${private_target}" INTERFACE brotlidec brotlicommon)
    if(LAGHU_DEPENDENCY_LINK_MODE STREQUAL STATIC)
      laghu_add_static_artifact_proof("${id}" brotlidec brotli_decoder_static_proof)
      laghu_add_static_artifact_proof("${id}" brotlicommon brotli_common_static_proof)
      add_dependencies("${private_target}"
        "${brotli_decoder_static_proof}" "${brotli_common_static_proof}")
    endif()
  endif()
  if(id STREQUAL ngtcp2 OR id STREQUAL nghttp3)
