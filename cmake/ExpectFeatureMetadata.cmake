...
foreach(feature IN ITEMS core tls http2 http3 structured_data async_dns regex compression_zlib compression_brotli compression_zstd geoip otlp idna password_auth)
  if(NOT metadata MATCHES "\"${feature}\": ")
    message(FATAL_ERROR "Laghu feature metadata expectation failed: missing_feature=${feature}")
  endif()
endforeach()
string(JSON requested_count LENGTH "${metadata}" requested)
string(JSON effective_count LENGTH "${metadata}" effective)
if(requested_count LESS 1 OR effective_count LESS 1)
  message(FATAL_ERROR "Laghu feature metadata expectation failed: selected_features_empty")
endif()
math(EXPR requested_last "${requested_count} - 1")
math(EXPR effective_last "${effective_count} - 1")
set(requested_features)
set(effective_features)
foreach(index RANGE ${requested_last})
  string(JSON feature GET "${metadata}" requested ${index})
  list(APPEND requested_features "${feature}")
endforeach()
foreach(index RANGE ${effective_last})
  string(JSON feature GET "${metadata}" effective ${index})
  list(APPEND effective_features "${feature}")
endforeach()
list(FIND effective_features core effective_core_index)
if(effective_core_index EQUAL -1)
  message(FATAL_ERROR "Laghu feature metadata expectation failed: core_not_effective")
endif()
foreach(feature IN LISTS requested_features)
  list(FIND effective_features "${feature}" effective_index)
  if(effective_index EQUAL -1)
    message(FATAL_ERROR "Laghu feature metadata expectation failed: requested_feature_not_effective=${feature}")
  endif()
endforeach()
