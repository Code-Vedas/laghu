...
function(laghu_configure_supply_chain)
  ...
  set(active_dependencies ${LAGHU_ACTIVE_DEPENDENCIES})
  list(SORT active_dependencies)
  foreach(id IN LISTS active_dependencies)
    laghu_dependency_property("${id}" LICENSE_EXPRESSION license)
    laghu_dependency_property("${id}" SOURCE_ONLY source_only)
    laghu_dependency_property("${id}" VENDORED_VERSION vendored_version)
    get_property(version GLOBAL PROPERTY "LAGHU_DEPENDENCY_SELECTED_VERSION_${id}")
    if(version STREQUAL "")
      set(version "${vendored_version}")
    endif()
    if(source_only)
      set(source VENDORED)
      set(linkage SOURCE_ONLY)
    else()
      set(source "${LAGHU_DEPENDENCY_SOURCE}")
      set(linkage "${LAGHU_DEPENDENCY_LINK_MODE}")
    endif()

    list(FIND active_dependencies "${id}" dependency_index)
    ...
    string(JSON verification GET "${manifest}" dependencies ${dependency_index} verification)
