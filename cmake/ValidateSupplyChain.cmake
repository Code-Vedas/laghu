...
string(JSON manifest_schema GET "${manifest}" schema_version)
...
if(NOT manifest_schema STREQUAL "laghu-build-manifest-v1")
  message(FATAL_ERROR "Laghu supply-chain validation failed: rule=manifest_schema_version")
endif()
...
set(expected_spdx_id "https://codevedas.com/laghu/spdx/${build_id}")
laghu_find_json_object(spdx_document_index "${spdx}" type SpdxDocument @graph)
laghu_require_json_value("${spdx}" "${expected_spdx_id}" spdx_build_id_mismatch
  @graph ${spdx_document_index} spdxId)
laghu_find_json_object(cdx_build_id_index "${cyclonedx}" name laghu:build-id
  metadata component properties)
laghu_require_json_value("${cyclonedx}" "${build_id}" cyclonedx_build_id_mismatch
  metadata component properties ${cdx_build_id_index} value)
