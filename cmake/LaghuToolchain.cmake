...
function(laghu_compile_probe capability source require_link)
  ...
  if(capability STREQUAL "posix_2008" AND NOT CMAKE_SYSTEM_NAME STREQUAL "Darwin")
    list(APPEND probe_definitions LAGHU_POSIX_REQUIRE_NUMERIC)
  endif()
...
function(laghu_add_validation_tests)
  ...
  foreach(capability IN ITEMS if_consteval expected byteswap to_underlying unreachable)
    add_test(NAME "laghu.toolchain.negative.${capability}" COMMAND "${CMAKE_COMMAND}" -DCXX=${CMAKE_CXX_COMPILER} -DSOURCE=${CMAKE_SOURCE_DIR}/tests/toolchain/negative/${capability}.cpp -DEXPECT_FAIL=ON -DEXPECT_TEXT=laghu\ forced-negative\ capability=${capability} -P "${expect_compile}")
  endforeach()
  add_test(NAME laghu.toolchain.cxx20_rejected COMMAND "${CMAKE_COMMAND}" -DCXX=${CMAKE_CXX_COMPILER} -DSOURCE=${CMAKE_SOURCE_DIR}/tests/toolchain/negative/cxx20-rejected.cpp -DSTANDARD=c++20 -DEXPECT_FAIL=ON -P "${expect_compile}")
  foreach(fixture IN ITEMS throw dynamic-cast)
    add_test(NAME "laghu.profile.compile_negative.${fixture}" COMMAND "${CMAKE_COMMAND}" -DCXX=${CMAKE_CXX_COMPILER} -DSOURCE=${CMAKE_SOURCE_DIR}/tests/profile/compile-negative/${fixture}.cpp "-DFLAGS=-fno-exceptions;-fno-rtti" -DEXPECT_FAIL=ON -P "${expect_compile}")
  endforeach()
  add_test(NAME laghu.profile.value_type COMMAND "${CMAKE_COMMAND}" -DCXX=${CMAKE_CXX_COMPILER} -DSOURCE=${CMAKE_SOURCE_DIR}/tests/profile/positive/value_type.cpp "-DFLAGS=-fno-exceptions;-fno-rtti" -DEXPECT_FAIL=OFF -P "${expect_compile}")
  add_test(NAME laghu.profile.third_party_isolated COMMAND "${CMAKE_COMMAND}" -DCXX=${CMAKE_CXX_COMPILER} -DSOURCE=${CMAKE_SOURCE_DIR}/tests/profile/third-party/throws.cpp -DEXPECT_FAIL=OFF -P "${expect_compile}")
  ...
  add_test(NAME laghu.warning.first_party_rejected COMMAND "${CMAKE_COMMAND}" -DCXX=${CMAKE_CXX_COMPILER} -DSOURCE=${CMAKE_SOURCE_DIR}/tests/warnings/first-party/conversion.cpp "-DFLAGS=${LAGHU_EFFECTIVE_WARNING_FLAGS}" -DEXPECT_FAIL=ON -P "${expect_compile}")
  add_test(NAME laghu.warning.third_party_isolated COMMAND "${CMAKE_COMMAND}" -DCXX=${CMAKE_CXX_COMPILER} -DSOURCE=${CMAKE_SOURCE_DIR}/tests/warnings/third-party/conversion.cpp -DEXPECT_FAIL=OFF -P "${expect_compile}")
