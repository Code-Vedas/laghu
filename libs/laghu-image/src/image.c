...
static bool laghu_image_filter_needs(laghu_image_filter_mask filter,
                                     laghu_image_capability_mask available,
                                     laghu_image_capability_mask required) {
  return (filter & LAGHU_IMAGE_FILTER_ALL) != 0U &&
...
laghu_image_format laghu_image_detect_format(laghu_buffer input) {
  ...
  if (input.length >= 8U && bytes[0] == 0x89U && bytes[1] == 'P' &&
      bytes[2] == 'N' && bytes[3] == 'G' && bytes[4] == 0x0dU &&
  ...
  if (input.length >= 12U && memcmp(bytes, "RIFF", 4U) == 0 &&
...
laghu_image_filter_mask laghu_image_effective_filters(
  ...
  if (!laghu_image_filter_needs(
          requested, capabilities,
          LAGHU_IMAGE_CAP_JPEG_LOAD | LAGHU_IMAGE_CAP_JPEG_SAVE)) {
    effective &= ~(LAGHU_IMAGE_RECOMPRESS_JPEG | LAGHU_IMAGE_JPEG_PROGRESSIVE |
                   LAGHU_IMAGE_JPEG_SAMPLING);
  }
  if (!laghu_image_filter_needs(
          requested, capabilities,
          LAGHU_IMAGE_CAP_PNG_LOAD | LAGHU_IMAGE_CAP_PNG_SAVE)) {
    effective &= ~LAGHU_IMAGE_RECOMPRESS_PNG;
  }
  if (!laghu_image_filter_needs(
          requested, capabilities,
          LAGHU_IMAGE_CAP_WEBP_LOAD | LAGHU_IMAGE_CAP_WEBP_SAVE)) {
    effective &= ~LAGHU_IMAGE_RECOMPRESS_WEBP;
  }
  if ((capabilities &
       (LAGHU_IMAGE_CAP_JPEG_LOAD | LAGHU_IMAGE_CAP_WEBP_SAVE)) !=
      (LAGHU_IMAGE_CAP_JPEG_LOAD | LAGHU_IMAGE_CAP_WEBP_SAVE)) {
    effective &= ~LAGHU_IMAGE_JPEG_TO_WEBP;
  }
  if ((capabilities & (LAGHU_IMAGE_CAP_PNG_LOAD | LAGHU_IMAGE_CAP_JPEG_SAVE)) !=
      (LAGHU_IMAGE_CAP_PNG_LOAD | LAGHU_IMAGE_CAP_JPEG_SAVE)) {
    effective &= ~LAGHU_IMAGE_PNG_TO_JPEG;
  }
  if ((capabilities & (LAGHU_IMAGE_CAP_GIF_LOAD | LAGHU_IMAGE_CAP_PNG_SAVE)) !=
      (LAGHU_IMAGE_CAP_GIF_LOAD | LAGHU_IMAGE_CAP_PNG_SAVE)) {
  ...
  if ((capabilities & (LAGHU_IMAGE_CAP_JPEG_LOAD | LAGHU_IMAGE_CAP_PNG_LOAD |
                       LAGHU_IMAGE_CAP_GIF_LOAD | LAGHU_IMAGE_CAP_WEBP_LOAD)) ==
      0U) {
    effective = 0U;
  }
  return effective;
...
bool laghu_image_variant_key(const laghu_image_backend *backend,
  ...
  if (backend == NULL || request == NULL || policy_key == NULL ||
      output == NULL || backend->backend_id[0] == '\0' ||
      request->quality == 0U || request->quality > 100U ||
      (request->resize_filter &
       ~(LAGHU_IMAGE_RESIZE_ATTRIBUTE | LAGHU_IMAGE_RESIZE_RENDERED |
         LAGHU_IMAGE_RESIZE_MOBILE)) != 0U ||
      (request->resize_filter != 0U &&
       (request->resize_filter & (request->resize_filter - 1U)) != 0U) ||
  ...
  return laghu_sha256_hex(
      (laghu_buffer){(const unsigned char *)canonical, (size_t)length}, output);
}

#if LAGHU_HAVE_VIPS

static bool laghu_vips_has_operation(const char *name) {
  return vips_type_find("VipsOperation", name) != 0U;
}

static void laghu_vips_probe_codec(laghu_image_format format,
                                   const char *load_operation,
                                   const char *save_operation,
                                   const unsigned char *fixture,
                                   size_t fixture_length, bool *can_load,
                                   bool *can_save) {
  ...
  *can_save = false;
  switch (format) {
    case LAGHU_IMAGE_FORMAT_JPEG:
      if (load_registered) {
        load_status = vips_jpegload_buffer((void *)(uintptr_t)fixture,
                                           fixture_length, &decoded, NULL);
      }
      break;
    case LAGHU_IMAGE_FORMAT_PNG:
      if (load_registered) {
        load_status = vips_pngload_buffer((void *)(uintptr_t)fixture,
                                          fixture_length, &decoded, NULL);
      }
      break;
    case LAGHU_IMAGE_FORMAT_GIF:
      if (load_registered) {
        load_status = vips_gifload_buffer((void *)(uintptr_t)fixture,
                                          fixture_length, &decoded, NULL);
      }
      break;
    case LAGHU_IMAGE_FORMAT_WEBP:
      if (load_registered) {
        load_status = vips_webpload_buffer((void *)(uintptr_t)fixture,
                                           fixture_length, &decoded, NULL);
      }
      break;
    case LAGHU_IMAGE_FORMAT_UNKNOWN:
  ...
  *can_load = load_status == 0;
  ...
  if (save_registered && vips_black(&source, 2, 2, "bands", 3, NULL) == 0) {
    switch (format) {
      case LAGHU_IMAGE_FORMAT_JPEG:
        save_status =
            vips_jpegsave_buffer(source, &encoded, &encoded_length, NULL);
        break;
      case LAGHU_IMAGE_FORMAT_PNG:
        save_status =
            vips_pngsave_buffer(source, &encoded, &encoded_length, NULL);
        break;
      case LAGHU_IMAGE_FORMAT_GIF:
        save_status =
            vips_gifsave_buffer(source, &encoded, &encoded_length, NULL);
        break;
      case LAGHU_IMAGE_FORMAT_WEBP:
        save_status =
            vips_webpsave_buffer(source, &encoded, &encoded_length, NULL);
        break;
      case LAGHU_IMAGE_FORMAT_UNKNOWN:
      default:
        break;
...
static bool laghu_vips_probe_animation(void) {
  ...
  if (vips_gifload_buffer((void *)(uintptr_t)laghu_probe_animated_gif,
                          sizeof(laghu_probe_animated_gif) - 1U, &gif, "n", 2,
                          NULL) != 0 ||
      vips_webpsave_buffer(gif, &webp_bytes, &webp_length, "lossless", TRUE,
                           NULL) != 0 ||
      vips_webpload_buffer(webp_bytes, webp_length, &webp, "n", 2, NULL) != 0) {
    goto cleanup;
  }
  success =
      vips_image_get_n_pages(gif) == 2 && vips_image_get_n_pages(webp) == 2 &&
      vips_image_hasalpha(gif) && vips_image_hasalpha(webp) &&
      vips_image_get_array_int(gif, "delay", &gif_delays, &gif_delay_count) ==
          0 &&
      vips_image_get_array_int(webp, "delay", &webp_delays,
                               &webp_delay_count) == 0 &&
      gif_delay_count == webp_delay_count && gif_delay_count == 2 &&
      memcmp(gif_delays, webp_delays,
             (size_t)gif_delay_count * sizeof(*gif_delays)) == 0 &&
      vips_image_get_int(gif, "loop", &gif_loop) == 0 &&
      vips_image_get_int(webp, "loop", &webp_loop) == 0 &&
      gif_loop == webp_loop;
...
bool laghu_image_backend_probe(laghu_image_backend *backend) {
  ...
  laghu_vips_probe_codec(LAGHU_IMAGE_FORMAT_JPEG, "jpegload_buffer",
                         "jpegsave_buffer", laghu_probe_jpeg,
                         sizeof(laghu_probe_jpeg) - 1U, &jpeg_load, &jpeg_save);
  laghu_vips_probe_codec(LAGHU_IMAGE_FORMAT_PNG, "pngload_buffer",
                         "pngsave_buffer", laghu_probe_png,
                         sizeof(laghu_probe_png) - 1U, &png_load, &png_save);
  laghu_vips_probe_codec(LAGHU_IMAGE_FORMAT_GIF, "gifload_buffer",
                         "gifsave_buffer", laghu_probe_gif,
                         sizeof(laghu_probe_gif) - 1U, &gif_load, &gif_save);
  ...
  (void)snprintf(backend->backend_id, sizeof(backend->backend_id),
                 "%s-libvips-%d.%d.%d-cap-%08x", LAGHU_IMAGE_BUILD_ID,
                 vips_version(0), vips_version(1), vips_version(2),
                 backend->capabilities);
  return true;
}

static int laghu_image_load(laghu_image_format format, laghu_buffer input,
                            unsigned int max_frames, VipsImage **image) {
  ...
  switch (format) {
    case LAGHU_IMAGE_FORMAT_JPEG:
      return vips_jpegload_buffer(bytes, input.length, image, "access",
                                  VIPS_ACCESS_RANDOM, NULL);
    case LAGHU_IMAGE_FORMAT_PNG:
      return vips_pngload_buffer(bytes, input.length, image, "access",
                                 VIPS_ACCESS_RANDOM, NULL);
    case LAGHU_IMAGE_FORMAT_GIF:
      status = vips_gifload_buffer(bytes, input.length, &first_page, "n", 1,
  ...
  if (format == LAGHU_IMAGE_FORMAT_GIF) {
    return vips_gifload_buffer(bytes, input.length, image, "n", frames,
                               "access", VIPS_ACCESS_RANDOM, NULL);
  }
  return vips_webpload_buffer(bytes, input.length, image, "n", frames, "access",
                              VIPS_ACCESS_RANDOM, NULL);
}
...
static bool laghu_image_dimensions_valid(VipsImage *image,
                                         const laghu_image_request *request,
                                         unsigned int *width,
                                         unsigned int *height,
                                         unsigned int *frames) {
  ...
  *frames = laghu_image_frames(image);
  ...
  return *width > 0U && *height > 0U && *width <= request->max_dimension &&
         *height <= request->max_dimension && *frames <= request->max_frames &&
         pixels <= request->max_pixels;
}

static bool laghu_image_is_opaque(VipsImage *image,
  ...
  bool has_alpha = vips_image_hasalpha(image) || bands == 2 ||
                   (bands == 4 && input_format != LAGHU_IMAGE_FORMAT_JPEG);
  ...
  if (vips_extract_band(image, &alpha, vips_image_get_bands(image) - 1, NULL) !=
          0 ||
      vips_min(alpha, &minimum, NULL) != 0 ||
...
static int laghu_image_prepare(VipsImage *input,
                               const laghu_image_request *request,
                               unsigned int frames, VipsImage **prepared,
  ...
  if (frames == 1U && (target_width > 0U || target_height > 0U) &&
    ...
    if (target_width < width || target_height < height) {
      ...
      if (vips_resize(oriented, &resized, scale, "kernel", VIPS_KERNEL_LANCZOS3,
                      NULL) != 0) {
...
static int laghu_image_save(VipsImage *image, laghu_image_format format,
                            const laghu_image_request *request, bool lossless,
                            bool progressive, bool sampling, void **output,
  ...
  switch (format) {
    case LAGHU_IMAGE_FORMAT_JPEG:
      return vips_jpegsave_buffer(
          image, output, output_length, "Q", (int)request->quality,
          "optimize_coding", TRUE, "interlace", progressive, "subsample_mode",
          sampling ? VIPS_FOREIGN_SUBSAMPLE_ON : VIPS_FOREIGN_SUBSAMPLE_AUTO,
          "keep", keep, NULL);
    case LAGHU_IMAGE_FORMAT_PNG:
      return vips_pngsave_buffer(image, output, output_length, "compression", 9,
                                 "filter", VIPS_FOREIGN_PNG_FILTER_ALL, "keep",
                                 keep, NULL);
    case LAGHU_IMAGE_FORMAT_WEBP:
...
static bool laghu_image_alpha_equal(VipsImage *expected, VipsImage *decoded) {
  ...
  if (expected_max == 0.0 || decoded_max == 0.0 ||
      vips_extract_band(expected, &expected_alpha,
                        vips_image_get_bands(expected) - 1, NULL) != 0 ||
      vips_extract_band(decoded, &decoded_alpha,
                        vips_image_get_bands(decoded) - 1, NULL) != 0 ||
      vips_linear1(expected_alpha, &expected_normalized, 1.0 / expected_max,
                   0.0, NULL) != 0 ||
      vips_linear1(decoded_alpha, &decoded_normalized, 1.0 / decoded_max, 0.0,
                   NULL) != 0) {
    goto cleanup;
  }
  expected_pixels =
      vips_image_write_to_memory(expected_normalized, &expected_size);
  decoded_pixels =
      vips_image_write_to_memory(decoded_normalized, &decoded_size);
  equal = expected_pixels != NULL && decoded_pixels != NULL &&
...
static bool laghu_image_candidate_valid(laghu_buffer candidate,
                                        laghu_image_format format,
                                        VipsImage *expected, bool lossless,
  ...
  valid = width == expected_width && height == expected_height &&
  ...
  if (valid && frames > 1U) {
    ...
    valid = expected_has_delay == decoded_has_delay &&
            expected_has_loop == decoded_has_loop;
    if (valid && expected_has_delay) {
      valid = vips_image_get_array_int(expected, "delay", &expected_delays,
                                       &expected_delay_count) == 0 &&
              vips_image_get_array_int(decoded, "delay", &decoded_delays,
                                       &decoded_delay_count) == 0 &&
              expected_delay_count == decoded_delay_count &&
              (expected_delay_count == 0 ||
               memcmp(expected_delays, decoded_delays,
                      (size_t)expected_delay_count *
                          sizeof(*expected_delays)) == 0);
    }
    if (valid && expected_has_loop) {
      valid = vips_image_get_int(expected, "loop", &expected_loop) == 0 &&
              vips_image_get_int(decoded, "loop", &decoded_loop) == 0 &&
  ...
  if (valid && lossless) {
    ...
    void *expected_pixels =
        vips_image_write_to_memory(expected, &expected_size);
    void *decoded_pixels = vips_image_write_to_memory(decoded, &decoded_size);
    valid = expected_pixels != NULL && decoded_pixels != NULL &&
            expected_size == decoded_size &&
            memcmp(expected_pixels, decoded_pixels, expected_size) == 0;
    g_free(expected_pixels);
    g_free(decoded_pixels);
...
static void laghu_image_consider(laghu_image_result *result, void *candidate,
  ...
  laghu_buffer current =
      result->used_candidate ? result->selected : result->original;
  ...
  if (finalized.decision == LAGHU_CANDIDATE_ACCEPTED &&
      laghu_image_candidate_valid(proposed, format, expected, lossless, width,
                                  height, frames)) {
...
bool laghu_image_optimize(const laghu_image_backend *backend,
  ...
  if (request == NULL || result == NULL ||
      (request->original.data == NULL && request->original.length != 0U)) {
    return false;
  }
  ...
  if (request->quality > 100U || request->quality == 0U ||
      request->max_input_bytes == 0U ||
      request->max_input_bytes > LAGHU_IMAGE_MAX_INPUT_BYTES ||
      request->max_dimension == 0U ||
      request->max_dimension > LAGHU_IMAGE_MAX_DIMENSION ||
      request->max_pixels == 0U ||
      request->max_pixels > LAGHU_IMAGE_MAX_PIXELS ||
      request->max_frames == 0U ||
      request->max_frames > LAGHU_IMAGE_MAX_FRAMES ||
      (request->resize_filter &
       ~(LAGHU_IMAGE_RESIZE_ATTRIBUTE | LAGHU_IMAGE_RESIZE_RENDERED |
         LAGHU_IMAGE_RESIZE_MOBILE)) != 0U ||
  ...
  if (input_format == LAGHU_IMAGE_FORMAT_UNKNOWN ||
      laghu_image_load(input_format, request->original, request->max_frames,
                       &input) != 0) {
    vips_error_clear();
    result->input_rejected = true;
    return true;
  ...
  effective =
      laghu_image_effective_filters(request->filters, backend->capabilities);
  if (laghu_image_prepare(input, request, frames, &prepared,
                          &prepared_filters) != 0) {
    vips_error_clear();
    result->input_rejected = true;
    g_object_unref(input);
...
#define LAGHU_TRY_SAVE(target_format, lossless_value, filter_bits)          \
  do {                                                                      \
    ...
    if (laghu_image_save(candidate_source, target_format, request,          \
                         lossless_value,                                    \
                         (effective & LAGHU_IMAGE_JPEG_PROGRESSIVE) != 0U,  \
                         (effective & LAGHU_IMAGE_JPEG_SAMPLING) != 0U,     \
                         &candidate_bytes, &candidate_length) == 0) {       \
      laghu_image_consider(result, candidate_bytes, candidate_length,       \
                           target_format, prepared_filters | (filter_bits), \
                           candidate_source, lossless_value, width, height, \
                           frames);                                         \
    } else {                                                                \
      vips_error_clear();                                                   \
    }                                                                       \
  ...
  switch (input_format) {
    case LAGHU_IMAGE_FORMAT_JPEG:
      if (request->allow_lossy &&
          (effective & LAGHU_IMAGE_RECOMPRESS_JPEG) != 0U) {
        LAGHU_TRY_SAVE(
            LAGHU_IMAGE_FORMAT_JPEG, false,
            effective &
                (LAGHU_IMAGE_REWRITE_IMAGES | LAGHU_IMAGE_RECOMPRESS_JPEG |
                 LAGHU_IMAGE_JPEG_PROGRESSIVE | LAGHU_IMAGE_JPEG_SAMPLING |
                 LAGHU_IMAGE_STRIP_METADATA | LAGHU_IMAGE_STRIP_COLOR_PROFILE));
      }
      if (request->accept_webp && request->allow_lossy &&
          (effective & LAGHU_IMAGE_JPEG_TO_WEBP) != 0U) {
        LAGHU_TRY_SAVE(
            LAGHU_IMAGE_FORMAT_WEBP, false,
            effective &
                (LAGHU_IMAGE_REWRITE_IMAGES | LAGHU_IMAGE_JPEG_TO_WEBP |
                 LAGHU_IMAGE_STRIP_METADATA | LAGHU_IMAGE_STRIP_COLOR_PROFILE |
        ...
        LAGHU_TRY_SAVE(
            ...
            effective &
                (LAGHU_IMAGE_REWRITE_IMAGES | LAGHU_IMAGE_RECOMPRESS_IMAGES |
                 LAGHU_IMAGE_RECOMPRESS_PNG | LAGHU_IMAGE_STRIP_METADATA |
                 LAGHU_IMAGE_STRIP_COLOR_PROFILE));
      }
      if (opaque && request->allow_lossy &&
          (effective & LAGHU_IMAGE_PNG_TO_JPEG) != 0U) {
        bool jpeg_ready =
            !vips_image_hasalpha(prepared) ||
            vips_extract_band(prepared, &opaque_rgb, 0, "n",
                              vips_image_get_bands(prepared) - 1, NULL) == 0;
        if (!jpeg_ready) {
          vips_error_clear();
        } else {
          candidate_source = opaque_rgb != NULL ? opaque_rgb : prepared;
          LAGHU_TRY_SAVE(LAGHU_IMAGE_FORMAT_JPEG, false,
                         effective & (LAGHU_IMAGE_REWRITE_IMAGES |
                                      LAGHU_IMAGE_PNG_TO_JPEG |
                                      LAGHU_IMAGE_STRIP_METADATA |
                                      LAGHU_IMAGE_STRIP_COLOR_PROFILE |
                                      LAGHU_IMAGE_IN_PLACE_BROWSER));
          candidate_source = prepared;
        }
      }
      if (request->accept_webp &&
          (effective & LAGHU_IMAGE_TO_WEBP_LOSSLESS) != 0U) {
        LAGHU_TRY_SAVE(
            LAGHU_IMAGE_FORMAT_WEBP, true,
            effective &
    ...
    case LAGHU_IMAGE_FORMAT_GIF:
      if (frames == 1U && (effective & LAGHU_IMAGE_GIF_TO_PNG) != 0U) {
        LAGHU_TRY_SAVE(
            LAGHU_IMAGE_FORMAT_PNG, true,
            effective &
                (LAGHU_IMAGE_REWRITE_IMAGES | LAGHU_IMAGE_GIF_TO_PNG |
                 LAGHU_IMAGE_STRIP_METADATA | LAGHU_IMAGE_STRIP_COLOR_PROFILE));
      }
      if (request->accept_webp && frames == 1U &&
          (effective & LAGHU_IMAGE_TO_WEBP_LOSSLESS) != 0U) {
        LAGHU_TRY_SAVE(
            LAGHU_IMAGE_FORMAT_WEBP, true,
            effective &
      ...
      if (request->accept_webp && frames > 1U &&
          (effective & LAGHU_IMAGE_TO_WEBP_ANIMATED) != 0U) {
        LAGHU_TRY_SAVE(
            LAGHU_IMAGE_FORMAT_WEBP, !request->allow_lossy,
            effective &
                (LAGHU_IMAGE_REWRITE_IMAGES | LAGHU_IMAGE_TO_WEBP_ANIMATED |
                 LAGHU_IMAGE_STRIP_METADATA | LAGHU_IMAGE_STRIP_COLOR_PROFILE |
                 LAGHU_IMAGE_IN_PLACE_BROWSER));
      }
      break;
    case LAGHU_IMAGE_FORMAT_WEBP:
      if ((effective & (LAGHU_IMAGE_RECOMPRESS_IMAGES |
                        LAGHU_IMAGE_RECOMPRESS_WEBP)) != 0U) {
        bool lossless = (effective & LAGHU_IMAGE_RECOMPRESS_IMAGES) != 0U ||
                        !request->allow_lossy;
        LAGHU_TRY_SAVE(
            LAGHU_IMAGE_FORMAT_WEBP, lossless,
            effective &
                (LAGHU_IMAGE_REWRITE_IMAGES | LAGHU_IMAGE_RECOMPRESS_IMAGES |
                 LAGHU_IMAGE_RECOMPRESS_WEBP | LAGHU_IMAGE_STRIP_METADATA |
                 LAGHU_IMAGE_STRIP_COLOR_PROFILE |
                 LAGHU_IMAGE_IN_PLACE_BROWSER));
      }
      break;
...
bool laghu_image_build_sprite(const laghu_image_backend *backend,
  ...
  if (backend == NULL || !backend->available || items == NULL ||
      item_count < 2U || item_count > (size_t)INT_MAX || result == NULL ||
      (output_format != LAGHU_IMAGE_FORMAT_PNG &&
       output_format != LAGHU_IMAGE_FORMAT_WEBP)) {
    return false;
  }
  memset(result, 0, sizeof(*result));
  ...
  for (index = 0U; index < item_count; ++index) {
    laghu_image_format input_format =
        laghu_image_detect_format(items[index].original);
    unsigned int width;
    unsigned int height;
    unsigned int frames;
    ...
    if (input_format == LAGHU_IMAGE_FORMAT_UNKNOWN ||
        items[index].original.length > SIZE_MAX - original_length ||
        laghu_image_load(input_format, items[index].original, 1U,
                         &images[index]) != 0 ||
        !laghu_image_dimensions_valid(images[index], &request, &width, &height,
                                      &frames) ||
        frames != 1U || width > LAGHU_IMAGE_MAX_DIMENSION - offset) {
      goto cleanup;
    }
  ...
  if (vips_arrayjoin(images, &joined, (int)item_count, "across",
                     (int)item_count, NULL) != 0) {
    goto cleanup;
  }
  laghu_image_request_init(&request);
  request.quality = 100U;
  request.filters = LAGHU_IMAGE_SPRITE | LAGHU_IMAGE_STRIP_METADATA;
  if (laghu_image_save(joined, output_format, &request, true, false, false,
                       &encoded, &encoded_length) != 0 ||
      encoded_length >= original_length ||
      !laghu_image_candidate_valid((laghu_buffer){encoded, encoded_length},
                                   output_format, joined, true,
                                   (unsigned int)vips_image_get_width(joined),
...
bool laghu_image_backend_probe(laghu_image_backend *backend) {
  ...
  memset(backend, 0, sizeof(*backend));
  (void)snprintf(backend->backend_id, sizeof(backend->backend_id),
                 "%s-libvips-unavailable", LAGHU_IMAGE_BUILD_ID);
  return true;
}
