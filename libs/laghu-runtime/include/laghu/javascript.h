...
typedef struct laghu_rum_instrumentation_record laghu_rum_instrumentation_record;
...
typedef struct {
  enum { LAGHU_JAVASCRIPT_DELAY_DEFER = 0, LAGHU_JAVASCRIPT_DELAY_INTERACTION } mode;
...
bool laghu_runtime_rewrite_javascript_traced(laghu_runtime_queue *queue, const char *cache_path, laghu_buffer source, const char *normalized_path,
                                             const char *policy_key, const char *target, bool module, bool include_source_map,
                                             const laghu_trace_context *trace, laghu_runtime_javascript_result *result);
