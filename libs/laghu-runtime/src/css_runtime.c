...
static bool laghu_runtime_sprite_key(const char *policy_key, const laghu_runtime_queue *queue, laghu_runtime_css_storage *storage,
                                     const laghu_css_parse_result *discovery, const laghu_image_resource *resources, size_t *ordered, size_t count,
  ...
  for (index = 0U; index < count; ++index) {
    ...
    written = snprintf(material + length, sizeof(material) - length, "\n%s\n%s\n%u:%u", discovery->dependencies[item].source_url,
                       storage[item].variant_key, resources[item].width, resources[item].height);
    if (written <= 0 || (size_t)written >= sizeof(material) - length) {
      return false;
    }
    length += (size_t)written;
  }
  return laghu_sha256_hex((laghu_buffer){(const unsigned char *)material, length}, output);
}

static laghu_catalog_variant *laghu_runtime_css_variant(laghu_catalog_record *catalog) {
  unsigned int index;
  for (index = 0U; index < catalog->variant_count; ++index) {
    laghu_catalog_variant *variant = &catalog->variants[index];
    if (variant->ready && variant->variant_key[0] != '\0' && catalog->original_content_type[0] != '\0' &&
...
static bool laghu_runtime_css_key(laghu_buffer css, const char *stylesheet_path, const char *policy_key, uint32_t capability_mask,
                                  const laghu_image_resource *resources, size_t resource_count, bool minify, bool allow_sprites,
                                  const char *import_dependency_key, char output[LAGHU_RUNTIME_KEY_SIZE]) {
  char source_hash[LAGHU_RUNTIME_KEY_SIZE];
  size_t capacity =
      512U + strlen(stylesheet_path != NULL ? stylesheet_path : "") + resource_count * (LAGHU_IMAGE_URL_SIZE + LAGHU_RUNTIME_KEY_SIZE * 2U);
  char *material;
  size_t length;
  size_t index;
  int written;
  if (!laghu_sha256_hex(css, source_hash) || stylesheet_path == NULL || policy_key == NULL) {
    return false;
  }
  ...
  written = snprintf(material, capacity, "laghu-css-v%u\n%s\n%s\n%s\n%u\n%d:%d\n%s", LAGHU_CSS_DERIVATION_VERSION, source_hash, stylesheet_path,
                     policy_key, capability_mask, minify ? 1 : 0, allow_sprites ? 1 : 0, import_dependency_key != NULL ? import_dependency_key : "");
  ...
  for (index = 0U; index < resource_count; ++index) {
    ...
    written = snprintf(material + length, capacity - length, "\n%s\n%s\n%s", resource->source_url != NULL ? resource->source_url : "",
                       resource->source_hash != NULL ? resource->source_hash : "", resource->optimized_url != NULL ? resource->optimized_url : "");
...
bool laghu_runtime_rewrite_css(laghu_runtime_queue *queue, const char *cache_path, laghu_buffer css, const char *stylesheet_path,
  ...
  bool success = false;
  if (result == NULL || cache_path == NULL || stylesheet_path == NULL || policy_key == NULL || ttl_seconds == 0U || inline_limit > 65536U ||
      outline_threshold < 1024U || outline_threshold > 1048576U || css.length > LAGHU_CSS_MAX_INPUT_BYTES) {
    return false;
  }
  memset(result, 0, sizeof(*result));
  discovery = calloc(1U, sizeof(*discovery));
  if (discovery == NULL || !laghu_css_discover(css, stylesheet_path, page_origin, discovery) || !discovery->bounded ||
      !laghu_sha256_hex(css, source_hash)) {
    goto finished;
  }
  resources = calloc(discovery->dependency_count == 0U ? 1U : discovery->dependency_count, sizeof(*resources));
  storage = calloc(discovery->dependency_count == 0U ? 1U : discovery->dependency_count, sizeof(*storage));
  ...
  (void)snprintf(stylesheet.normalized_url, sizeof(stylesheet.normalized_url), "%s", stylesheet_path);
  ...
  for (index = 0U; index < discovery->dependency_count; ++index) {
    ...
    if (!laghu_catalog_lookup_url(cache_path, discovery->dependencies[index].source_url, policy_key, capability_mask, now, ttl_seconds, &catalog)) {
      pending = true;
      continue;
    }
    ...
    for (variant_index = 0U; variant_index < catalog.variant_count; ++variant_index) {
    ...
    if (!laghu_image_variant_url(variant->variant_key, storage[index].optimized)) {
      goto finished;
    }
    ...
    memcpy(storage[index].variant_key, variant->variant_key, sizeof(storage[index].variant_key));
  ...
  if (allow_sprites) {
    ...
    for (index = 0U; index < discovery->dependency_count && sprite_count < LAGHU_RUNTIME_MAX_SPRITE_INPUTS; ++index) {
      if (discovery->dependencies[index].sprite_eligible && storage[index].variant_key[0] != '\0') {
    ...
    for (outer = 0U; outer < sprite_count; ++outer) {
      ...
      for (inner = outer + 1U; inner < sprite_count; ++inner) {
        if (strcmp(discovery->dependencies[ordered[outer]].source_url, discovery->dependencies[ordered[inner]].source_url) > 0) {
    ...
    if (sprite_count >= 2U && laghu_runtime_sprite_key(policy_key, queue, storage, discovery, resources, ordered, sprite_count, sprite_key)) {
      if (laghu_runtime_cache_lookup_variant(cache_path, sprite_key, &sprite_entry)) {
        ...
        for (outer = 0U; outer < sprite_count; ++outer) {
          size_t item = ordered[outer];
          memcpy(job.sprite_variant_keys[outer], storage[item].variant_key, LAGHU_RUNTIME_KEY_SIZE);
  ...
  if (allow_sprites && discovery->has_imports) {
    if (!laghu_runtime_flatten_css_imports(cache_path, css, stylesheet_path, page_origin, policy_key, capability_mask, now, ttl_seconds, inline_limit,
                                           outline_threshold, &imports)) {
      goto finished;
    }
  ...
  if (!laghu_runtime_css_key(css, stylesheet_path, policy_key, capability_mask, resources, discovery->dependency_count, minify, allow_sprites,
                             imports.flattened ? imports.dependency_key : NULL, derivation_key)) {
    goto finished;
  }
  memcpy(result->dependency_key, derivation_key, sizeof(result->dependency_key));
  if (laghu_runtime_cache_lookup_variant(cache_path, derivation_key, &entry)) {
    memcpy(stylesheet.derived_key, derivation_key, sizeof(stylesheet.derived_key));
    memcpy(stylesheet.dependency_key, derivation_key, sizeof(stylesheet.dependency_key));
  ...
  if (discovery->valid && minify) {
    success = laghu_css_minify_and_rewrite(transform_css, stylesheet_path, page_origin, &options, false, &rewritten);
  } else {
    success = laghu_css_fallback_rewrite_urls(css, stylesheet_path, page_origin, &options, &rewritten);
  ...
  if ((rewritten.length >= css.length && rewritten.length - css.length >= byte_savings) || rewritten.applied_filters == 0U) {
    stylesheet.terminally_excluded = true;
    (void)laghu_stylesheet_publish(cache_path, &stylesheet);
