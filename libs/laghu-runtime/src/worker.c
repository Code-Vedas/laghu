...
bool laghu_worker_lifecycle_start(laghu_worker_lifecycle *lifecycle,
                                  ...
                                  uint64_t now) {
  if (lifecycle == NULL || cache_path == NULL || cache_path[0] == '\0')
    return false;
  lifecycle->queue = queue;
  lifecycle->started = laghu_operational_registry_open(
      &lifecycle->registry, cache_path, LAGHU_OPERATIONAL_SURFACE_WORKER, kind,
      required, now);
...
void laghu_worker_lifecycle_heartbeat(laghu_worker_lifecycle *lifecycle,
  ...
  (void)laghu_operational_registry_heartbeat(&lifecycle->registry, now, healthy,
                                             capacity, occupied);
}
...
void laghu_worker_lifecycle_job(laghu_worker_lifecycle *lifecycle, bool success,
  ...
  if (lifecycle == NULL || !lifecycle->started) return;
  laghu_operational_registry_worker_job(&lifecycle->registry, success,
                                        elapsed_microseconds, failure);
}
...
void laghu_worker_lifecycle_stop(laghu_worker_lifecycle *lifecycle,
  ...
  if (lifecycle == NULL) return;
  if (lifecycle->started)
    (void)laghu_operational_registry_heartbeat(&lifecycle->registry, now, false,
                                               0U, 0U);
