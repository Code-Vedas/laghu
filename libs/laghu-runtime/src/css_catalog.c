...
static bool laghu_stylesheet_key(const char *normalized_url,
  ...
  length = snprintf(
      material, sizeof(material), "stylesheet-catalog-v%u\n%s\n%s\n%u\n%u\n%u",
      LAGHU_STYLESHEET_CATALOG_VERSION, normalized_url, policy_key,
      capability_mask, inline_limit, outline_threshold);
...
bool laghu_stylesheet_publish(const char *cache_path,
  ...
  if (record == NULL || record->version != LAGHU_STYLESHEET_CATALOG_VERSION ||
      !laghu_stylesheet_key(record->normalized_url, record->policy_key,
                            record->capability_mask, record->inline_limit,
                            record->outline_threshold, key)) {
    return false;
  }
  return laghu_runtime_cache_publish(
      cache_path, key, key, record->source_hash,
      "application/x-laghu-stylesheet-catalog", "laghu-css-catalog-v1",
...
bool laghu_stylesheet_lookup(const char *cache_path, const char *normalized_url,
  ...
  if (record == NULL || ttl_seconds == 0U ||
      !laghu_stylesheet_key(normalized_url, policy_key, capability_mask,
                            inline_limit, outline_threshold, key) ||
      ...
      !laghu_runtime_cache_read(&entry, (unsigned char *)record,
                                sizeof(*record)) ||
      record->version != LAGHU_STYLESHEET_CATALOG_VERSION ||
      strcmp(record->normalized_url, normalized_url) != 0 ||
      strcmp(record->policy_key, policy_key) != 0 ||
      record->capability_mask != capability_mask ||
      record->inline_limit != inline_limit ||
      record->outline_threshold != outline_threshold ||
