...
#define LAGHU_JAVASCRIPT_DEFAULT_TARGET "defaults and supports es6-module and not dead"
...
bool laghu_javascript_target_normalize(const char *target, char output[LAGHU_JAVASCRIPT_TARGET_SIZE]) {
  ...
  for (; start < end; ++start) {
    ...
    if (iscntrl(value) || value == '$' || (value == '.' && start + 1U < end && target[start + 1U] == '.')) return false;
    if (isspace(value)) {
      space = written != 0U;
    } else {
...
static bool laghu_javascript_key(laghu_buffer source, const char *path, const char *policy, const char *target, bool module, bool include_source_map,
                                 char output[LAGHU_RUNTIME_KEY_SIZE], char source_hash[LAGHU_RUNTIME_KEY_SIZE]) {
  char canonical[LAGHU_RUNTIME_PATH_SIZE + LAGHU_RUNTIME_KEY_SIZE + LAGHU_JAVASCRIPT_TARGET_SIZE + 96U];
  int length;
  if (!laghu_sha256_hex(source, source_hash)) return false;
  length = snprintf(canonical, sizeof(canonical), "laghu-js-v%u\n%s\n%s\n%s\n%s\n%c\n%c", LAGHU_JAVASCRIPT_DERIVATION_VERSION, source_hash, path,
                    policy, target, module ? 'm' : 'c', include_source_map ? 's' : '-');
...
bool laghu_runtime_rewrite_javascript(laghu_runtime_queue *queue, const char *cache_path, laghu_buffer source, const char *normalized_path,
  ...
  if (queue == NULL || cache_path == NULL || source.data == NULL || source.length == 0U || source.length > LAGHU_JAVASCRIPT_MAX_BYTES ||
      normalized_path == NULL || policy_key == NULL || !laghu_javascript_target_normalize(target, normalized_target) ||
      !laghu_javascript_key(source, normalized_path, policy_key, normalized_target, module, include_source_map, key, source_hash))
    return false;
  if (laghu_runtime_cache_lookup(cache_path, key, source_hash, &entry) && entry.length > 0U && entry.length < source.length &&
      entry.length <= LAGHU_JAVASCRIPT_MAX_BYTES) {
    result->data = malloc(entry.length);
    if (result->data != NULL && laghu_runtime_cache_read(&entry, result->data, entry.length)) {
      result->length = entry.length;
      result->rewritten = true;
      memcpy(result->dependency_key, entry.payload_hash, sizeof(result->dependency_key));
      return true;
    }
    free(result->data);
    memset(result, 0, sizeof(*result));
  ...
  if (!laghu_copy(job.index_key, sizeof(job.index_key), key) || !laghu_copy(job.request_path, sizeof(job.request_path), normalized_path) ||
      ...
      !laghu_copy(job.content_type, sizeof(job.content_type), "application/javascript") ||
      !laghu_copy(job.policy_key, sizeof(job.policy_key), policy_key) ||
      !laghu_copy(job.javascript_target, sizeof(job.javascript_target), normalized_target))
    return false;
...
static unsigned char laghu_lower(unsigned char value) { return value >= 'A' && value <= 'Z' ? (unsigned char)(value + 32U) : value; }

static const unsigned char *laghu_find_ascii(const unsigned char *start, const unsigned char *end, const char *needle) {
...
static bool laghu_tag_attribute(const unsigned char *start, const unsigned char *end, const char *name, const unsigned char **value,
                                size_t *value_length) {
  ...
  while (start < end) {
    ...
    key = start;
    while (start < end && (isalnum(*start) || *start == '-' || *start == '_')) ++start;
    key_end = start;
    while (start < end && isspace(*start)) ++start;
    item = NULL;
    *value_length = 0U;
...
static bool laghu_tag_attributes_allowed(const unsigned char *start, const unsigned char *end, bool combine) {
  while (start < end) {
    ...
    allowed = (length == 3U && laghu_find_ascii(key, start, "src") == key) || (length == 4U && laghu_find_ascii(key, start, "type") == key) ||
              (length == 5U && laghu_find_ascii(key, start, "nonce") == key) ||
              (!combine && length == 2U && laghu_find_ascii(key, start, "id") == key) ||
              (length >= 5U && laghu_find_ascii(key, key + 5U, "data-") == key &&
               (!combine || (length == 18U && laghu_find_ascii(key, start, "data-laghu-combine") == key)));
...
static bool laghu_javascript_interaction_attributes(const unsigned char *start, const unsigned char *end) {
  ...
  while (start < end) {
    ...
    if (length == 0U || (!source && !nonce) || (source && source_seen) || (nonce && nonce_seen)) return false;
    source_seen |= source;
    nonce_seen |= nonce;
    while (start < end && isspace(*start)) ++start;
    if (start == end || *start++ != '=') return false;
...
static bool laghu_javascript_interaction_nonce_safe(const unsigned char *value, size_t length) {
  size_t index;
  if (value == NULL || length == 0U || length > 128U) return false;
  for (index = 0U; index < length; ++index)
    if (!(isalnum(value[index]) || value[index] == '+' || value[index] == '/' || value[index] == '=' || value[index] == '-' || value[index] == '_'))
      return false;
  return true;
}
...
static bool laghu_javascript_interaction_url_safe(const char *url) {
  size_t index;
  if (url == NULL || strncmp(url, "https://", 8U) != 0 || strpbrk(url, "?#\\*[]\\\"'<>") != NULL || strstr(url, "..") != NULL) return false;
  for (index = 0U; url[index] != '\0'; ++index)
    if ((unsigned char)url[index] <= 32U || (unsigned char)url[index] >= 127U) return false;
  return index > 9U && index < LAGHU_RUNTIME_PATH_SIZE;
}

static bool laghu_javascript_interaction_markup(const char *url, const unsigned char *nonce, size_t nonce_length, unsigned char **output,
  ...
  *output_length = 0U;
  if (nonce != NULL && nonce_length != 0U) {
    if (!laghu_javascript_interaction_nonce_safe(nonce, nonce_length) ||
        snprintf(deferred_nonce, sizeof(deferred_nonce), " data-laghu-interaction-nonce=\"%.*s\"", (int)nonce_length, nonce) <= 0 ||
        snprintf(loader_nonce, sizeof(loader_nonce), " nonce=\"%.*s\"", (int)nonce_length, nonce) <= 0)
      return false;
  }
...
static bool laghu_tag_attribute_span(const unsigned char *tag, const unsigned char *tag_end, const unsigned char *value, size_t value_length,
                                     const unsigned char **span_start, const unsigned char **span_end) {
  const unsigned char *cursor = value;
  const unsigned char *end = value + value_length;
  if (tag == NULL || tag_end == NULL || value == NULL || value < tag || end > tag_end) return false;
  ...
  --cursor;
  while (cursor > tag && isspace(cursor[-1])) --cursor;
  while (cursor > tag && (isalnum(cursor[-1]) || cursor[-1] == '-' || cursor[-1] == '_')) --cursor;
  while (cursor > tag && isspace(cursor[-1])) --cursor;
  *span_start = cursor;
  *span_end = end;
  return true;
...
static void laghu_javascript_nonce(const unsigned char *tag, const unsigned char *tag_end, const unsigned char **nonce, size_t *nonce_length) {
  *nonce = NULL;
  *nonce_length = 0U;
  (void)laghu_tag_attribute(tag, tag_end, "nonce", nonce, nonce_length);
}

static bool laghu_javascript_csp_allows_inline(const laghu_csp_policy *csp, const unsigned char *tag, const unsigned char *tag_end) {
...
static bool laghu_javascript_csp_allows_external(const laghu_csp_policy *csp, const unsigned char *tag, const unsigned char *tag_end) {
  const unsigned char *nonce;
  size_t nonce_length;
  laghu_javascript_nonce(tag, tag_end, &nonce, &nonce_length);
...
static bool laghu_javascript_safe_nonce(const unsigned char *nonce, size_t nonce_length) {
  size_t index;
  ...
  for (index = 0U; index < nonce_length; ++index) {
    ...
    if (padding != 0U || !(isalnum(value) || value == '+' || value == '/' || value == '-' || value == '_')) return false;
  }
  return true;
}

bool laghu_runtime_add_javascript_yield(laghu_buffer html, const laghu_csp_policy *csp, bool allow, laghu_runtime_html_result *result) {
  ...
  while ((cursor = laghu_find_ascii(cursor, end, "<script")) != NULL) {
    ...
    if (cursor + 7U < end && !isspace(cursor[7U]) && cursor[7U] != '>' && cursor[7U] != '/') {
      cursor += 7U;
      continue;
    }
    if (!laghu_tag_attribute(cursor + 7U, open_end, "data-laghu-yield", &marker, &marker_length) || marker == NULL ||
        marker_length != sizeof("cooperative") - 1U || memcmp(marker, "cooperative", sizeof("cooperative") - 1U) != 0 ||
        !laghu_tag_attribute(cursor + 7U, open_end, "nonce", &nonce, &nonce_length) || !laghu_javascript_safe_nonce(nonce, nonce_length) ||
        !laghu_csp_allows_inline_script(csp, nonce, nonce_length)) {
      cursor = open_end + 1U;
      continue;
    }
    helper_length = snprintf(NULL, 0U, helper, (int)nonce_length, nonce);
    if (helper_length <= 0 || (size_t)helper_length > SIZE_MAX - html.length) return true;
    ...
    (void)snprintf((char *)output + prefix, (size_t)helper_length + 1U, helper, (int)nonce_length, nonce);
    memcpy(output + prefix + (size_t)helper_length, cursor, html.length - prefix);
    output[output_length] = '\0';
    result->data = output;
    result->length = output_length;
    result->rewritten = true;
    (void)laghu_sha256_hex((laghu_buffer){(const unsigned char *)helper, sizeof(helper) - 1U}, result->dependency_key);
    return true;
  }
  return true;
}

static bool laghu_javascript_external_lookup(const char *cache_path, const char *url, const char *policy_key, const char *target, bool module,
                                             uint64_t now, unsigned int ttl_seconds, laghu_javascript_catalog_record *record) {
  ...
  read_success = fread(catalog, 1U, sizeof(catalog), file) == sizeof(catalog) && fgetc(file) == EOF;
  ...
  if (!laghu_sha256_hex((laghu_buffer){catalog, 1840U}, checksum) || magic != LAGHU_JAVASCRIPT_CATALOG_MAGIC || version != 3U ||
      stored_module != (module ? 1U : 0U) || updated > now || now - updated > ttl_seconds || source_length == 0U || variant_length == 0U ||
      variant_length >= source_length || memchr(catalog + 40U, '\0', 1024U) == NULL || memchr(catalog + 1064U, '\0', 65U) == NULL ||
      memchr(catalog + 1129U, '\0', 512U) == NULL || memchr(catalog + 1840U, '\0', 65U) == NULL || strcmp((const char *)catalog + 40U, url) != 0 ||
      strcmp((const char *)catalog + 1064U, policy_key) != 0 || strcmp((const char *)catalog + 1129U, target) != 0 ||
      strcmp((const char *)catalog + 1840U, checksum) != 0 ||
      (flags & ~(LAGHU_JAVASCRIPT_FLAG_MODULE | LAGHU_JAVASCRIPT_FLAG_URL_INDEPENDENT | LAGHU_JAVASCRIPT_FLAG_CONCAT_SAFE |
                 LAGHU_JAVASCRIPT_FLAG_TOP_LEVEL_DECLARATION_FREE | LAGHU_JAVASCRIPT_FLAG_DEFER_SAFE)) != 0U ||
      (((flags & LAGHU_JAVASCRIPT_FLAG_MODULE) != 0U) != module) || (module && (flags & LAGHU_JAVASCRIPT_FLAG_CONCAT_SAFE) != 0U) ||
      memchr(catalog + 1706U, '\0', LAGHU_RUNTIME_KEY_SIZE) == NULL ||
      !laghu_runtime_cache_lookup_variant(cache_path, (const char *)catalog + 1706U, &entry) || entry.length != variant_length)
    return false;
...
static bool laghu_javascript_rum_ready(laghu_rum_engine *rum, const char *template_key, const char *page_origin, const char *url, unsigned int bucket,
                                       uint64_t now, char evidence_key[LAGHU_RUNTIME_KEY_SIZE]) {
  ...
  unsigned int index;
  int length;
  if (rum == NULL || template_key == NULL || page_origin == NULL || url == NULL || bucket > 1U ||
      !laghu_rum_engine_read(rum, LAGHU_RUM_RECORD_INSTRUMENTATION, template_key, now, &record, sizeof(record), &value) ||
      value.length != sizeof(record) || record.version != LAGHU_INSTRUMENTATION_VERSION || record.observations[bucket] < 100U)
    return false;
  length = snprintf(absolute, sizeof(absolute), "%s%s", page_origin, url);
  if (length <= 0 || (size_t)length >= sizeof(absolute) ||
      !laghu_sha256_hex((laghu_buffer){(const unsigned char *)absolute, (size_t)length}, script_key))
    return false;
  for (index = 0U; index < record.script_count; ++index)
    if (strcmp(record.script_keys[index], script_key) == 0) {
      ...
      int material_length;
      if (!laghu_javascript_defer_recommended(&record, bucket, index)) return false;
      if (evidence_key == NULL) return true;
      material_length = snprintf(material, sizeof(material), "rum-js-defer-evidence-v1\n%s\n%u\n%llu\n%llu\n%s\n%llu", template_key, bucket,
                                 (unsigned long long)record.updated_at, (unsigned long long)record.observations[bucket], script_key,
                                 (unsigned long long)record.script_observations[bucket][index]);
      return material_length > 0 && (size_t)material_length < sizeof(material) &&
             laghu_sha256_hex((laghu_buffer){(const unsigned char *)material, (size_t)material_length}, evidence_key);
    }
  return false;
}

static bool laghu_javascript_append(laghu_javascript_builder *builder, const void *data, size_t length) {
  size_t capacity;
  unsigned char *next;
  if (length == 0U) return true;
  if (builder == NULL || data == NULL || length > SIZE_MAX - builder->length) return false;
...
static bool laghu_javascript_copy_attribute(const unsigned char *value, size_t length, char *output, size_t output_size) {
...
static bool laghu_javascript_group_valid(const char *group) {
  ...
  if (length == 0U || length >= LAGHU_JAVASCRIPT_COMBINE_GROUP_SIZE) return false;
  for (index = 0U; index < length; ++index)
    if (!(isalnum((unsigned char)group[index]) || group[index] == '-' || group[index] == '_' || group[index] == '.')) return false;
  return true;
}

static bool laghu_javascript_parse_combine_script(laghu_buffer html, size_t offset, laghu_javascript_combine_script *script) {
  ...
  if ((size_t)(html.data + html.length - start) < 7U || laghu_find_ascii(start, start + 7U, "<script") != start) return false;
  ...
  if (!laghu_tag_attribute(start + 7U, open_end, "src", &value, &length) ||
      !laghu_javascript_copy_attribute(value, length, script->url, sizeof(script->url)) || script->url[0] != '/' || script->url[1] == '/')
    return false;
  if (!laghu_tag_attribute(start + 7U, open_end, "data-laghu-combine", &value, &length) ||
      !laghu_javascript_copy_attribute(value, length, script->group, sizeof(script->group)) || !laghu_javascript_group_valid(script->group))
    return false;
  if (laghu_tag_attribute(start + 7U, open_end, "type", &value, &length)) {
    if (!laghu_javascript_copy_attribute(value, length, script->type, sizeof(script->type)) ||
        (strcmp(script->type, "text/javascript") != 0 && strcmp(script->type, "application/javascript") != 0))
      return false;
  }
  if (laghu_tag_attribute(start + 7U, open_end, "nonce", &value, &length) &&
      !laghu_javascript_copy_attribute(value, length, script->nonce, sizeof(script->nonce)))
    return false;
  if (laghu_tag_attribute(start + 7U, open_end, "integrity", &value, &length) ||
      laghu_tag_attribute(start + 7U, open_end, "async", &value, &length) || laghu_tag_attribute(start + 7U, open_end, "defer", &value, &length) ||
      laghu_tag_attribute(start + 7U, open_end, "nomodule", &value, &length) ||
      laghu_tag_attribute(start + 7U, open_end, "onload", &value, &length) || laghu_tag_attribute(start + 7U, open_end, "onerror", &value, &length) ||
...
static bool laghu_javascript_combine_key(const laghu_javascript_combine_script *scripts, size_t count, const char *policy_key, const char *target,
  ...
  success = success && laghu_javascript_append(&material, scripts[0].group, strlen(scripts[0].group)) &&
            laghu_javascript_append(&material, "\n", 1U) && laghu_javascript_append(&material, scripts[0].type, strlen(scripts[0].type)) &&
            laghu_javascript_append(&material, "\n", 1U) && laghu_javascript_append(&material, scripts[0].nonce, strlen(scripts[0].nonce)) &&
            laghu_javascript_append(&material, "\n", 1U) && laghu_javascript_append(&material, policy_key, strlen(policy_key)) &&
            laghu_javascript_append(&material, "\n", 1U) && laghu_javascript_append(&material, target, strlen(target)) &&
            laghu_sha256_hex((laghu_buffer){material.data, material.length}, output);
  free(material.data);
  return success;
}

static bool laghu_javascript_combine(const char *cache_path, laghu_buffer html, const char *policy_key, const char *target, uint64_t now,
                                     unsigned int ttl_seconds, bool include_source_maps, laghu_javascript_combine_result *result) {
  laghu_javascript_builder output = {0};
  size_t cursor = 0U;
  bool changed = false;
  memset(result, 0, sizeof(*result));
  while (cursor < html.length) {
    const unsigned char *found = laghu_find_ascii(html.data + cursor, html.data + html.length, "<script");
    ...
    if (found == NULL) {
      ready = laghu_javascript_append(&output, html.data + cursor, html.length - cursor);
      if (!ready) goto failed;
      break;
    }
    scan = (size_t)(found - html.data);
    if (!laghu_javascript_parse_combine_script(html, scan, &scripts[0])) {
      if (!laghu_javascript_append(&output, html.data + cursor, scan + 1U - cursor)) goto failed;
      cursor = scan + 1U;
      continue;
    }
    count = 1U;
    ...
    while (count < LAGHU_JAVASCRIPT_COMBINE_MAX) {
      ...
      if (!laghu_javascript_parse_combine_script(html, scan, &scripts[count]) || strcmp(scripts[count].group, scripts[0].group) != 0 ||
          strcmp(scripts[count].type, scripts[0].type) != 0 || strcmp(scripts[count].nonce, scripts[0].nonce) != 0)
        break;
    ...
    if (count == LAGHU_JAVASCRIPT_COMBINE_MAX) {
      ...
      scan = group_end;
      while (scan < html.length && isspace(html.data[scan])) ++scan;
      while (laghu_javascript_parse_combine_script(html, scan, &extra) && strcmp(extra.group, scripts[0].group) == 0 &&
             strcmp(extra.type, scripts[0].type) == 0 && strcmp(extra.nonce, scripts[0].nonce) == 0) {
        overflow = true;
        group_end = extra.end;
        scan = group_end;
        while (scan < html.length && isspace(html.data[scan])) ++scan;
    ...
    if (count < 2U || overflow) {
      if (!laghu_javascript_append(&output, html.data + cursor, group_end - cursor)) goto failed;
      cursor = group_end;
      continue;
    }
    for (index = 0U; index < count; ++index) {
      if (!laghu_javascript_external_lookup(cache_path, scripts[index].url, policy_key, target, false, now, ttl_seconds, &scripts[index].record) ||
          (scripts[index].record.flags & LAGHU_JAVASCRIPT_FLAG_CONCAT_SAFE) == 0U ||
          (scripts[index].record.flags & LAGHU_JAVASCRIPT_FLAG_TOP_LEVEL_DECLARATION_FREE) == 0U ||
          !laghu_runtime_cache_lookup_variant(cache_path, scripts[index].record.variant, &entry)) {
        ready = false;
        break;
      }
      scripts[index].body = malloc(entry.length);
      scripts[index].body_length = entry.length;
      if (scripts[index].body == NULL || !laghu_runtime_cache_read(&entry, scripts[index].body, entry.length)) {
        ready = false;
        break;
      }
      if (include_source_maps) {
        static const char marker[] = "\n//# sourcemappingurl=/.laghu/js/";
        const unsigned char *map = laghu_find_ascii(scripts[index].body, scripts[index].body + scripts[index].body_length, marker);
        size_t remaining = map == NULL ? 0U : (size_t)(scripts[index].body + scripts[index].body_length - map);
        if (map == NULL || remaining != sizeof(marker) - 1U + LAGHU_SHA256_HEX_LENGTH + 4U || memcmp(map + remaining - 4U, ".map", 4U) != 0) {
          indexed_map = false;
        } else {
          memcpy(scripts[index].map_variant, map + sizeof(marker) - 1U, LAGHU_SHA256_HEX_LENGTH);
    ...
    if (ready) {
      ...
      size_t line = 0U;
      if (indexed_map && !laghu_javascript_append(&indexed, "{\"version\":3,\"sections\":[", sizeof("{\"version\":3,\"sections\":[") - 1U))
        ready = false;
      for (index = 0U; ready && index < count; ++index) {
        ...
        if (indexed_map) {
          section_length = snprintf(section, sizeof(section),
                                    "%s{\"offset\":{\"line\":%zu,\"column\":0},\"url\":"
                                    "\"/.laghu/js/%s.map\"}",
                                    index == 0U ? "" : ",", line, scripts[index].map_variant);
          if (section_length <= 0 || (size_t)section_length >= sizeof(section) ||
              !laghu_javascript_append(&indexed, section, (size_t)section_length)) {
            ready = false;
            break;
          }
        }
        if (!laghu_javascript_append(&combined, scripts[index].body, scripts[index].body_length) || combined.length > LAGHU_JAVASCRIPT_MAX_BYTES) {
          ready = false;
          break;
        }
    ...
    if (ready) {
      markup_length =
          snprintf(markup, sizeof(markup),
                   ...
                   key, scripts[0].group, scripts[0].type[0] == '\0' ? "" : " type=\"", scripts[0].type, scripts[0].type[0] == '\0' ? "" : "\"",
                   scripts[0].nonce[0] == '\0' ? "" : " nonce=\"", scripts[0].nonce, scripts[0].nonce[0] == '\0' ? "" : "\"");
    }
    if (ready && markup_length > 0 && (size_t)markup_length < sizeof(markup) &&
        laghu_javascript_append(&output, html.data + cursor, scripts[0].start - cursor) &&
        laghu_javascript_append(&output, markup, (size_t)markup_length)) {
      for (index = 0U; index < count; ++index) result->original_external_bytes += scripts[index].record.derived_length;
      ...
      if (result->dependency_key[0] == '\0') {
        ...
        int dependency_length = snprintf(dependency_material, sizeof(dependency_material), "%s\n%s", result->dependency_key, key);
        if (dependency_length <= 0 || (size_t)dependency_length >= sizeof(dependency_material) ||
            !laghu_sha256_hex((laghu_buffer){(const unsigned char *)dependency_material, (size_t)dependency_length}, result->dependency_key)) {
...
static bool laghu_javascript_defer_suffix_ready(const char *cache_path, const char *policy_key, const char *target, const char *page_path,
                                                const char *page_origin, const unsigned char *cursor, const unsigned char *end, uint64_t now,
                                                unsigned int ttl_seconds, laghu_rum_engine *rum, const char *template_key,
  ...
  while ((cursor = laghu_find_ascii(cursor, end, "<script")) != NULL) {
    ...
    if (close == NULL || !laghu_tag_attribute(cursor + 7U, open_end, "src", &source, &source_length) || source == NULL || source_length == 0U ||
        source_length >= sizeof(url) || source[0] != '/' || laghu_tag_attribute(cursor + 7U, open_end, "integrity", &attribute, &attribute_length) ||
        laghu_tag_attribute(cursor + 7U, open_end, "async", &attribute, &attribute_length) ||
        laghu_tag_attribute(cursor + 7U, open_end, "defer", &attribute, &attribute_length) ||
        laghu_tag_attribute(cursor + 7U, open_end, "nomodule", &attribute, &attribute_length) ||
        !laghu_tag_attributes_allowed(cursor + 7U, open_end, false))
      return false;
    if (laghu_tag_attribute(cursor + 7U, open_end, "type", &attribute, &attribute_length) &&
        !((attribute_length == 15U && laghu_find_ascii(attribute, attribute + attribute_length, "text/javascript") == attribute) ||
          (attribute_length == 22U && laghu_find_ascii(attribute, attribute + attribute_length, "application/javascript") == attribute)))
      return false;
    memcpy(url, source, source_length);
    url[source_length] = '\0';
    if (strchr(url, '?') != NULL || strchr(url, '#') != NULL ||
        !laghu_javascript_external_lookup(cache_path, url, policy_key, target, false, now, ttl_seconds, &record) ||
        (record.flags & LAGHU_JAVASCRIPT_FLAG_DEFER_SAFE) == 0U ||
        !laghu_javascript_rum_ready(rum, template_key, page_origin, url, viewport_bucket, now, NULL) ||
...
bool laghu_runtime_rewrite_javascript_html(laghu_runtime_queue *queue, const char *cache_path, laghu_buffer html, const char *page_path,
                                           ...
                                           unsigned int viewport_bucket, const laghu_javascript_defer_set *defer_set, bool allow_defer,
                                           bool allow_defer_suggestions, bool allow_combine, bool allow_inline, bool allow_outline,
                                           bool include_source_maps, unsigned int inline_limit, unsigned int outline_threshold,
  ...
  if (result == NULL) return false;
  memset(result, 0, sizeof(*result));
  if (queue == NULL || cache_path == NULL || html.data == NULL || page_path == NULL || policy_key == NULL) return true;
  if (!laghu_javascript_target_normalize(target, normalized_target)) return true;
  memset(&combined, 0, sizeof(combined));
  if (allow_combine && laghu_csp_allows_external_script(csp, NULL, 0U)) {
    if (!laghu_javascript_combine(cache_path, html, policy_key, normalized_target, now, ttl_seconds, include_source_maps, &combined)) return false;
    if (combined.rewritten) {
      ...
      memcpy(dependencies[dependency_count++], combined.dependency_key, LAGHU_RUNTIME_KEY_SIZE);
  ...
  while ((cursor = laghu_find_ascii(cursor, end, "<script")) != NULL) {
    ...
    }
    {
      ...
      bool external = laghu_tag_attribute(cursor + 7U, open_end, "src", &source_attribute, &source_length);
      if (laghu_tag_attribute(cursor + 7U, open_end, "integrity", &attribute, &attribute_length)) accepted = false;
      if (laghu_tag_attribute(cursor + 7U, open_end, "type", &attribute, &attribute_length)) {
        module = attribute != NULL && attribute_length == 6U && laghu_find_ascii(attribute, attribute + attribute_length, "module") == attribute;
        if (!module && !(attribute_length == 15U && laghu_find_ascii(attribute, attribute + attribute_length, "text/javascript") == attribute) &&
            !(attribute_length == 22U && laghu_find_ascii(attribute, attribute + attribute_length, "application/javascript") == attribute))
          accepted = false;
      }
      if (!external && !laghu_javascript_csp_allows_inline(csp, cursor + 7U, open_end)) accepted = false;
      if (laghu_find_ascii(open_end + 1U, close, "sourcemappingurl") != NULL || laghu_find_ascii(open_end + 1U, close, "sourceurl") != NULL)
        accepted = false;
      if (external) {
        char url[LAGHU_RUNTIME_PATH_SIZE];
        ...
        int route_length;
        if (accepted && allow_defer && !module && source_attribute != NULL && source_length > 0U && source_length < sizeof(url) &&
            source_attribute[0] != '/' && laghu_javascript_interaction_attributes(cursor + 7U, open_end) && close == open_end + 1U &&
            laghu_javascript_csp_allows_external(csp, cursor + 7U, open_end) && laghu_javascript_csp_allows_inline(csp, cursor + 7U, open_end)) {
          const unsigned char *nonce = NULL;
          ...
          url[source_length] = '\0';
          (void)laghu_tag_attribute(cursor + 7U, open_end, "nonce", &nonce, &nonce_length);
          if (laghu_javascript_interaction_url_safe(url) && laghu_javascript_interaction_approved(defer_set, url, page_path) &&
              laghu_javascript_interaction_markup(url, nonce, nonce_length, &markup, &markup_length)) {
        ...
        if (accepted && source_attribute != NULL && source_length > 0U && source_length < sizeof(url) && source_attribute[0] == '/') {
          memcpy(url, source_attribute, source_length);
          url[source_length] = '\0';
          if (laghu_javascript_external_lookup(cache_path, url, policy_key, normalized_target, module, now, ttl_seconds, &record)) {
            bool defer_ready = allow_defer && !module && strchr(url, '?') == NULL && strchr(url, '#') == NULL &&
                               (record.flags & LAGHU_JAVASCRIPT_FLAG_DEFER_SAFE) != 0U &&
                               laghu_javascript_rum_ready(rum, template_key, page_origin, url, viewport_bucket, now, defer_evidence_key) &&
                               laghu_tag_attributes_allowed(cursor + 7U, open_end, false) &&
                               !laghu_tag_attribute(cursor + 7U, open_end, "async", &attribute, &attribute_length) &&
                               !laghu_tag_attribute(cursor + 7U, open_end, "defer", &attribute, &attribute_length) &&
                               !laghu_tag_attribute(cursor + 7U, open_end, "nomodule", &attribute, &attribute_length) &&
                               laghu_javascript_defer_suffix_ready(cache_path, policy_key, normalized_target, page_path, page_origin, close + 8U, end,
                                                                   now, ttl_seconds, rum, template_key, viewport_bucket, defer_set);
            original_bundle += (size_t)record.derived_length;
            if (defer_ready && !laghu_javascript_defer_approved(defer_set, url, page_path) && allow_defer_suggestions &&
                !result->javascript_defer_recommended) {
              result->javascript_defer_recommended = true;
              (void)laghu_copy(result->javascript_defer_path, sizeof(result->javascript_defer_path), url);
              (void)laghu_copy(result->javascript_defer_template, sizeof(result->javascript_defer_template), template_key);
              result->javascript_defer_bucket = viewport_bucket;
              {
                laghu_rum_instrumentation_record evidence;
                laghu_rum_value evidence_value;
                if (laghu_rum_engine_read(rum, LAGHU_RUM_RECORD_INSTRUMENTATION, template_key, now, &evidence, sizeof(evidence), &evidence_value))
                  result->javascript_defer_observations = evidence.observations[viewport_bucket];
              }
            }
            if (defer_ready && laghu_javascript_defer_approved(defer_set, url, page_path)) {
              ...
              defer_growth += 6U;
              {
                ...
                int material_length = snprintf(material, sizeof(material), "%s\n%s", defer_set->digest, defer_evidence_key);
                if (material_length <= 0 || (size_t)material_length >= sizeof(material) ||
                    !laghu_sha256_hex((laghu_buffer){(const unsigned char *)material, (size_t)material_length}, dependencies[dependency_count++])) {
                  free(output);
                  return false;
                }
              }
            ...
            if (allow_inline && inline_limit != 0U && record.derived_length <= inline_limit &&
                laghu_tag_attributes_allowed(cursor + 7U, open_end, false) &&
                !laghu_tag_attribute(cursor + 7U, open_end, "async", &attribute, &attribute_length) &&
                !laghu_tag_attribute(cursor + 7U, open_end, "defer", &attribute, &attribute_length) &&
                !laghu_tag_attribute(cursor + 7U, open_end, "nomodule", &attribute, &attribute_length) &&
                (record.flags & LAGHU_JAVASCRIPT_FLAG_URL_INDEPENDENT) != 0U && laghu_javascript_csp_allows_inline(csp, cursor + 7U, open_end) &&
                laghu_runtime_cache_lookup_variant(cache_path, record.variant, &entry) && entry.length == record.derived_length) {
              ...
              if (body != NULL && laghu_runtime_cache_read(&entry, body, entry.length) &&
                  laghu_tag_attribute_span(cursor + 7U, open_end, source_attribute, source_length, &span_start, &span_end)) {
                ...
                unsigned char *next = malloc(prefix + middle + entry.length + suffix);
                if (next == NULL) {
                  free(body);
                  free(output);
                  return false;
                ...
                memcpy(dependencies[dependency_count++], record.variant, LAGHU_RUNTIME_KEY_SIZE);
                continue;
              }
              free(body);
            }
            if (laghu_runtime_cache_lookup_variant(cache_path, record.variant, &entry) && entry.length == record.derived_length &&
                (route_length = snprintf(route, sizeof(route), "/.laghu/js/%s", record.variant)) > 0 && (size_t)route_length < sizeof(route)) {
              size_t prefix = (size_t)(source_attribute - html.data);
              size_t suffix = html.length - prefix - source_length;
              unsigned char *next = malloc(prefix + (size_t)route_length + suffix);
              if (next == NULL) {
                free(output);
                return false;
              }
              ...
              memcpy(next + prefix + (size_t)route_length, source_attribute + source_length, suffix);
              free(output);
              output = next;
    ...
    location_length = snprintf(location, sizeof(location), "%s#script-%zu", page_path, script_count);
    if (accepted && location_length > 0 && (size_t)location_length < sizeof(location) &&
        laghu_runtime_rewrite_javascript(queue, cache_path, (laghu_buffer){open_end + 1U, (size_t)(close - open_end - 1U)}, location, policy_key,
                                         normalized_target, module, include_source_maps, &javascript)) {
      if (javascript.rewritten) {
        laghu_javascript_catalog_record inline_record;
        size_t source_length = (size_t)(close - open_end - 1U);
        if (allow_outline && source_length >= outline_threshold && laghu_tag_attributes_allowed(cursor + 7U, open_end, false) &&
            laghu_javascript_csp_allows_external(csp, cursor + 7U, open_end) &&
            laghu_javascript_external_lookup(cache_path, location, policy_key, normalized_target, module, now, ttl_seconds, &inline_record) &&
          ...
          if (source_attribute_length > 0 && (size_t)source_attribute_length < sizeof(source_attribute)) {
            unsigned char *next = malloc(prefix + (size_t)source_attribute_length + suffix);
            if (next == NULL) {
            ...
            memcpy(next, html.data, prefix);
            memcpy(next + prefix, source_attribute, (size_t)source_attribute_length);
            memcpy(next + prefix + (size_t)source_attribute_length, close, suffix);
            free(output);
            output = next;
            output_length = prefix + (size_t)source_attribute_length + suffix;
            rewritten_external += javascript.length;
            memcpy(dependencies[dependency_count++], inline_record.variant, LAGHU_RUNTIME_KEY_SIZE);
            html = (laghu_buffer){output, output_length};
        ...
        memcpy(dependencies[dependency_count++], javascript.dependency_key, LAGHU_RUNTIME_KEY_SIZE);
  ...
  if (output != NULL) {
    if (!interaction_rewritten && output_length - defer_growth + rewritten_external >= original_bundle) {
      free(output);
      return true;
    }
    result->data = output;
    ...
    if (dependency_count != 0U) {
      laghu_buffer material = {(const unsigned char *)dependencies, dependency_count * LAGHU_RUNTIME_KEY_SIZE};
