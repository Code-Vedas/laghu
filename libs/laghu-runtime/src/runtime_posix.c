...
bool laghu_runtime_queue_status(laghu_runtime_queue *queue, uint64_t *capacity,
  ...
  if (queue == NULL || queue->mapping == NULL || capacity == NULL ||
      occupied == NULL || !laghu_lock(queue->platform_file, F_RDLCK))
    return false;
  header = laghu_queue_header_at(queue);
  ...
  for (index = 0U; index < header->slot_count; ++index)
    if (laghu_queue_slot_at(queue, index)->state == LAGHU_SLOT_READY) ++count;
...
bool laghu_runtime_queue_try_publish(laghu_runtime_queue *queue,
  ...
  if (slot->state == LAGHU_SLOT_EMPTY &&
      ...
      laghu_string_copy(slot->policy_key, sizeof(slot->policy_key),
                        job->policy_key) &&
      laghu_string_copy(slot->provider_id, sizeof(slot->provider_id),
                        job->provider_id) &&
      laghu_string_copy(slot->provider_digest, sizeof(slot->provider_digest),
                        job->provider_digest) &&
      laghu_string_copy(slot->javascript_target,
                        sizeof(slot->javascript_target),
                        job->javascript_target)) {
...
bool laghu_runtime_shared_mapping_open(laghu_runtime_shared_mapping *mapping,
  ...
  if (status.st_size != 0 && (uintmax_t)status.st_size != (uintmax_t)size) {
    laghu_unlock(file);
    close(file);
    return false;
  }
  if (status.st_size == 0 && ftruncate(file, (off_t)size) != 0) {
    laghu_unlock(file);
    close(file);
    return false;
  }
...
bool laghu_runtime_shared_mapping_try_lock(
  ...
  return mapping != NULL && mapping->mapping != NULL &&
         laghu_lock((int)mapping->platform_file, F_WRLCK);
}

void laghu_runtime_shared_mapping_unlock(
    laghu_runtime_shared_mapping *mapping) {
  if (mapping != NULL && mapping->platform_file >= 0)
    laghu_unlock((int)mapping->platform_file);
}
