...
static bool laghu_cache_index_valid(laghu_cache_backend *backend,
...
static void laghu_cache_index_recover(laghu_cache_backend *backend,
                                      laghu_file_cache_state *state) {
...
static bool laghu_cache_metadata_path(char *output, size_t output_size,
                                      const char *cache_path) {
  int written = snprintf(output, output_size, "%s.laghu-metadata", cache_path);
  return written > 0 && (size_t)written < output_size;
}

static bool laghu_cache_delete_slot(laghu_cache_backend *backend,
                                    laghu_cache_index_slot *slot) {
  char path[LAGHU_RUNTIME_PATH_SIZE];
  bool success = true;
  if (slot == NULL || slot->state != LAGHU_CACHE_SLOT_READY) return false;
  if (!laghu_cache_file_path(path, sizeof(path), backend->path, "variant-",
                             slot->variant_key, ".bin") ||
      !laghu_runtime_file_remove(path))
    success = false;
  if (!laghu_cache_file_path(path, sizeof(path), backend->path, "index-",
                             slot->variant_key, ".meta") ||
      !laghu_runtime_file_remove(path))
    success = false;
  if (strcmp(slot->index_key, slot->variant_key) != 0 &&
      (!laghu_cache_file_path(path, sizeof(path), backend->path, "index-",
                              slot->index_key, ".meta") ||
       !laghu_runtime_file_remove(path)))
    success = false;
  return success;
...
static laghu_cache_index_slot *laghu_cache_slot(laghu_file_cache_state *state,
                                                const char *variant_key,
                                                bool empty_allowed) {
  uint64_t hash;
  unsigned int index;
  laghu_cache_index_slot *empty = NULL;
  if (state == NULL || state->header == NULL || variant_key == NULL ||
      state->header->slot_count == 0U)
    return NULL;
  hash = laghu_cache_hash_key(variant_key);
  for (index = 0U; index < state->header->slot_count; ++index) {
    laghu_cache_index_slot *slot =
        &state->slots[(hash + index) % state->header->slot_count];
...
static uint64_t laghu_cache_source_generation(laghu_file_cache_state *state,
  ...
  for (index = 0U; index < LAGHU_CACHE_TOMBSTONE_COUNT; ++index)
    if (strcmp(state->tombstones[index].source_hash, source_hash) == 0 &&
        state->tombstones[index].generation > generation)
...
static bool laghu_cache_metadata_open(laghu_cache_backend *backend) {
  ...
  int written;
  bool initialize;
  if (backend == NULL || !laghu_runtime_directory_exists(backend->path))
    return false;
  state = calloc(1U, sizeof(*state));
  if (state == NULL) return false;
  laghu_runtime_shared_mapping_init(&state->mapping);
  written =
      laghu_cache_metadata_path(path, sizeof(path), backend->path) ? 1 : 0;
  if (written == 0 ||
      !laghu_runtime_shared_mapping_open(&state->mapping, path,
                                         backend->limits.metadata_size) ||
  ...
  if (state->mapping.mapping_length <=
      sizeof(*state->header) +
          sizeof(laghu_cache_tombstone) * LAGHU_CACHE_TOMBSTONE_COUNT) {
  ...
  slots = (state->mapping.mapping_length - sizeof(*state->header) -
           sizeof(laghu_cache_tombstone) * LAGHU_CACHE_TOMBSTONE_COUNT) /
          sizeof(*state->slots);
  state->slots =
      (laghu_cache_index_slot *)(void *)(state->tombstones +
                                         LAGHU_CACHE_TOMBSTONE_COUNT);
...
static void laghu_cache_record_hit(laghu_cache_backend *backend,
...
static bool laghu_cache_file_lookup(laghu_cache_backend *backend,
  ...
  bool visible = slot == NULL ||
                 slot->generation >=
                     laghu_cache_source_generation(state, slot->source_hash);
  bool found = visible && laghu_runtime_file_cache_lookup(
                              backend->path, index_key, validator, entry);
  laghu_cache_record_hit(backend, entry, found);
  return found;
}

static bool laghu_cache_file_lookup_variant(laghu_cache_backend *backend,
                                            const char *variant_key,
                                            laghu_runtime_cache_entry *entry) {
  laghu_file_cache_state *state = backend->implementation;
  ...
  bool visible = slot == NULL ||
                 slot->generation >=
                     laghu_cache_source_generation(state, slot->source_hash);
  bool found = visible && laghu_runtime_file_cache_lookup_variant(
                              backend->path, variant_key, entry);
  laghu_cache_record_hit(backend, entry, found);
  return found;
}

static bool laghu_cache_file_read(laghu_cache_backend *backend,
                                  const laghu_runtime_cache_entry *entry,
...
static bool laghu_cache_file_publish(
    laghu_cache_backend *backend, const char *index_key,
    const char *variant_key, const char *validator, const char *content_type,
  ...
  {
    laghu_cache_index_slot *association =
        laghu_cache_slot(state, index_key, false);
    if (association != NULL)
      (void)snprintf(source_hash, sizeof(source_hash), "%s",
                     association->source_hash);
    if (source_hash[0] == '\0' &&
        strcmp(backend->path, laghu_cache_scoped_path) == 0)
      (void)snprintf(source_hash, sizeof(source_hash), "%s",
                     laghu_cache_scoped_source);
  }
  ...
  if (slot == NULL ||
      (!existing &&
       (state->header->bytes > state->header->size_limit ||
        payload.length > state->header->size_limit - state->header->bytes ||
  ...
  if (slot != NULL) {
    ...
    (void)snprintf(slot->variant_key, sizeof(slot->variant_key), "%s",
                   variant_key);
    (void)snprintf(slot->payload_hash, sizeof(slot->payload_hash), "%s",
                   entry->payload_hash);
...
static bool laghu_cache_file_touch(laghu_cache_backend *backend,
                                   const char *variant_key, uint64_t now) {
  laghu_file_cache_state *state = backend->implementation;
  laghu_cache_index_slot *slot;
  if (state == NULL || !laghu_runtime_shared_mapping_try_lock(&state->mapping))
    return false;
...
static bool laghu_cache_file_remove(laghu_cache_backend *backend,
                                    const char *variant_key) {
  laghu_file_cache_state *state = backend->implementation;
  laghu_cache_index_slot *slot;
  bool removed;
  ...
  if (removed) {
    state->header->bytes = state->header->bytes >= slot->length
                               ? state->header->bytes - slot->length
                               : 0U;
    state->header->files = state->header->files >= slot->file_count
                               ? state->header->files - slot->file_count
                               : 0U;
    memset(slot, 0, sizeof(*slot));
...
static bool laghu_cache_file_maintain(laghu_cache_backend *backend,
                                      uint64_t now) {
  laghu_file_cache_state *state = backend->implementation;
  ...
  if (state->header->cleaner_lease_until > now ||
      (state->header->last_cleanup != 0U &&
       now < state->header->last_cleanup + backend->limits.clean_interval)) {
    laghu_runtime_shared_mapping_unlock(&state->mapping);
    return true;
  }
  state->header->cleaner_lease_until = now + backend->limits.clean_interval;
  target_bytes =
      backend->limits.size_limit * LAGHU_CACHE_LOW_WATER_PERCENT / 100U;
  target_files =
      backend->limits.inode_limit * LAGHU_CACHE_LOW_WATER_PERCENT / 100U;
  ...
  for (index = 0U; index < count && (state->header->bytes > target_bytes ||
                                     state->header->files > target_files);
       ++index) {
...
static bool laghu_cache_file_health(laghu_cache_backend *backend,
                                    laghu_cache_stats *stats) {
  laghu_file_cache_state *state;
  if (backend == NULL || stats == NULL || backend->implementation == NULL)
    return false;
...
static const laghu_cache_backend_contract laghu_cache_file_contract = {
    laghu_cache_file_lookup,   laghu_cache_file_lookup_variant,
    laghu_cache_file_read,     laghu_cache_file_publish,
    laghu_cache_file_touch,    laghu_cache_file_remove,
    laghu_cache_file_maintain, laghu_cache_file_health,
    laghu_cache_file_close};
...
bool laghu_cache_size_parse(const char *value, uint64_t minimum,
  ...
  if (value == NULL || value[0] == '\0' || value[0] == '-' || result == NULL)
    return false;
  errno = 0;
...
bool laghu_cache_backend_uri_parse(const char *uri, char *path,
  ...
  const char *input;
  size_t used = 0U;
  if (uri == NULL || path == NULL || path_size == 0U ||
      strncmp(uri, "file:///", 8U) != 0 || uri[8] == '\0' ||
      strchr(uri + 8U, '?') != NULL || strchr(uri + 8U, '#') != NULL) {
    return false;
  }
  input = uri + 7U;
#ifdef _WIN32
  ...
  while (*input != '\0') {
    ...
    if (value == '%') {
      ...
      if (input[0] == '\0' || input[1] == '\0' ||
          (high = laghu_hex((unsigned char)input[0])) < 0 ||
          (low = laghu_hex((unsigned char)input[1])) < 0) {
        return false;
      }
...
bool laghu_cache_backend_open(laghu_cache_backend *backend, const char *uri,
  ...
  if (backend == NULL || uri == NULL || strlen(uri) >= sizeof(backend->uri))
    return false;
  memset(backend, 0, sizeof(*backend));
  if (!laghu_cache_backend_uri_parse(uri, backend->path, sizeof(backend->path)))
    return false;
  laghu_cache_limits_init(&defaults);
  backend->limits = limits != NULL ? *limits : defaults;
  if (backend->limits.size_limit == 0U || backend->limits.inode_limit == 0U ||
      backend->limits.metadata_size < 16384U ||
...
bool laghu_cache_backend_register_path(const char *path,
  ...
  if (limits != NULL) {
    ...
    laghu_cache_index_header header;
    FILE *file;
    if (laghu_cache_metadata_path(metadata_path, sizeof(metadata_path), path) &&
        laghu_runtime_file_size(metadata_path, &metadata_size) &&
        metadata_size >= 16384U && metadata_size <= SIZE_MAX &&
        (file = fopen(metadata_path, "rb")) != NULL) {
      bool read = fread(&header, sizeof(header), 1U, file) == 1U;
      (void)fclose(file);
      if (read && header.magic == LAGHU_CACHE_INDEX_MAGIC &&
          header.version == LAGHU_CACHE_INDEX_VERSION &&
          header.size_limit != 0U && header.inode_limit != 0U &&
          header.clean_interval != 0U) {
  ...
  for (index = 0U; index < laghu_cache_backend_count; ++index) {
    ...
    if (strcmp(backend->path, path) == 0)
      return backend->limits.size_limit == expected.size_limit &&
             backend->limits.inode_limit == expected.inode_limit &&
             backend->limits.metadata_size == expected.metadata_size &&
             backend->limits.clean_interval == expected.clean_interval;
  }
  if (laghu_cache_backend_count >= LAGHU_CACHE_BACKEND_REGISTRY_SIZE ||
      !laghu_cache_backend_open_path(
          &laghu_cache_backends[laghu_cache_backend_count], path, &expected))
    return false;
  ++laghu_cache_backend_count;
  return true;
}

static laghu_cache_backend *laghu_cache_backend_registered(
    const char *cache_path) {
  size_t index;
  for (index = 0U; cache_path != NULL && index < laghu_cache_backend_count;
       ++index)
    if (strcmp(laghu_cache_backends[index].path, cache_path) == 0)
      return &laghu_cache_backends[index];
  return NULL;
}
...
bool laghu_cache_source_normalize(const char *target, char *normalized,
  ...
  if (target == NULL || normalized == NULL || normalized_size < 2U ||
      target[0] != '/' || target[1] == '/' || strchr(target, '\\') != NULL ||
      strchr(target, '@') != NULL)
    return false;
  ...
  path_length =
      (size_t)((query != NULL
                    ? query
                    : (fragment != NULL ? fragment : target + strlen(target))) -
               target);
  ...
  if (query != NULL) {
    ...
    while (cursor <= end) {
      ...
      for (percent = cursor; percent < pair_end; ++percent)
        if (*percent == '%' &&
            (percent + 2U >= pair_end || !isxdigit((unsigned char)percent[1]) ||
             !isxdigit((unsigned char)percent[2])))
          return false;
      if (pair_length == sizeof("laghu=purge") - 1U &&
          memcmp(cursor, "laghu=purge", pair_length) == 0) {
...
bool laghu_cache_source_hash(const char *target,
  ...
  return laghu_cache_source_normalize(target, normalized, sizeof(normalized),
                                      NULL) &&
         laghu_sha256_hex((laghu_buffer){(const unsigned char *)normalized,
                                         strlen(normalized)},
                          output);
}
...
bool laghu_cache_backend_associate(laghu_cache_backend *backend,
  ...
  laghu_file_cache_state *state;
  laghu_cache_index_slot *slot;
  if (backend == NULL || backend->implementation == NULL ||
      !laghu_cache_hash_valid(index_key) ||
      !laghu_cache_hash_valid(source_hash))
    return false;
  state = backend->implementation;
  if (!laghu_runtime_shared_mapping_try_lock(&state->mapping)) return false;
  if (!laghu_cache_index_valid(backend, state))
...
bool laghu_cache_backend_associate_path(const char *path, const char *index_key,
  ...
  laghu_cache_backend *backend = laghu_cache_backend_registered(path);
  return backend != NULL &&
         laghu_cache_source_hash(source_target, source_hash) &&
         laghu_cache_backend_associate(backend, index_key, source_hash);
}
...
void laghu_cache_source_scope(const char *path, const char *source_target) {
  ...
  if (path == NULL || strlen(path) >= sizeof(laghu_cache_scoped_path) ||
      !laghu_cache_source_hash(source_target, laghu_cache_scoped_source))
    return;
...
laghu_cache_purge_result laghu_cache_backend_purge_url(
  ...
  if (backend == NULL || backend->implementation == NULL ||
      !laghu_cache_source_hash(source_target, source_hash))
    return LAGHU_CACHE_PURGE_INVALID;
  state = backend->implementation;
  if (!laghu_runtime_shared_mapping_try_lock(&state->mapping))
    return LAGHU_CACHE_PURGE_UNAVAILABLE;
  if (!laghu_cache_index_valid(backend, state))
    laghu_cache_index_recover(backend, state);
  for (index = 0U; index < LAGHU_CACHE_TOMBSTONE_COUNT; ++index) {
    if (strcmp(state->tombstones[index].source_hash, source_hash) == 0) {
    ...
    if (available == NULL && state->tombstones[index].source_hash[0] == '\0')
      available = &state->tombstones[index];
  }
  if (available == NULL) {
    laghu_runtime_shared_mapping_unlock(&state->mapping);
  ...
  (void)snprintf(available->source_hash, sizeof(available->source_hash), "%s",
                 source_hash);
...
laghu_cache_purge_result laghu_cache_backend_purge_url_path(
    ...
    uint64_t *matched_artifacts) {
  laghu_cache_backend *backend = laghu_cache_backend_registered(path);
  return backend == NULL ? LAGHU_CACHE_PURGE_UNAVAILABLE
                         : laghu_cache_backend_purge_url(
                               backend, source_target, now, matched_artifacts);
}
...
bool laghu_cache_backend_flush(laghu_cache_backend *backend,
  ...
  if (requested_generation != 0U &&
      requested_generation <= state->header->cache_generation) {
    laghu_runtime_shared_mapping_unlock(&state->mapping);
    return false;
  }
  state->header->cache_generation = requested_generation != 0U
...
bool laghu_cache_backend_flush_path(const char *path,
                                    ...
                                    uint64_t *generation) {
  laghu_cache_backend *backend = laghu_cache_backend_registered(path);
  return backend != NULL && laghu_cache_backend_flush(
                                backend, requested_generation, now, generation);
}
...
bool laghu_cache_flush_file_poll(const char *path, const char *flush_file,
  ...
  if (line_length > 0U && line[line_length - 1U] == '\r')
    line[--line_length] = '\0';
  if (line_length == 0U ||
      sscanf(line, "laghu-cache-flush-v1 %llu%c", &requested, &trailing) != 1 ||
      requested == 0U)
    return false;
  return laghu_cache_backend_flush_path(path, (uint64_t)requested, now,
                                        generation);
}
...
bool laghu_runtime_cache_publish(const char *cache_path, const char *index_key,
  ...
  return backend != NULL ? laghu_cache_backend_publish(
                               backend, index_key, variant_key, validator,
                               content_type, backend_id, payload, entry)
                         : laghu_runtime_file_cache_publish(
                               cache_path, index_key, variant_key, validator,
                               content_type, backend_id, payload, entry);
}
...
bool laghu_runtime_cache_lookup(const char *cache_path, const char *index_key,
  ...
  laghu_cache_backend *backend = laghu_cache_backend_registered(cache_path);
  return backend != NULL
             ? laghu_cache_backend_lookup(backend, index_key, validator, entry)
             : laghu_runtime_file_cache_lookup(cache_path, index_key, validator,
                                               entry);
}
...
bool laghu_runtime_cache_lookup_variant(const char *cache_path,
  ...
  laghu_cache_backend *backend = laghu_cache_backend_registered(cache_path);
  return backend != NULL
             ? laghu_cache_backend_lookup_variant(backend, variant_key, entry)
             : laghu_runtime_file_cache_lookup_variant(cache_path, variant_key,
                                                       entry);
}
...
bool laghu_runtime_cache_read(const laghu_runtime_cache_entry *entry,
  ...
  if (entry != NULL) {
    for (index = 0U; index < laghu_cache_backend_count; ++index) {
      ...
      if (strncmp(entry->variant_path, laghu_cache_backends[index].path,
                  length) == 0 &&
          (entry->variant_path[length] == '/' ||
           entry->variant_path[length] == '\\'))
        return laghu_cache_backend_read(&laghu_cache_backends[index], entry,
...
void laghu_cache_backend_close(laghu_cache_backend *backend) {
  ...
  if (backend->contract != NULL && backend->contract->close != NULL)
    backend->contract->close(backend);
  memset(backend, 0, sizeof(*backend));
}
...
bool laghu_cache_backend_lookup(laghu_cache_backend *backend,
                                ...
                                laghu_runtime_cache_entry *entry) {
  return backend != NULL && backend->contract != NULL &&
         backend->contract->lookup != NULL &&
...
bool laghu_cache_backend_lookup_variant(laghu_cache_backend *backend,
                                        ...
                                        laghu_runtime_cache_entry *entry) {
  return backend != NULL && backend->contract != NULL &&
         backend->contract->lookup_variant != NULL &&
...
bool laghu_cache_backend_publish(laghu_cache_backend *backend,
                                 ...
                                 laghu_runtime_cache_entry *entry) {
  return backend != NULL && backend->contract != NULL &&
         backend->contract->publish != NULL &&
         backend->contract->publish(backend, index_key, variant_key, validator,
                                    content_type, backend_id, payload, entry);
}

bool laghu_cache_backend_maintain(laghu_cache_backend *backend, uint64_t now) {
  return backend != NULL && backend->contract != NULL &&
         backend->contract->maintain != NULL &&
         backend->contract->maintain(backend, now);
}

bool laghu_cache_backend_health(laghu_cache_backend *backend,
                                laghu_cache_stats *stats) {
  return backend != NULL && backend->contract != NULL &&
         backend->contract->health != NULL &&
         backend->contract->health(backend, stats);
}
