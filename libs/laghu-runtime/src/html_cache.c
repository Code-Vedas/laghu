...
#if defined(_WIN32)
  {
    struct _utimbuf times = {.actime = (__time64_t)now,
                             .modtime = (__time64_t)now};
...
bool laghu_html_cache_key(const char *origin, const char *request_path,
  ...
  int written;
  if (origin == NULL || request_path == NULL || output == NULL ||
      origin[0] == '\0' || request_path[0] != '/')
    return false;
  written =
      snprintf(canonical, sizeof(canonical), "%s\n%s", origin, request_path);
  return written > 0 && (size_t)written < sizeof(canonical) &&
         laghu_sha256_hex(
             (laghu_buffer){(const unsigned char *)canonical, (size_t)written},
             output);
}
...
bool laghu_html_cache_lookup(const char *cache_path, const char *origin,
  ...
  memset(record, 0, sizeof(*record));
  if (ttl == 0U || !laghu_html_cache_key(origin, request_path, key) ||
  ...
  record->state =
      now < fresh_until ? LAGHU_HTML_CACHE_FRESH : LAGHU_HTML_CACHE_STALE;
  return true;
}
...
bool laghu_html_cache_renew(const char *cache_path, const char *origin,
  ...
  laghu_runtime_cache_entry entry;
  struct stat information;
  if (record == NULL || validator == NULL || validator[0] == '\0' ||
      !laghu_html_cache_key(origin, request_path, key)) {
    return false;
  }
  if (!laghu_runtime_cache_lookup_variant(cache_path, key, &entry) ||
      strcmp(entry.validator, validator) != 0) {
    return false;
  }
  if (!laghu_html_cache_touch(entry.variant_path, now) ||
      stat(entry.variant_path, &information) != 0 || information.st_mtime < 0) {
    return false;
  }
  memset(record, 0, sizeof(*record));
