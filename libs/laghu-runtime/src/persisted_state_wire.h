...
#define LAGHU_PERSISTED_U64(name, value) static const uint64_t LAGHU_WIRE_##name = (value);
#include "../persisted_state_contract.def"
#undef LAGHU_PERSISTED_U32
#undef LAGHU_PERSISTED_U64
...
static inline uint64_t laghu_wire_u64_read(const unsigned char *input) {
  uint64_t value = 0U;
  unsigned int index;
  for (index = 0U; index < 8U; ++index) value |= (uint64_t)input[index] << (index * 8U);
  return value;
}
...
static inline void laghu_wire_u64_write(unsigned char *output, uint64_t value) {
  unsigned int index;
  for (index = 0U; index < 8U; ++index) output[index] = (unsigned char)(value >> (index * 8U));
}

static inline bool laghu_wire_string_valid(const unsigned char *input, size_t capacity) {
  return input != NULL && capacity != 0U && memchr(input, '\0', capacity) != NULL;
}

static inline bool laghu_wire_zeroes(const unsigned char *input, size_t length) {
  size_t index;
