...
#define LAGHU_RUM_REDIS_MERGE_VERSION 2U
...
static const char laghu_rum_redis_merge_script_one[] =
    "local CONTRACT=2\n"
    "local IMAGE_LEN=98\n"
    "local CRITICAL_LEN=1499\n"
    "local OLD_INSTRUMENTATION_LEN=14479\n"
    "local INSTRUMENTATION_LEN=15591\n"
    ...
    "local function eqat(a,b,p,q,n) for i=0,n-1 do if a[p+i]~=b[q+i] then "
    ...
    " if kind==1 and current and #current==OLD_INSTRUMENTATION_LEN then current="
    "current..string.rep(string.char(0),INSTRUMENTATION_LEN-OLD_INSTRUMENTATION_LEN) end\n"
    ...
    " local d=bytes(delta)\n"
    " if (kind==1 and u32(d,1)~=2) or (kind==2 and u32(d,1)~=2) or "
    "(kind==3 and d[98]>1) then return nil,'record version' end\n"
    " if not current then return delta end\n"
    " local a=bytes(current)\n"
    " if (kind==1 and u32(a,1)~=2) or (kind==2 and u32(a,1)~=2) or "
    "(kind==3 and a[98]>1) then return nil,'stored version' end\n"
    " if kind==3 then\n"
    "  if not eq(a,d,1,65) then return nil,'image identity' end\n"
    "  maxle(a,d,66,8); for p=74,94,4 do maxle(a,d,p,4) end; if d[98]~=0 then "
    "a[98]=1 end\n"
    " elseif kind==2 then\n"
    "  if not eq(a,d,5,65) or not eq(a,d,1094,65) or not eq(a,d,1159,65) then "
    "return nil,'critical identity' end\n"
    "  maxle(a,d,1224,8); satadd(a,d,1232,4); for p=1236,1242,2 do satadd(a,d,p,2) end; "
    "for p=1244,1499 do a[p]=bit.bor(a[p],d[p]) end\n"
    " else\n"
    "  if not eq(a,d,5,195) or not eq(a,d,208,4164) or not eq(a,d,7996,5188) then "
    "return nil,'instrumentation identity' end\n"
    ...
    "  for bucket=0,3 do local base=13184+bucket*324; satadd(a,d,base,2); "
    "satadd(a,d,base+2,2); for p=base+4,base+66,2 do satadd(a,d,p,2) end; "
    "for p=base+68,base+322,2 do satadd(a,d,p,2) end end\n"
    "  for slot=0,7 do local from=14480+slot*139; if d[from]~=0 then local found="
    "false; local empty=0; if d[from+138]>1 then return nil,'receipt flag' end; "
    "for target=0,7 do local at=14480+target*139; if a[at]~=0 and eqat(a,d,at,from,65) then "
    "if not eqat(a,d,at+65,from+65,65) or not eqat(a,d,at+130,from+130,8) or a[at+138]>1 "
    "then return nil,'receipt binding' end; if d[from+138]~=0 then a[at+138]=1 end; found=true; break "
    "elseif a[at]==0 and empty==0 then empty=at end end; if not found and empty~=0 then "
    "for p=0,138 do a[empty+p]=d[from+p] end end end end\n"
    " end\n"
    " return encode(a)\n"
    "end\n";
