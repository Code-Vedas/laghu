...
static bool laghu_hint_append(laghu_hint_builder *builder, const void *data,
                              size_t length) {
  unsigned char *grown;
  size_t needed;
...
static bool laghu_hint_equal(const unsigned char *value, size_t length,
                             const char *expected) {
  size_t index;
...
static bool laghu_hint_attribute(const unsigned char *tag, size_t length,
                                 const char *name, const unsigned char **value,
                                 size_t *value_length, unsigned int *matches) {
  size_t cursor = 1U;
  *value = NULL;
  *value_length = 0U;
  *matches = 0U;
  ...
  while (cursor < length) {
    ...
    while (cursor < length && (isalnum(tag[cursor]) || tag[cursor] == '-' ||
                               tag[cursor] == '_' || tag[cursor] == ':')) {
      ++cursor;
    }
    ...
    if (cursor >= length || tag[cursor] != '=') {
      if (laghu_hint_equal(tag + attribute_start,
    ...
    if (laghu_hint_equal(tag + attribute_start, attribute_end - attribute_start,
                         name)) {
      ++*matches;
      *value = tag + start;
...
static bool laghu_hint_language(const unsigned char *value, size_t length,
                                char output[LAGHU_HTML_LANGUAGE_SIZE]) {
  size_t index;
...
static bool laghu_hint_normalize_url(const char *page_path,
                                     const char *page_origin,
                                     const unsigned char *url, size_t length,
                                     char output[LAGHU_RUNTIME_PATH_SIZE],
                                     bool *same_origin) {
  size_t origin_length = page_origin == NULL ? 0U : strlen(page_origin);
  size_t prefix = 0U;
  *same_origin = false;
  if (length == 0U || length >= LAGHU_RUNTIME_PATH_SIZE || url[0] == '#' ||
...
static bool laghu_hint_origin(const unsigned char *url, size_t length,
                              const char *page_origin,
                              char output[LAGHU_RUNTIME_PATH_SIZE]) {
  ...
  if (host_end == host_start || url[host_start] == '.' ||
      url[host_end - 1U] == '.' || url[host_start] == '-' ||
      url[host_end - 1U] == '-') {
    return false;
  }
  for (cursor = host_start; cursor < host_end; ++cursor) {
    ...
    if (!(isalnum(byte) || byte == '.' || byte == '-') ||
        (byte == '.' && cursor + 1U < host_end && url[cursor + 1U] == '.') ||
        (byte == '-' && cursor > host_start && url[cursor - 1U] == '.')) {
      return false;
    }
  }
  if ((scheme_length == 7U && port == 80U) ||
      (scheme_length == 8U && port == 443U)) {
    end = host_end;
  }
  ...
  if (page_origin != NULL) {
    ...
    if (laghu_hint_origin((const unsigned char *)page_origin,
                          strlen(page_origin), NULL, normalized_page) &&
...
static void laghu_hint_existing_markup(laghu_buffer html, char *output,
                                       size_t capacity) {
  size_t cursor = 0U;
  size_t used = 0U;
  output[0] = '\0';
  while (cursor < html.length) {
    ...
    if (start + 5U < end &&
        laghu_hint_equal(html.data + start + 1U, 4U, "link") &&
        laghu_hint_attribute(html.data + start, end - start + 1U, "rel", &rel,
                             &rel_length, &rel_matches) &&
        laghu_hint_attribute(html.data + start, end - start + 1U, "href", &href,
                             &href_length, &href_matches) &&
        rel_matches == 1U && href_matches == 1U &&
        (laghu_hint_equal(rel, rel_length, "preload") ||
         laghu_hint_equal(rel, rel_length, "dns-prefetch") ||
...
static bool laghu_hint_ready_image(const char *cache_path, const char *url,
                                   const char *policy_key,
                                   uint32_t capability_mask, uint64_t now,
                                   unsigned int ttl_seconds, char *target,
                                   const char **type) {
  laghu_catalog_record record;
  unsigned int index;
  if (!laghu_catalog_lookup_url(cache_path, url, policy_key, capability_mask,
                                now, ttl_seconds, &record)) {
    return false;
  }
  for (index = 0U; index < record.variant_count; ++index) {
    laghu_catalog_variant *variant = &record.variants[index];
    laghu_runtime_cache_entry entry;
    if (variant->ready && !variant->terminally_excluded &&
...
bool laghu_runtime_finalize_html_headers(
  ...
  if (all_existing_links[0] == '\0' && existing_link_headers != NULL &&
      existing_link_headers[0] != '\0') {
    return false;
  }
  ...
  while (cursor < html.length) {
    ...
    bool closing;
    if (open == NULL) {
      if (!laghu_hint_append(&builder, html.data + cursor,
                             html.length - cursor)) {
        invalid = true;
      }
      break;
    ...
    if (start + 4U <= html.length &&
      ...
      if (close == NULL ||
          !laghu_hint_append(&builder, html.data + start,
                             (size_t)(close - html.data) + 3U - start)) {
        invalid = true;
        break;
      }
    ...
    name_length = 0U;
    while (name + name_length < html.data + end &&
    ...
    if (!closing && head_depth > 0U &&
        (plan & LAGHU_HTML_PLAN_CONVERT_META_TAGS) != 0U &&
        laghu_hint_equal(name, name_length, "meta")) {
      ...
      if (!laghu_hint_attribute(html.data + start, end - start + 1U,
                                "http-equiv", &http_equiv, &http_length,
                                &http_matches) ||
          !laghu_hint_attribute(html.data + start, end - start + 1U, "content",
                                &content, &content_length, &content_matches)) {
        invalid = true;
        break;
      }
      ...
      if (http_matches == 1U && content_matches == 1U &&
          laghu_hint_equal(http_equiv, http_length, "content-language")) {
        char current[LAGHU_HTML_LANGUAGE_SIZE];
        if (!laghu_hint_language(content, content_length, current) ||
            (language[0] != '\0' &&
             !laghu_hint_case_equal(language, current)) ||
            (existing_content_language != NULL &&
             existing_content_language[0] != '\0' &&
    ...
    if (!closing && (plan & LAGHU_HTML_PLAN_RESOURCE_HINTS) != 0U) {
      ...
      bool resource_tag = laghu_hint_equal(name, name_length, "script") ||
                          laghu_hint_equal(name, name_length, "link") ||
                          laghu_hint_equal(name, name_length, "img") ||
                          laghu_hint_equal(name, name_length, "source") ||
                          laghu_hint_equal(name, name_length, "video") ||
                          laghu_hint_equal(name, name_length, "audio") ||
                          laghu_hint_equal(name, name_length, "iframe");
      if (laghu_hint_equal(name, name_length, "img")) {
        ++image_index;
      }
      for (attribute_index = 0U; resource_tag && attribute_index < 3U;
           ++attribute_index) {
        ...
        if (!laghu_hint_attribute(html.data + start, end - start + 1U,
                                  attributes[attribute_index], &url,
                                  &url_length, &matches)) {
          invalid = true;
          break;
        }
      ...
      if (resource_tag && dns_count < LAGHU_HTML_MAX_DNS_PREFETCH &&
        ...
        if (!laghu_hint_attribute(html.data + start, end - start + 1U, "srcset",
                                  &srcset, &srcset_length, &srcset_matches)) {
          invalid = true;
          break;
        }
        if (srcset_matches == 1U) {
          size_t item = 0U;
          while (item < srcset_length &&
                 dns_count < LAGHU_HTML_MAX_DNS_PREFETCH) {
            size_t url_start;
            size_t url_end;
            char origin[LAGHU_RUNTIME_PATH_SIZE];
            while (item < srcset_length &&
                   (isspace(srcset[item]) || srcset[item] == ',')) {
              ++item;
            }
            url_start = item;
            while (item < srcset_length && !isspace(srcset[item]) &&
                   srcset[item] != ',') {
              ++item;
            }
            url_end = item;
      ...
      if (preload_count < LAGHU_HTML_MAX_PRELOADS &&
          laghu_hint_equal(name, name_length, "link")) {
        const unsigned char *rel;
        const unsigned char *href;
        size_t rel_length;
        ...
        unsigned int rel_matches;
        unsigned int href_matches;
        if (!laghu_hint_attribute(html.data + start, end - start + 1U, "rel",
                                  &rel, &rel_length, &rel_matches) ||
            !laghu_hint_attribute(html.data + start, end - start + 1U, "href",
                                  &href, &href_length, &href_matches)) {
          invalid = true;
          break;
        }
        if (rel_matches == 1U && href_matches == 1U &&
            laghu_hint_equal(rel, rel_length, "stylesheet")) {
          char normalized[LAGHU_RUNTIME_PATH_SIZE];
          bool same_origin;
          laghu_stylesheet_record record;
          if (laghu_hint_normalize_url(page_path, page_origin, href,
                                       href_length, normalized, &same_origin) &&
              same_origin &&
              laghu_stylesheet_lookup(cache_path, normalized, policy_key,
                                      capability_mask, css_inline_limit,
                                      css_outline_threshold, now, ttl_seconds,
                                      &record) &&
            ...
            if (laghu_runtime_cache_lookup_variant(
                    cache_path, record.derived_key, &stylesheet_entry) &&
                strcmp(stylesheet_entry.content_type, "text/css") == 0) {
              (void)snprintf(target, sizeof(target), "/.laghu/css/%s",
                             record.derived_key);
              if (!laghu_hint_add(result, existing_link_headers, target,
      ...
      } else if (preload_count < LAGHU_HTML_MAX_PRELOADS &&
                 laghu_hint_equal(name, name_length, "img")) {
        ...
        if (!laghu_hint_attribute(html.data + start, end - start + 1U, "src",
                                  &src, &src_length, &src_matches) ||
            !laghu_hint_attribute(html.data + start, end - start + 1U,
                                  "fetchpriority", &priority, &priority_length,
                                  &priority_matches)) {
          invalid = true;
          break;
        }
        critical =
            critical || (priority_matches == 1U &&
                         laghu_hint_equal(priority, priority_length, "high"));
        if (src_matches == 1U &&
            laghu_hint_normalize_url(page_path, page_origin, src, src_length,
                                     normalized, &same_origin) &&
            same_origin) {
          laghu_catalog_record record;
          if (!critical && laghu_catalog_lookup_url(
                               cache_path, normalized, policy_key,
                               capability_mask, now, ttl_seconds, &record)) {
            critical = record.learned_above_fold;
          }
          if (critical) {
            char target[sizeof("/.laghu/image/") + LAGHU_RUNTIME_KEY_SIZE];
            const char *type = NULL;
            if (laghu_hint_ready_image(cache_path, normalized, policy_key,
                                       capability_mask, now, ttl_seconds,
                                       target, &type)) {
              char relation[160U];
              if (laghu_hint_mime(type)) {
                (void)snprintf(relation, sizeof(relation),
                               "rel=preload; as=image; type=\"%s\"", type);
              } else {
                memcpy(relation, "rel=preload; as=image",
                       sizeof("rel=preload; as=image"));
              }
              if (!laghu_hint_add(result, existing_link_headers, target,
                                  relation)) {
                invalid = true;
  ...
  if (language[0] != '\0' && (existing_content_language == NULL ||
                              existing_content_language[0] == '\0')) {
  ...
  }
  {
    ...
    written = snprintf(
        ...
        "laghu-html-hints-v1\n%s\n%s\n%08x\n%s\n%s\n", source_hash, policy_key,
        (unsigned int)plan,
        existing_content_language == NULL ? "" : existing_content_language,
        existing_link_headers == NULL ? "" : existing_link_headers);
    ...
    if (!already_warm && !laghu_runtime_cache_lookup_variant(
                             cache_path, result->dependency_key, &entry)) {
