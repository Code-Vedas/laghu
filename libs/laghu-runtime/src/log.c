...
static bool laghu_log_number(laghu_log_builder *builder, uint64_t value) {
  ...
  return length > 0 && (size_t)length < sizeof(text) && laghu_log_append(builder, text);
}

static bool laghu_log_string(laghu_log_builder *builder, const char *value, size_t limit, bool *truncated) {
...
static bool laghu_log_common_render(laghu_log_builder *builder, const laghu_log_common *common, const char *event) {
  char timestamp[32U];
  if (common == NULL || !laghu_log_timestamp(common->timestamp, timestamp) ||
      !laghu_log_append(builder, "{\"schema\":\"" LAGHU_LOG_SCHEMA "\",\"timestamp\":\"") || !laghu_log_append(builder, timestamp) ||
      !laghu_log_append(builder, "\",\"surface\":"))
    return false;
  if (!laghu_log_string(builder, common->surface, 32U, NULL) || !laghu_log_append(builder, ",\"component\":") ||
      !laghu_log_string(builder, common->component, 32U, NULL) || !laghu_log_append(builder, ",\"event\":") ||
      !laghu_log_string(builder, event, 16U, NULL))
    return false;
  if (common->trace_id != NULL && common->trace_id[0] != '\0') {
    if (!laghu_log_append(builder, ",\"trace_id\":") || !laghu_log_string(builder, common->trace_id, 32U, NULL)) return false;
  }
  if (common->span_id != NULL && common->span_id[0] != '\0') {
    if (!laghu_log_append(builder, ",\"span_id\":") || !laghu_log_string(builder, common->span_id, 16U, NULL)) return false;
  }
  return true;
}
...
bool laghu_log_render_transaction(const laghu_log_transaction *record, char *output, size_t capacity) {
  ...
  path_length = query == NULL ? (record->path == NULL ? 0U : strlen(record->path)) : (size_t)(query - record->path);
  if (path_length == 0U) {
    strcpy(path, "/");
  } else {
  ...
  return laghu_log_common_render(&builder, &record->common, "transaction") && laghu_log_append(&builder, ",\"method\":") &&
         laghu_log_string(&builder, record->method, 16U, NULL) && laghu_log_append(&builder, ",\"path\":") &&
         laghu_log_string(&builder, path, LAGHU_LOG_PATH_SIZE, &path_truncated) && laghu_log_append(&builder, ",\"path_truncated\":") &&
         laghu_log_append(&builder, path_truncated ? "true" : "false") && laghu_log_append(&builder, ",\"status\":") &&
         laghu_log_number(&builder, record->status) && laghu_log_append(&builder, ",\"decision\":") &&
         laghu_log_string(&builder, record->decision, 64U, NULL) && laghu_log_append(&builder, ",\"request_bytes\":") &&
         laghu_log_number(&builder, record->request_bytes) && laghu_log_append(&builder, ",\"original_bytes\":") &&
         laghu_log_number(&builder, record->original_bytes) && laghu_log_append(&builder, ",\"output_bytes\":") &&
...
bool laghu_log_render_lifecycle(const laghu_log_lifecycle *record, char *output, size_t capacity) {
  ...
  if (record == NULL || output == NULL || capacity == 0U) return false;
  output[0] = '\0';
  return laghu_log_common_render(&builder, &record->common, "lifecycle") && laghu_log_append(&builder, ",\"state\":") &&
         laghu_log_string(&builder, record->state, 16U, NULL) && laghu_log_append(&builder, ",\"failure\":") &&
         laghu_log_string(&builder, record->failure, 64U, NULL) && laghu_log_append(&builder, "}");
}
...
bool laghu_log_render_job(const laghu_log_job *record, char *output, size_t capacity) {
  ...
  if (record == NULL || output == NULL || capacity == 0U) return false;
  output[0] = '\0';
  return laghu_log_common_render(&builder, &record->common, "job") && laghu_log_append(&builder, ",\"job_kind\":") &&
         laghu_log_string(&builder, record->job_kind, 32U, NULL) && laghu_log_append(&builder, ",\"outcome\":") &&
