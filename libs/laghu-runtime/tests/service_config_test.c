...
static void test_descriptors(void) {
  ...
  for (setting = LAGHU_SERVICE_SETTING_FILE_CACHE_BACKEND;
       setting < LAGHU_SERVICE_SETTING_COUNT; ++setting) {
  ...
  assert(laghu_service_setting_find("FileCacheSize") ==
         LAGHU_SERVICE_SETTING_FILE_CACHE_SIZE);
  assert(laghu_service_setting_find("--rum-store") ==
         LAGHU_SERVICE_SETTING_RUM_STORE);
  assert(laghu_service_setting_find("not-a-setting") ==
         LAGHU_SERVICE_SETTING_UNKNOWN);
}

static void test_config_size(void) {
  assert(sizeof(laghu_service_config) < 65536U);
}
...
static void test_descriptor_matrix(void) {
  static const struct {
    ...
    const char *value;
      ...
      {LAGHU_SERVICE_SETTING_FILE_CACHE_BACKEND, "file:///tmp/cache"},
  ...
  for (index = 0U; index < sizeof(cases) / sizeof(cases[0]); ++index) {
    ...
    assert(laghu_service_config_apply(&config, cases[index].setting,
                                      cases[index].value, &diagnostic));
  ...
  }
  {
    ...
    assert(laghu_service_config_apply_pair(
        &config, LAGHU_SERVICE_SETTING_FILE_SOURCE_MAP,
        "https://assets.example/", TEST_SOURCE_ROOT, &diagnostic));
    assert(!laghu_service_config_apply_pair(
        &config, LAGHU_SERVICE_SETTING_FILE_SOURCE_MAP,
        "https://assets.example/", TEST_SOURCE_ROOT, &diagnostic));
  }
...
static void test_apply_and_merge(void) {
  ...
  assert(laghu_service_config_apply(
      &parent, LAGHU_SERVICE_SETTING_FILE_CACHE_SIZE, "2m", &diagnostic));
  ...
  assert(!laghu_service_config_apply(
      &child, LAGHU_SERVICE_SETTING_FILE_CACHE_SIZE, "8m", &diagnostic));
  assert(diagnostic.code == LAGHU_SERVICE_DIAGNOSTIC_DUPLICATE);
  assert(!laghu_service_config_apply(
      &child, LAGHU_SERVICE_SETTING_RUM_STORE_TIMEOUT, "9", &diagnostic));
  ...
  assert(laghu_service_config_apply(&parent,
                                    ...
                                    TEST_PARENT_CACHE_URI, &diagnostic));
  assert(laghu_service_config_apply(&child, LAGHU_SERVICE_SETTING_IMAGE_CACHE,
                                    TEST_CHILD_CACHE_ROOT, &diagnostic));
  assert(laghu_service_config_merge(&merged, &parent, &child, &diagnostic));
  assert(strcmp(merged.file_cache_backend, "") == 0);
  assert(strcmp(merged.image_cache, TEST_CHILD_CACHE_ROOT) == 0);
  assert(laghu_service_config_finalize(
      &merged, &(laghu_service_finalize_options){0}, &diagnostic));

  laghu_service_config_init(&parent);
  laghu_service_config_init(&child);
  assert(laghu_service_config_apply(&parent, LAGHU_SERVICE_SETTING_IMAGE_CACHE,
                                    TEST_PARENT_CACHE_ROOT, &diagnostic));
  assert(laghu_service_config_apply(&child,
                                    LAGHU_SERVICE_SETTING_FILE_CACHE_BACKEND,
                                    TEST_CHILD_CACHE_URI, &diagnostic));
  assert(laghu_service_config_merge(&merged, &parent, &child, &diagnostic));
...
static void test_cidr_fixtures(void) {
  static const char *const valid[] = {"0.0.0.0/0",
                                      "192.0.2.0/24",
                                      "255.255.255.255/32",
                                      "::/0",
                                      "::1/128",
  ...
  static const char *const invalid[] = {
      "1.2.3/24",        "1.2.3.4.5/32",     "256.0.0.0/8",
      "01.2.3.4/32",     "192.0.2.0/33",     "192.0.2.0/24 ",
      "2001:db8:::1/64", "1:2:3:4:5:6:7/64", "1:2:3:4:5:6:7:8:9/128",
      "12345::/64",      "fe80::1%zone/128", " 2001:db8::/32",
...
static void test_cidrs_and_dependencies(void) {
  ...
  assert(laghu_service_cidr_matches(&cidr, address,
                                    LAGHU_SERVICE_CIDR_FAMILY_IPV4));
  assert(!laghu_service_cidr_matches(&cidr, address,
                                     LAGHU_SERVICE_CIDR_FAMILY_IPV6));
  assert(!laghu_service_cidr_parse("192.0.2.1/24", &cidr));
  assert(!laghu_service_cidr_parse("not-an-address/24", &cidr));
  assert(laghu_service_config_apply(&config, LAGHU_SERVICE_SETTING_METRICS,
                                    "on", &diagnostic));
  ...
  assert(laghu_service_config_apply(&config,
                                    ...
                                    "/tmp/token", &diagnostic));
  assert(laghu_service_config_apply(&config, LAGHU_SERVICE_SETTING_PURGE_ALLOW,
                                    "192.0.2.0/24", &diagnostic));
  assert(laghu_service_config_finalize(&config, &options, &diagnostic));
  laghu_service_config_init(&config);
  {
    unsigned int index;
    ...
    for (index = 0U; index < LAGHU_SERVICE_CONFIG_MAX_CIDRS; ++index) {
      ...
      assert(laghu_service_config_apply(
          &config, LAGHU_SERVICE_SETTING_TRUSTED_PROXY, value, &diagnostic));
    }
    assert(!laghu_service_config_apply(&config,
                                       LAGHU_SERVICE_SETTING_TRUSTED_PROXY,
                                       "198.51.100.0/24", &diagnostic));
...
static void test_cache_backend_normalization(void) {
  ...
  laghu_service_config_init(&config);
  assert(laghu_service_config_apply(&config, LAGHU_SERVICE_SETTING_IMAGE_CACHE,
                                    TEST_CACHE_ROOT, &diagnostic));
  assert(laghu_service_config_finalize(&config, &options, &diagnostic));
  assert(strcmp(config.image_cache, TEST_CACHE_ROOT) == 0);
  laghu_service_config_init(&config);
  assert(laghu_service_config_apply(&config,
  ...
  assert(laghu_service_config_apply(&config,
                                    LAGHU_SERVICE_SETTING_FILE_CACHE_BACKEND,
                                    "redis://cache.example", &diagnostic));
  assert(!laghu_service_config_finalize(&config, &options, &diagnostic));
...
static void test_prepared_resources(void) {
  ...
  assert(laghu_service_config_prepare_resources(&parent, &diagnostic));
  assert(laghu_service_config_apply(
      &parent, LAGHU_SERVICE_SETTING_JAVASCRIPT_OBSERVATION_CONFIG,
      "/missing/laghu-observations", &diagnostic));
  parent.owned_font_providers =
      calloc(1U, sizeof(*parent.owned_font_providers));
  ...
  assert(laghu_service_config_apply(&parent,
                                    ...
                                    "/missing/laghu-assets", &diagnostic));
  ...
  laghu_service_config_init(&child);
  assert(laghu_service_config_apply(
      &parent, LAGHU_SERVICE_SETTING_JAVASCRIPT_DEFER_CONFIG,
      "/prepared/parent", &diagnostic));
  parent.owned_javascript_defer =
      calloc(1U, sizeof(*parent.owned_javascript_defer));
  ...
  assert(laghu_service_config_apply(
      &child, LAGHU_SERVICE_SETTING_JAVASCRIPT_DEFER_CONFIG, "/prepared/child",
      &diagnostic));
  child.owned_javascript_defer =
      calloc(1U, sizeof(*child.owned_javascript_defer));
  ...
  assert(laghu_service_config_merge(&child, &parent, &child, &diagnostic));
  assert(child.javascript_defer != NULL &&
         child.owned_javascript_defer == child.javascript_defer);
