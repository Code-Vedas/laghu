...
enum {
  TEST_OPERATIONAL_FILE_SIZE =
      LAGHU_WIRE_OPERATIONAL_HEADER_SIZE +
      LAGHU_WIRE_OPERATIONAL_SLOT_COUNT * LAGHU_WIRE_OPERATIONAL_SLOT_SIZE
};
...
static unsigned char *operational_slot(laghu_runtime_shared_mapping *mapping,
                                       unsigned int index) {
  return (unsigned char *)mapping->mapping +
         LAGHU_WIRE_OPERATIONAL_HEADER_SIZE +
         (size_t)index * LAGHU_WIRE_OPERATIONAL_SLOT_SIZE;
}

static unsigned int operational_active_slot(
    laghu_runtime_shared_mapping *mapping) {
  unsigned int index;
  for (index = 0U; index < LAGHU_WIRE_OPERATIONAL_SLOT_COUNT; ++index)
    if (laghu_wire_u32_read(operational_slot(mapping, index) +
                            LAGHU_WIRE_OPERATIONAL_SLOT_ACTIVE_OFFSET) != 0U)
      return index;
...
static void test_snapshot_ignores_whole_file_lock(const char *root) {
      ...
      LAGHU_OPERATIONAL_PROCESS_ADAPTER, false, 100U));
  {
    ...
    if (child == 0) {
      ...
      if (!laghu_runtime_shared_mapping_open(&mapping, path,
                                             TEST_OPERATIONAL_FILE_SIZE) ||
          !laghu_runtime_shared_mapping_try_lock(&mapping))
        _exit(1);
...
static void test_invalid_files_fail_open(const char *root) {
  ...
  laghu_wire_u64_write((unsigned char *)mapping.mapping +
                           LAGHU_WIRE_OPERATIONAL_HEADER_MAGIC_OFFSET,
                       LAGHU_WIRE_OPERATIONAL_MAGIC);
  laghu_wire_u32_write((unsigned char *)mapping.mapping +
                           LAGHU_WIRE_OPERATIONAL_HEADER_VERSION_OFFSET,
                       LAGHU_WIRE_OPERATIONAL_VERSION + 1U);
  laghu_wire_u32_write((unsigned char *)mapping.mapping +
...
static void test_generation_sequence_and_saturation(const char *root) {
  ...
  assert(laghu_operational_registry_open(
      &owner, root, LAGHU_OPERATIONAL_SURFACE_STANDALONE,
      LAGHU_OPERATIONAL_PROCESS_ADAPTER, false, 100U));
  laghu_runtime_shared_mapping_init(&mapping);
  assert(laghu_runtime_shared_mapping_open(&mapping, path,
                                           TEST_OPERATIONAL_FILE_SIZE));
  ...
  assert(laghu_operational_registry_open(
      &replacement, root, LAGHU_OPERATIONAL_SURFACE_WORKER,
      LAGHU_OPERATIONAL_PROCESS_JAVASCRIPT, true, 200U));
  ...
  slot = operational_active_slot(&mapping);
  assert(slot < LAGHU_WIRE_OPERATIONAL_SLOT_COUNT);
  assert(laghu_wire_u64_read(operational_slot(&mapping, slot) +
                             LAGHU_WIRE_OPERATIONAL_SLOT_GENERATION_OFFSET) !=
         generation);
  laghu_runtime_shared_mapping_close(&mapping);
  ...
  assert(laghu_operational_registry_open(
      &owner, root, LAGHU_OPERATIONAL_SURFACE_STANDALONE,
      LAGHU_OPERATIONAL_PROCESS_ADAPTER, false, 300U));
  laghu_runtime_shared_mapping_init(&mapping);
  assert(laghu_runtime_shared_mapping_open(&mapping, path,
                                           TEST_OPERATIONAL_FILE_SIZE));
  assert(laghu_runtime_shared_mapping_try_lock(&mapping));
  ...
  generation =
      laghu_wire_u64_read(operational_slot(&mapping, slot) +
                          LAGHU_WIRE_OPERATIONAL_SLOT_GENERATION_OFFSET);
  laghu_wire_u64_write(operational_slot(&mapping, slot) +
                           LAGHU_WIRE_OPERATIONAL_SLOT_SEQUENCE_OFFSET,
                       3U);
  laghu_runtime_shared_mapping_unlock(&mapping);
  assert(!laghu_operational_registry_snapshot(&owner, &snapshot));
  assert(laghu_operational_registry_open(
      &other, root, LAGHU_OPERATIONAL_SURFACE_WORKER,
      LAGHU_OPERATIONAL_PROCESS_JAVASCRIPT, true, 400U));
  assert(laghu_wire_u64_read(operational_slot(&mapping, slot) +
                             LAGHU_WIRE_OPERATIONAL_SLOT_GENERATION_OFFSET) ==
         generation);
  laghu_wire_u64_write(operational_slot(&mapping, slot) +
...
int main(void) {
  ...
  assert(laghu_operational_registry_open(
      &registry, root, LAGHU_OPERATIONAL_SURFACE_STANDALONE,
      LAGHU_OPERATIONAL_PROCESS_ADAPTER, false, 100U));
  assert(operational_path(root, operations_path_buffer,
                          sizeof(operations_path_buffer)));
