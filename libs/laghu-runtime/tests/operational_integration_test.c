...
static void *operational_thread(void *data) {
  ...
  for (index = 0U; index < context->iterations; ++index)
    laghu_operational_registry_record(context->registry, LAGHU_OPERATIONAL_DECISION_ORIGINAL, 10U, 10U, 1000U);
  return NULL;
}
...
static void test_operational_registry(const char *cache_path) {
  ...
  assert(laghu_operational_registry_open(&adapter, cache_path, LAGHU_OPERATIONAL_SURFACE_STANDALONE, LAGHU_OPERATIONAL_PROCESS_ADAPTER, true, now));
  assert(laghu_operational_registry_open(&worker, cache_path, LAGHU_OPERATIONAL_SURFACE_WORKER, LAGHU_OPERATIONAL_PROCESS_LIBVIPS, true, now));
  thread_context.registry = &adapter;
  thread_context.iterations = 10000U;
  for (thread_index = 0U; thread_index < 4U; ++thread_index) {
    assert(pthread_create(&threads[thread_index], NULL, operational_thread, &thread_context) == 0);
  }
  for (thread_index = 0U; thread_index < 4U; ++thread_index) {
    assert(pthread_join(threads[thread_index], NULL) == 0);
  }
  laghu_operational_registry_record(&adapter, LAGHU_OPERATIONAL_DECISION_OPTIMIZED, 1000U, 600U, 25000U);
  laghu_operational_registry_record(&adapter, LAGHU_OPERATIONAL_DECISION_BYPASS, 200U, 200U, 5000001U);
  laghu_operational_registry_failure(&adapter, LAGHU_OPERATIONAL_FAILURE_TRANSFORM);
  cache.hits = 4U;
  cache.misses = 2U;
  ...
  laghu_operational_registry_budget(&adapter, &budget, 50U);
  {
    ...
    laghu_operational_registry_lcp(&adapter, LAGHU_LCP_DECISION_LEARNED, true, observations, ready);
  }
  assert(laghu_operational_registry_heartbeat(&worker, now, true, 64U, 3U));
  laghu_operational_registry_worker_job(&worker, false, 1000U, LAGHU_OPERATIONAL_FAILURE_WORKER);
  assert(laghu_operational_registry_snapshot(&adapter, snapshot));
  {
    unsigned int slot;
    bool found = false;
    for (slot = 0U; slot < snapshot->slot_count; ++slot) {
      if (snapshot->slots[slot].active != 0U && snapshot->slots[slot].surface == LAGHU_OPERATIONAL_SURFACE_STANDALONE) {
  ...
  assert(laghu_operational_render_prometheus(snapshot, now, output, LAGHU_OPERATIONAL_RENDER_SIZE, &length));
  ...
  assert(strstr(output, "laghu_failures_total{subsystem=\"worker\"} 1") != NULL);
  assert(strstr(output, "laghu_variant_occupancy 3") != NULL);
  assert(strstr(output, "laghu_lcp_decisions_total{decision=\"learned\"} 1") != NULL);
  assert(strstr(output,
                "laghu_lcp_profile_ready{viewport=\"mobile\",theme=\"dark\"} "
                "1") != NULL);
  assert(strstr(output, "laghu_transform_rejections_total{reason=\"deadline\"} 1") != NULL);
  assert(strstr(output, "laghu_response_bytes_total{kind=\"saved\"} 400") != NULL);
  assert(strstr(output, cache_path) == NULL);
  assert(laghu_operational_readiness_evaluate(snapshot, now, true, true, false, &readiness));
  ...
  assert(laghu_operational_readiness_evaluate(snapshot, now, true, true, true, &readiness));
  assert(!readiness.workers_ready && readiness.degraded);
  assert(laghu_operational_render_readiness(&readiness, true, output, LAGHU_OPERATIONAL_RENDER_SIZE, &length));
