  ...
  laghu_lcp_result_release(&result);
  {
    ...
    assert(strstr(result.link_header, "</hero-2.webp>") != NULL &&
           strstr(result.link_header,
                  "imagesrcset=\"/hero-1.webp 1x, /hero-2.webp 2x\"") != NULL);
    assert(strstr((const char *)result.data, "fetchpriority=\"high\"") != NULL);
    laghu_lcp_result_release(&result);
  }
  {
...
static void test_image_markup_and_headers(laghu_rum_engine *rum,
  ...
  assert(
      laghu_sha256_hex((laghu_buffer){(const unsigned char *)"runtime payload",
  ...
  assert(laghu_csp_policy_add(&deny_data_csp, "img-src 'none'",
                              sizeof("img-src 'none'") - 1U));
  assert(
      laghu_sha256_hex((laghu_buffer){tiny_png, sizeof(tiny_png)}, first_key));
  ...
  assert(laghu_runtime_cache_publish(
      temporary, index_key, second_key, "page", "image/png", "test-backend",
      (laghu_buffer){(const unsigned char *)"runtime payload",
                     sizeof("runtime payload") - 1U},
      &entry));
  ...
  assert(laghu_catalog_publish_url(temporary, &catalog));
  {
    ...
    assert(laghu_runtime_finalize_html_headers(
        temporary, (laghu_buffer){hinted_html, sizeof(hinted_html) - 1U},
        "/index.html", "https://example.test", policy_key, 0x55aaU, 2001U,
        604800U, LAGHU_HTML_PLAN_CONVERT_META_TAGS, "fr-CA", NULL, 2048U, 8192U,
        true, &hints));
  ...
  assert(laghu_runtime_rewrite_html(
      rum, temporary, (laghu_buffer){html, sizeof(html) - 1U}, "/index.html",
  ...
  assert(laghu_runtime_rewrite_html(
      rum, temporary, (laghu_buffer){inline_html, sizeof(inline_html) - 1U},
      "/index.html", "https://example.test", policy_key, 0x55aaU, 2001U,
      604800U, LAGHU_IMAGE_INLINE | LAGHU_IMAGE_DEDUP_INLINE, true, false,
      false, false, 0U, NULL, false, 2048U, 2048U, 8192U, 0U, 100U, &page));
  ...
  assert(laghu_runtime_rewrite_html(
      ...
      "/index.html", "https://example.test", policy_key, 0x55aaU, 2001U,
      604800U, LAGHU_IMAGE_INLINE | LAGHU_IMAGE_DEDUP_INLINE, true, false,
      false, false, 0U, &deny_data_csp, false, 2048U, 2048U, 8192U, 0U, 100U,
      &page));
  assert(page.rewritten);
...
static void test_instrumentation_beacons(const laghu_test_workspace *workspace,
                                         laghu_rum_engine *rum_engine,
                                         const char *policy_key) {
  static const char valid[] =
      "# exact observation rules\nhost cdn.example.test /assets/\n";
  ...
  assert(laghu_test_workspace_path(workspace, "javascript-observation.conf",
                                   observation_path, sizeof(observation_path)));
  assert(laghu_test_workspace_write(workspace, "javascript-observation.conf",
                                    (const unsigned char *)valid,
                                    sizeof(valid) - 1U));
  assert(laghu_javascript_observations_load(observation_path, &providers, NULL,
                                            0U));
  assert(providers.count == 1U && providers.digest[0] != '\0');
  assert(laghu_runtime_add_instrumentation(
      rum_engine, workspace->path, &providers,
      (laghu_buffer){
          (const unsigned char
  ...
  assert(laghu_instrumentation_apply_beacon(rum_engine, workspace->path, 3001U,
                                            604800U, &rum));
  assert(snprintf(json, sizeof(json),
  ...
  assert(rum.version == 2U && rum.color_scheme_bucket == 1U &&
         rum.lcp_kind == 0U);
  assert(laghu_instrumentation_apply_beacon(rum_engine, workspace->path, 3002U,
                                            604800U, &rum));
  ...
  assert(laghu_test_workspace_write(workspace, "javascript-observation.conf",
                                    (const unsigned char *)invalid_host,
                                    sizeof(invalid_host) - 1U));
  assert(!laghu_javascript_observations_load(observation_path, &providers, NULL,
                                             0U));
  assert(laghu_test_workspace_write(workspace, "javascript-observation.conf",
