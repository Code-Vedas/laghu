...
int main(void) {
  ...
  assert(
      laghu_test_workspace_path(&first, "nested/data.bin", path, sizeof(path)));
  assert(laghu_test_workspace_write(&first, "nested/data.bin", data,
                                    sizeof(data)));
  assert(fixture_path_exists(path));
  file = fopen(path, "rb");
  assert(file != NULL &&
         fread(readback, 1U, sizeof(readback), file) == sizeof(readback) &&
         fclose(file) == 0);
  ...
  assert(!laghu_test_workspace_path(&first, "nested/../escape", path,
                                    sizeof(path)));
  assert(
      !laghu_test_workspace_path(&first, "nested\\escape", path, sizeof(path)));
  ...
  assert(
      laghu_test_queue_pair_open(&pair, &first, "queue/jobs.queue", 2U, 64U));
  laghu_test_queue_pair_close(&pair);
  assert(!laghu_test_queue_pair_open(&pair, &first, "queue/invalid.queue", 0U,
                                     64U));
