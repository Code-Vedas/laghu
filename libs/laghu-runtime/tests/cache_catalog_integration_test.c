...
static void test_cache_backend_uri(const laghu_test_workspace *workspace) {
  ...
  assert(!laghu_cache_backend_uri_parse("memcached://127.0.0.1", parsed, sizeof(parsed)));
  assert(!laghu_cache_backend_uri_parse("file://server/cache", parsed, sizeof(parsed)));
  assert(!laghu_cache_backend_uri_parse("file:///tmp/cache?secret=x", parsed, sizeof(parsed)));
  assert(!laghu_cache_backend_uri_parse("file:///tmp/%2fcache", parsed, sizeof(parsed)));
...
static void test_cache_backend_governance(const laghu_test_workspace *workspace, const unsigned char *payload, size_t payload_length,
                                          const char *index_key, const char *no_webp_index_key, const char *policy_key) {
  laghu_cache_backend *backend = calloc(1U, sizeof(*backend));
  laghu_cache_limits limits;
  ...
  assert(laghu_test_workspace_path(workspace, "backend-cache", backend_path, sizeof(backend_path)));
  assert(laghu_test_workspace_path(workspace, "backend-cache/.keep", placeholder, sizeof(placeholder)));
  assert(laghu_test_workspace_write(workspace, "backend-cache/.keep", NULL, 0U));
  ...
  assert(stats.bytes == payload_length && stats.files == 3U && stats.hits == 1U && stats.publications == 1U && stats.variant_occupancy == 1U &&
         stats.cache_generation == 1U);
  {
    ...
    assert(snprintf(metadata_path, sizeof(metadata_path), "%s.laghu-metadata", backend_path) > 0);
    ...
    for (cursor = 0U; cursor + payload_length <= length; ++cursor)
      if (memcmp(metadata + cursor, payload, payload_length) == 0) payload_found = true;
    assert(!payload_found);
    free(metadata);
  }
  assert(!laghu_cache_backend_publish(backend, no_webp_index_key, no_webp_index_key, "etag-2", "text/plain", "test",
  ...
  assert(!laghu_cache_backend_lookup(backend, index_key, "etag", &backend_entry));
  assert(laghu_cache_backend_health(backend, &stats));
  assert(stats.bytes == 0U && stats.files == 0U && stats.evictions == 1U && stats.variant_occupancy == 0U);
  {
    char normalized[LAGHU_RUNTIME_PATH_SIZE];
    char source_hash[LAGHU_RUNTIME_KEY_SIZE];
    bool control = false;
    uint64_t matched = 0U, generation = 0U;
    assert(laghu_cache_source_normalize("/image.png?v=1&laghu=purge#ignored", normalized, sizeof(normalized), &control));
    assert(control && strcmp(normalized, "/image.png?v=1") == 0);
    assert(!laghu_cache_source_normalize("/image.png?laghu=purge&laghu=purge", normalized, sizeof(normalized), &control));
    ...
    assert(laghu_cache_backend_publish(backend, index_key, policy_key, "etag", "text/plain", "test", (laghu_buffer){payload, payload_length},
                                       &backend_entry));
    assert(laghu_cache_backend_purge_url(backend, "/image.png?v=1", 200U, &matched) == LAGHU_CACHE_PURGE_ACCEPTED);
    assert(matched == 1U);
    assert(!laghu_cache_backend_lookup(backend, index_key, "etag", &backend_entry));
    ...
    assert(laghu_cache_backend_health(backend, &stats));
    assert(stats.url_purges == 1U && stats.full_purges == 1U && stats.invalidated_artifacts >= 1U && stats.cache_generation == 7U);
  }
  {
    char metadata_path[LAGHU_RUNTIME_PATH_SIZE];
    uint64_t zero = 0U;
...
static void test_catalog_learning(const char *temporary, const char *index_key, const char *policy_key) {
  ...
  assert(loaded.natural_width == 800U && loaded.variant_count == 2U);
  {
    ...
    assert(laghu_runtime_parse_image_beacon((laghu_buffer){json, sizeof(json) - 1U}, &beacon));
    assert(beacon.mobile && beacon.above_fold && beacon.width == 240U);
    assert(laghu_catalog_apply_beacon(rum_engine, temporary, policy_key, 0x55aaU, 1002U, 604800U, &beacon));
    assert(laghu_catalog_lookup_url(temporary, "/image.png", policy_key, 0x55aaU, 1003U, 604800U, &loaded));
    {
      char identity[LAGHU_RUNTIME_KEY_SIZE];
      laghu_rum_image_record learning;
      laghu_rum_value value;
      assert(laghu_catalog_url_identity("/image.png", policy_key, 0x55aaU, identity));
      assert(laghu_rum_engine_read(rum_engine, LAGHU_RUM_RECORD_IMAGE, identity, 1003U, &learning, sizeof(learning), &value));
    ...
    }
    {
      ...
      assert(!laghu_runtime_parse_image_beacon((laghu_buffer){invalid, sizeof(invalid) - 1U}, &beacon));
    }
  }
  assert(!laghu_catalog_lookup(temporary, catalog_key, 700000U, 604800U, &loaded));
  {
    ...
    assert(laghu_catalog_key(catalog.normalized_url, catalog.source_hash, catalog.policy_key, catalog.capability_mask, catalog_key));
    assert(snprintf(lock_path, sizeof(lock_path), "%s/catalog/%s.meta.lock", temporary, catalog_key) > 0);
    ...
    assert(!laghu_catalog_lookup_url(temporary, "/locked.png", policy_key, 0x55aaU, 800101U, 604800U, &loaded));
    assert(laghu_catalog_lookup_url(temporary, "/newest.png", policy_key, 0x55aaU, 800101U, 604800U, &loaded));
  }
  laghu_rum_engine_destroy(rum_engine);
}

static void test_file_cache_artifacts(const char *temporary, const unsigned char *payload, size_t payload_length, const char *index_key,
                                      const char *policy_key) {
  laghu_runtime_cache_entry entry;
  unsigned char cached[64];

  assert(laghu_runtime_cache_publish(temporary, index_key, policy_key, "etag", "image/png", "test-backend", (laghu_buffer){payload, payload_length},
                                     &entry));
  assert(!laghu_runtime_cache_publish(NULL, index_key, policy_key, "etag", "image/png", "test-backend", (laghu_buffer){payload, payload_length},
                                      &entry));
...
int main(void) {
  ...
  assert(laghu_runtime_index_key("/image.png", "etag", policy_key, true, false, false, 0U, 0U, index_key));
  assert(laghu_runtime_index_key("/image.png", "etag", policy_key, false, false, false, 0U, 0U, no_webp_index_key));
  assert(strcmp(index_key, no_webp_index_key) != 0);
  assert(laghu_runtime_index_key("/image.png", "etag", policy_key, true, false, false, 640U, 480U, no_webp_index_key));
  assert(strcmp(index_key, no_webp_index_key) != 0);
