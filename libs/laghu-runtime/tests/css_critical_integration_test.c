...
static void test_font_css(const laghu_test_workspace *workspace) {
  ...
  assert(laghu_test_workspace_path(workspace, "font-providers.conf", path,
                                   sizeof(path)));
  assert(laghu_test_workspace_write(workspace, "font-providers.conf",
  ...
  assert(laghu_runtime_rewrite_font_css(&queue.producer, workspace->path,
                                        &providers,
                                        (laghu_buffer){page, sizeof(page) - 1U},
                                        100U, true, NULL, 2048U, &rewritten));
  assert(!rewritten.rewritten && rewritten.dependencies_pending);
  assert(laghu_runtime_queue_try_take(&queue.consumer, &job, ignored,
                                      sizeof(ignored)));
  assert(job.kind == LAGHU_RUNTIME_JOB_FONT_CSS);
  assert(strcmp(job.provider_id, "google_fonts") == 0);
  assert(laghu_sha256_hex((laghu_buffer){google_css, sizeof(google_css) - 1U},
                          css_key));
  assert(laghu_runtime_cache_publish(
      workspace->path, css_key, css_key, css_key, "text/css", "test-font-fetch",
...
static void test_css_derivation(const char *path, const char *policy_key) {
  ...
  assert(laghu_runtime_rewrite_css(
      NULL, path, (laghu_buffer){css, sizeof(css) - 1U}, "/site.css",
  ...
  assert(laghu_runtime_rewrite_css(
      NULL, path, (laghu_buffer){leaf_css, sizeof(leaf_css) - 1U}, "/leaf.css",
      "https://example.test", policy_key, 0x55aaU, 900010U, 604800U, true,
      false, 2048U, 8192U, &stylesheet));
  assert(stylesheet.published && !stylesheet.rewritten);
  laghu_runtime_css_result_release(&stylesheet);
  assert(laghu_runtime_rewrite_css(
      NULL, path, (laghu_buffer){leaf_css, sizeof(leaf_css) - 1U}, "/leaf.css",
      "https://example.test", policy_key, 0x55aaU, 900011U, 604800U, true,
      false, 2048U, 8192U, &stylesheet));
  assert(stylesheet.rewritten);
  laghu_runtime_css_result_release(&stylesheet);
  assert(laghu_runtime_rewrite_css(
      NULL, path, (laghu_buffer){root_css, sizeof(root_css) - 1U}, "/root.css",
      "https://example.test", policy_key, 0x55aaU, 900012U, 604800U, true, true,
      2048U, 8192U, &stylesheet));
  assert(stylesheet.published && !stylesheet.rewritten);
  laghu_runtime_css_result_release(&stylesheet);
  assert(laghu_runtime_rewrite_css(
      NULL, path, (laghu_buffer){root_css, sizeof(root_css) - 1U}, "/root.css",
      "https://example.test", policy_key, 0x55aaU, 900013U, 604800U, true, true,
      2048U, 8192U, &stylesheet));
  assert(stylesheet.rewritten &&
         strstr((const char *)stylesheet.data, "@import") == NULL);
  ...
  laghu_runtime_css_result_release(&stylesheet);
  {
    ...
    assert(laghu_runtime_rewrite_css(
        NULL, path, (laghu_buffer){cycle_a, sizeof(cycle_a) - 1U},
        "/cycle-a.css", "https://example.test", policy_key, 0x55aaU, 900014U,
        604800U, true, true, 2048U, 8192U, &stylesheet));
    assert(stylesheet.dependencies_pending && !stylesheet.rewritten);
    laghu_runtime_css_result_release(&stylesheet);
    assert(laghu_runtime_rewrite_css(
        NULL, path, (laghu_buffer){cycle_b, sizeof(cycle_b) - 1U},
        "/cycle-b.css", "https://example.test", policy_key, 0x55aaU, 900015U,
        604800U, true, true, 2048U, 8192U, &stylesheet));
  ...
  }
  {
    ...
    assert(laghu_runtime_rewrite_css_markup(
        path, (laghu_buffer){linked, sizeof(linked) - 1U}, "/index.html",
    ...
    assert(
        laghu_csp_policy_add(&nonce_csp, "style-src 'self' 'nonce-c2FmZQ=='",
                             sizeof("style-src 'self' 'nonce-c2FmZQ=='") - 1U));
    assert(laghu_csp_allows_inline_style(
        &nonce_csp, (const unsigned char *)"c2FmZQ==", 8U));
    assert(laghu_runtime_rewrite_css_markup_csp(
        path, (laghu_buffer){nonce_linked, sizeof(nonce_linked) - 1U},
        "/index.html", "https://example.test", policy_key, 0x55aaU, 900003U,
        604800U, true, false, false, 0U, &nonce_csp, 2048U, 8192U, &markup));
    assert(!markup.rewritten && markup.dependencies_pending);
    laghu_runtime_html_result_release(&markup);
    assert(laghu_runtime_rewrite_css_markup_csp(
        path, (laghu_buffer){nonce_linked, sizeof(nonce_linked) - 1U},
        "/index.html", "https://example.test", policy_key, 0x55aaU, 900004U,
        604800U, true, false, false, 0U, &nonce_csp, 2048U, 8192U, &markup));
    assert(markup.rewritten && strstr((const char *)markup.data,
                                      "<style nonce=\"c2FmZQ==\">") != NULL);
    laghu_runtime_html_result_release(&markup);
  }
  {
    ...
    assert(laghu_runtime_rewrite_css(
        NULL, path, (laghu_buffer){long_leaf_css, sizeof(long_leaf_css) - 1U},
        long_path, "https://example.test", policy_key, 0x55aaU, 900020U,
        604800U, true, false, 2048U, 8192U, &stylesheet));
    laghu_runtime_css_result_release(&stylesheet);
    assert(laghu_runtime_rewrite_css(
        NULL, path, (laghu_buffer){long_leaf_css, sizeof(long_leaf_css) - 1U},
        long_path, "https://example.test", policy_key, 0x55aaU, 900021U,
        604800U, true, false, 2048U, 8192U, &stylesheet));
    assert(stylesheet.rewritten);
    laghu_runtime_css_result_release(&stylesheet);
...
static void test_css_outline_combine_and_csp(const char *path,
  ...
  offset += 8U;
  assert(laghu_runtime_rewrite_css_markup(
      path, (laghu_buffer){outlined, offset}, "/outline.html",
  ...
  assert(laghu_runtime_rewrite_css(
      NULL, path, (laghu_buffer){first_css, sizeof(first_css) - 1U},
  ...
  laghu_runtime_css_result_release(&sheet);
  assert(laghu_runtime_rewrite_css(
      NULL, path, (laghu_buffer){second_css, sizeof(second_css) - 1U},
  ...
  assert(laghu_runtime_combine_css_markup(
      path, (laghu_buffer){combined_html, sizeof(combined_html) - 1U},
  ...
  laghu_runtime_css_combine_result_release(&combined);
  {
    ...
    assert(laghu_runtime_rewrite_css(
        NULL, path, (laghu_buffer){changed_css, sizeof(changed_css) - 1U},
        "/first.css", "https://example.test", policy_key, 0x55aaU, 910004U,
        604800U, true, false, 2048U, 8192U, &sheet));
    laghu_runtime_css_result_release(&sheet);
    assert(laghu_runtime_rewrite_css(
        NULL, path, (laghu_buffer){changed_css, sizeof(changed_css) - 1U},
        "/first.css", "https://example.test", policy_key, 0x55aaU, 910005U,
        604800U, true, false, 2048U, 8192U, &sheet));
    laghu_runtime_css_result_release(&sheet);
    assert(laghu_runtime_combine_css_markup(
        path, (laghu_buffer){combined_html, sizeof(combined_html) - 1U},
  ...
  assert(laghu_runtime_combine_css_markup(
      path, (laghu_buffer){split_html, sizeof(split_html) - 1U}, "/index.html",
      "https://example.test", policy_key, 0x55aaU, 910003U, 604800U, 2048U,
      8192U, &combined));
  assert(!combined.rewritten && !combined.dependencies_pending);
  laghu_runtime_css_combine_result_release(&combined);
  assert(laghu_runtime_combine_css_markup(
      path, (laghu_buffer){media_html, sizeof(media_html) - 1U}, "/index.html",
      "https://example.test", policy_key, 0x55aaU, 910003U, 604800U, 2048U,
      8192U, &combined));
  assert(!combined.rewritten && !combined.dependencies_pending);
  laghu_runtime_css_combine_result_release(&combined);
  {
    ...
    assert(laghu_csp_allows_external_script(&csp, nonce, sizeof(nonce) - 1U));
    {
      ...
      size_t index;
      for (index = 0U; index + sizeof(nonce) - 1U <= sizeof(csp); ++index)
        assert(memcmp(bytes + index, nonce, sizeof(nonce) - 1U) != 0);
    }
  ...
  }
  {
    ...
    laghu_csp_policy_init(&csp, "https://example.test");
    assert(laghu_csp_policy_add_meta(&csp,
                                     (laghu_buffer){meta, sizeof(meta) - 1U}));
  ...
  }
  {
    ...
    assert(laghu_csp_policy_add(
        &csp, "default-src 'self', style-src https:",
        sizeof("default-src 'self', style-src https:") - 1U));
    assert(laghu_csp_allows_external_style(&csp, NULL, 0U));
    assert(!laghu_csp_allows_inline_style(&csp, NULL, 0U));
    laghu_csp_policy_init(&csp, "https://app.example.test");
    assert(laghu_csp_policy_add(
        &csp, "style-src https://*.example.test/; script-src *.example.test",
        sizeof("style-src https://*.example.test/; script-src *.example.test") -
            1U));
    assert(laghu_csp_allows_external_style(&csp, NULL, 0U));
    assert(laghu_csp_allows_external_script(&csp, NULL, 0U));
    laghu_csp_policy_init(&csp, "https://app.example.test");
    assert(laghu_csp_policy_add(
        &csp, "style-src https://app.example.test/assets/",
        sizeof("style-src https://app.example.test/assets/") - 1U));
    assert(!laghu_csp_allows_external_style(&csp, NULL, 0U));
  }
  {
    static laghu_csp_policy csp;
    ...
    laghu_csp_policy_init(&csp, "https://example.test");
    assert(!laghu_csp_policy_add(&csp, "script-src 'nonce-@@@'",
                                 sizeof("script-src 'nonce-@@@'") - 1U));
    assert(!laghu_csp_allows_external_script(&csp, NULL, 0U));
...
static void test_critical_css(laghu_rum_engine *rum, const char *path,
                              const char *policy_key) {
  static const unsigned char css[] =
  ...
  assert(laghu_runtime_cache_publish(
      path, css_key, css_key, css_key, "text/css", "critical-test",
  ...
  assert(laghu_runtime_prioritize_critical_css(
      rum, path, (laghu_buffer){document, sizeof(document) - 1U}, "/critical",
      "https://example.test", policy_key, 0x55aaU, 2001U, 604800U, 2048U, 8192U,
      1024U, true, NULL, &page));
  ...
  for (index = 0U; index < 3U; ++index)
    assert(laghu_critical_css_apply_beacon(rum, path, policy_key, 2002U + index,
                                           604800U, &observation));
  assert(laghu_runtime_prioritize_critical_css(
      rum, path, (laghu_buffer){document, sizeof(document) - 1U}, "/critical",
      "https://example.test", policy_key, 0x55aaU, 2005U, 604800U, 2048U, 8192U,
      1024U, true, NULL, &page));
  assert(strstr((const char *)page.data, "<style>") == NULL);
  laghu_runtime_html_result_release(&page);
  observation.color_scheme_bucket = 1U;
  for (index = 0U; index < 3U; ++index)
    assert(laghu_critical_css_apply_beacon(rum, path, policy_key, 2005U + index,
                                           604800U, &observation));
  assert(laghu_runtime_prioritize_critical_css(
      rum, path, (laghu_buffer){document, sizeof(document) - 1U}, "/critical",
      "https://example.test", policy_key, 0x55aaU, 2008U, 604800U, 2048U, 8192U,
      1024U, true, NULL, &page));
  assert(page.rewritten);
