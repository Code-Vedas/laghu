...
static bool conformance_contains(const unsigned char *data, size_t length, const char *needle) {
  size_t needle_length = strlen(needle), index;
  if (data == NULL || needle_length == 0U || needle_length > length) return false;
...
static bool conformance_path(char *output, size_t output_size, const char *directory, const char *suffix) {
  int written;
  if (output == NULL || output_size == 0U || directory == NULL || suffix == NULL) return false;
...
static void conformance_run_worker_once(const char *worker, const char *queue_path, const char *cache_path) {
  char command[LAGHU_RUNTIME_PATH_SIZE * 3U];
  int written = snprintf(command, sizeof(command), "\"%s\" --once \"%s\" \"%s\"", worker, queue_path, cache_path);
  assert(written > 0 && (size_t)written < sizeof(command));
  assert(system(command) == 0);
}

static void conformance_signal(const char *base, const char *name, const char *suffix) {
  char path[LAGHU_RUNTIME_PATH_SIZE];
  int written = snprintf(path, sizeof(path), "%s.%s%s", base, name, suffix);
  assert(written > 0 && (size_t)written < sizeof(path));
  assert(laghu_runtime_file_write_atomic(path, (const unsigned char *)"ok", 2U));
}

static const laghu_operational_slot_snapshot *conformance_javascript_slot(const laghu_operational_snapshot *snapshot) {
  unsigned int index;
  for (index = 0U; index < snapshot->slot_count; ++index) {
    const laghu_operational_slot_snapshot *slot = &snapshot->slots[index];
    if (slot->active != 0U && slot->process_kind == LAGHU_OPERATIONAL_PROCESS_JAVASCRIPT) return slot;
  }
  return NULL;
}

static void conformance_registry_peer(const char *cache_path, const char *signal_path) {
  ...
  assert(laghu_operational_registry_open(&observer, cache_path, LAGHU_OPERATIONAL_SURFACE_STANDALONE, LAGHU_OPERATIONAL_PROCESS_ADAPTER, false,
                                         conformance_now()));
  ...
  for (index = 0U; index < sizeof(steps) / sizeof(steps[0]); ++index) {
    ...
    {
      int written = snprintf(request, sizeof(request), "%s.%s", signal_path, steps[index]);
    ...
    if (strcmp(steps[index], "healthy") == 0) {
      assert(javascript != NULL && javascript->healthy != 0U && javascript->queue_capacity == 8U && javascript->queue_occupied == 0U &&
             javascript->latency_count == 1U && javascript->failures[LAGHU_OPERATIONAL_FAILURE_TRANSFORM] == 1U);
      first_generation = javascript->generation;
      assert(laghu_operational_render_prometheus(&snapshot, conformance_now(), output, sizeof(output), &length));
      assert(strstr(output, "laghu_worker_up{process_kind=\"javascript\"") != NULL);
      assert(strstr(output, "laghu_failures_total{subsystem=\"transform\"} 1") != NULL);
      assert(laghu_operational_readiness_evaluate(&snapshot, conformance_now(), true, true, true, &readiness));
      assert(readiness.workers_ready && !readiness.degraded && readiness.configured_workers == 1U && readiness.healthy_workers == 1U);
    } else if (strcmp(steps[index], "stale") == 0) {
      assert(javascript != NULL && javascript->heartbeat == 1U);
      assert(laghu_operational_readiness_evaluate(&snapshot, conformance_now(), true, true, true, &readiness));
      assert(!readiness.workers_ready && readiness.degraded && readiness.configured_workers == 1U && readiness.healthy_workers == 0U);
    } else if (strcmp(steps[index], "released") == 0) {
      assert(javascript == NULL);
    } else {
      assert(javascript != NULL && javascript->generation > first_generation && javascript->healthy != 0U);
    }
...
static void conformance_consume_rust_job(const char *queue_path) {
  ...
  assert(job.kind == LAGHU_RUNTIME_JOB_JAVASCRIPT && strcmp(job.request_path, "/cross-language.js") == 0 &&
         strcmp(job.javascript_target, "last 2 chrome versions") == 0 && job.payload.length == strlen("console.log('rust producer');") &&
         memcmp(payload, "console.log('rust producer');", job.payload.length) == 0);
