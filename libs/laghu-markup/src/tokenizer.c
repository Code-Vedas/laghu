...
static bool laghu_markup_name_char(unsigned char value) {
...
bool laghu_html_next_tag(laghu_buffer html, size_t *cursor,
  ...
  if (cursor == NULL || tag == NULL ||
      (html.data == NULL && html.length != 0U) || *cursor > html.length)
    return false;
  start = *cursor;
next_candidate:
  ...
  if (html.length - start >= 4U && memcmp(html.data + start, "<!--", 4U) == 0) {
    size_t end = start + 4U;
    while (end + 3U <= html.length && memcmp(html.data + end, "-->", 3U) != 0)
      ++end;
...
bool laghu_html_tag_attribute(const laghu_html_tag *tag, const char *name,
  ...
  size_t cursor;
  if (tag == NULL || name == NULL || attribute == NULL ||
      tag->source.data == NULL)
    return false;
  cursor = 1U;
  if (cursor < tag->source.length && tag->source.data[cursor] == '/') ++cursor;
  while (cursor < tag->source.length &&
         laghu_markup_name_char(tag->source.data[cursor]))
    ++cursor;
  while (cursor < tag->source.length) {
    size_t start, name_end, value_start = 0U;
    unsigned char quote = 0U;
    while (cursor < tag->source.length && (isspace(tag->source.data[cursor]) ||
                                           tag->source.data[cursor] == '/'))
      ++cursor;
    ...
    attribute->name =
        (laghu_buffer){tag->source.data + start, name_end - start};
    ...
    if (cursor < tag->source.length && tag->source.data[cursor] == '=') {
      ...
      while (cursor < tag->source.length && isspace(tag->source.data[cursor]))
        ++cursor;
      if (cursor < tag->source.length &&
          (tag->source.data[cursor] == '\'' || tag->source.data[cursor] == '"'))
        quote = tag->source.data[cursor++];
      value_start = cursor;
      while (cursor < tag->source.length &&
             (quote != 0U ? tag->source.data[cursor] != quote
                          : !isspace(tag->source.data[cursor]) &&
                                tag->source.data[cursor] != '>'))
        ++cursor;
      attribute->value =
          (laghu_buffer){tag->source.data + value_start, cursor - value_start};
...
bool laghu_srcset_next(laghu_buffer srcset, size_t *cursor,
  ...
  if (cursor == NULL || candidate == NULL ||
      (srcset.data == NULL && srcset.length != 0U) || *cursor > srcset.length)
    return false;
  while (*cursor < srcset.length &&
         (isspace(srcset.data[*cursor]) || srcset.data[*cursor] == ','))
    ++*cursor;
  if (*cursor == srcset.length) return false;
  start = *cursor;
  while (*cursor < srcset.length && !isspace(srcset.data[*cursor]) &&
         srcset.data[*cursor] != ',')
    ++*cursor;
  end = *cursor;
  ...
  while (descriptor_end > descriptor_start &&
         isspace(srcset.data[descriptor_end - 1U]))
    --descriptor_end;
  if (*cursor < srcset.length) ++*cursor;
  candidate->url = (laghu_buffer){srcset.data + start, end - start};
  candidate->descriptor = (laghu_buffer){srcset.data + descriptor_start,
