...
static bool laghu_http_administrative_query_parse_path_target(laghu_buffer query, char *output, size_t output_capacity, bool required);

static unsigned char laghu_http_administrative_ascii_lower(unsigned char value) {
  return value >= 'A' && value <= 'Z' ? (unsigned char)(value + ('a' - 'A')) : value;
}

static bool laghu_http_administrative_view_valid(laghu_buffer value) { return value.length == 0U || value.data != NULL; }

static bool laghu_http_administrative_is_digit(unsigned char value) { return value >= '0' && value <= '9'; }

static bool laghu_http_administrative_target_split_query(laghu_buffer target, laghu_buffer *path, laghu_buffer *query) {
  size_t index;
  size_t query_start = target.length;
  if (!laghu_http_administrative_view_valid(target) || path == NULL || query == NULL) {
    return false;
  }
  ...
  *path = (laghu_buffer){target.data, query_start};
  if (query_start < target.length) {
    *query = (laghu_buffer){target.data + query_start + 1U, target.length - query_start - 1U};
  } else {
...
static bool laghu_http_administrative_query_param_value(laghu_buffer query, const char *name, bool required, char *output, size_t output_capacity) {
  size_t name_length = strlen(name);
  size_t index = 0U;
  size_t end = 0U;
  size_t equal = 0U;
  if (!laghu_http_administrative_view_valid(query) || (required && output == NULL) || output_capacity == 0U) {
    return false;
  }
  ...
  while (index < query.length) {
    ...
    if (equal > index && (size_t)(equal - index) == name_length && memcmp(query.data + index, name, name_length) == 0) {
      if (equal == end) {
...
static bool laghu_http_administrative_query_parse_limit(laghu_buffer query, unsigned int *value) {
  char text[12];
  size_t index;
  size_t digits = 0U;
  ...
  text[0] = '\0';
  if (!laghu_http_administrative_query_param_value(query, "limit", false, text, sizeof(text))) {
    return false;
  }
  if (text[0] == '\0') {
    *value = 0U;
  ...
  for (index = 0U; text[index] != '\0'; ++index) {
    ...
    if ((parsed > UINT_MAX / 10U) || (parsed > (UINT_MAX - ((unsigned int)(text[index] - '0'))) / 10U)) {
      return false;
    }
...
static bool laghu_http_administrative_append(char *output, size_t capacity, size_t *length, const char *format, ...) {
  va_list args;
  int written;
...
static bool laghu_http_administrative_render_history_fragment(const laghu_http_administrative_history_model *model, char *output, size_t capacity,
                                                              size_t *length) {
  unsigned int index;
  char row[256];
  int written;
  ...
  if (!laghu_http_administrative_append(output, capacity, length,
  ...
  for (index = 0U; index < model->count; ++index) {
    ...
    uint64_t saved = record->selected_bytes > record->original_bytes ? 0U : record->original_bytes - record->selected_bytes;
    ...
    written = snprintf(row, sizeof(row),
                       ...
                       "<td>%llu (%u.%02u%%)</td></tr>",
                       record->index, record->surface, record->process, record->active ? "yes" : "no", record->healthy ? "yes" : "no",
                       record->required ? "yes" : "no", (unsigned long long)record->requests, (unsigned long long)record->original_bytes,
                       (unsigned long long)record->selected_bytes, (unsigned long long)saved, saved_ppm / 100U, saved_ppm % 100U);
...
static bool laghu_http_administrative_render_explain_fragment(const laghu_http_administrative_explain_model *model, char *output, size_t capacity,
                                                              size_t *length) {
  ...
  if (model->has_before || model->has_after || model->has_filter) {
    ...
    const char *before_hash = model->before_hash[0] != '\0' ? model->before_hash : "Unavailable";
    const char *after_target = model->has_after ? model->after : "Not set";
    const char *after_hash = model->after_hash[0] != '\0' ? model->after_hash : "Unavailable";
    const char *comparison_status;
    bool hashes_match = model->has_before && model->has_after && model->before_hash[0] != '\0' && model->after_hash[0] != '\0' &&
    ...
    if (snprintf(comparison, sizeof(comparison),
                 ...
                 before_target, before_hash, after_target, after_hash, model->has_filter ? model->filter : "none", comparison_status) < 0) {
      return false;
    }
  ...
  if (!laghu_http_administrative_append(output, capacity, length,
                                        ...
                                        model->target, model->status, model->source_hash, model->runtime_ready ? "ready" : "unavailable",
                                        model->cache_ready ? "ready" : "unavailable", model->workers_ready ? "ready" : "unavailable",
                                        (unsigned long long)model->hits_ratio_ppm, model->recommendation)) {
    return false;
  }
  if (comparison_block[0] != '\0' && !laghu_http_administrative_append(output, capacity, length, "%s", comparison_block)) {
    return false;
  }
  return laghu_http_administrative_append(output, capacity, length, "</section>");
}

static bool laghu_http_administrative_query_parse_path(laghu_buffer query, laghu_http_administrative_explain_query *query_out) {
  if (query_out == NULL) return false;
  if (query_out->target[0] != '\0') {
    query_out->target[0] = '\0';
  }
  return laghu_http_administrative_query_parse_path_target(query, query_out->target, sizeof(query_out->target), true);
}

static bool laghu_http_administrative_query_parse_path_target(laghu_buffer query, char *output, size_t output_capacity, bool required) {
  if (output == NULL || output_capacity == 0U) return false;
  output[0] = '\0';
  if (!laghu_http_administrative_query_param_value(query, "path", false, output, output_capacity)) {
    return false;
  }
  if (output[0] == '\0') {
    if (!laghu_http_administrative_query_param_value(query, "url", false, output, output_capacity)) {
      return false;
    }
  }
...
static bool laghu_http_administrative_query_parse_text(laghu_buffer query, const char *name, char *output, size_t output_capacity) {
  if (name == NULL || output == NULL || output_capacity == 0U) return false;
  output[0] = '\0';
  return laghu_http_administrative_query_param_value(query, name, false, output, output_capacity);
}

static bool laghu_http_administrative_query_parse_format(laghu_buffer query, bool *json) {
  char format[16];
  if (json == NULL) {
    return false;
  }
  *json = false;
  format[0] = '\0';
  if (!laghu_http_administrative_query_param_value(query, "format", false, format, sizeof(format))) {
    return false;
  }
...
static bool laghu_http_administrative_query_parse_console_view(laghu_buffer query, laghu_http_administrative_console_view *view) {
  char selected[24];
  if (view == NULL) return false;
  selected[0] = '\0';
  *view = LAGHU_HTTP_ADMINISTRATIVE_CONSOLE_VIEW_OVERVIEW;
  if (!laghu_http_administrative_query_param_value(query, "view", false, selected, sizeof(selected))) {
    return false;
  }
  if (selected[0] == '\0') {
    return true;
...
static bool laghu_http_administrative_normalize_legacy_path(laghu_buffer path, char *output, size_t capacity) {
  if (!laghu_http_administrative_view_valid(path) || output == NULL || capacity < 2U || path.length == 0U || path.length >= capacity) {
    return false;
  }
  if (path.data == NULL) return false;
  memcpy(output, path.data, path.length);
  output[path.length] = '\0';
  if (strcmp(output, "/pagespeed_admin") == 0 || strcmp(output, "/pagespeed_admin/") == 0) {
    memcpy(output, "/.laghu/console", sizeof("/.laghu/console"));
    return true;
  }
  if (strcmp(output, "/pagespeed_console") == 0 || strcmp(output, "/pagespeed_console/") == 0) {
    memcpy(output, "/.laghu/metrics", sizeof("/.laghu/metrics"));
    return true;
  }
  if (strcmp(output, "/pagespeed_statistics") == 0 || strcmp(output, "/pagespeed_statistics/") == 0 || strcmp(output, "/pagespeed_stats") == 0 ||
...
static bool laghu_http_administrative_method_is(laghu_buffer method, const char *expected) {
  size_t expected_length = strlen(expected);
  size_t index;
  if (!laghu_http_administrative_view_valid(method) || method.length != expected_length) {
    return false;
  }
  for (index = 0U; index < expected_length; ++index) {
    if (laghu_http_administrative_ascii_lower(method.data[index]) != laghu_http_administrative_ascii_lower((unsigned char)expected[index])) {
      return false;
    }
  }
  return true;
...
static bool laghu_http_administrative_target_copy(laghu_buffer target, char *output, size_t capacity) {
  size_t index;
  if (!laghu_http_administrative_view_valid(target) || output == NULL || capacity < 2U || target.length == 0U || target.length >= capacity) {
    return false;
  }
  for (index = 0U; index < target.length; ++index) {
    if (target.data[index] == '\0' || target.data[index] == '\r' || target.data[index] == '\n') {
      return false;
    }
  }
...
static bool laghu_http_administrative_target_mentions_purge(laghu_buffer target) {
  static const char marker[] = "laghu=purge";
  size_t index;
  if (!laghu_http_administrative_view_valid(target) || target.length < sizeof(marker) - 1U) {
    return false;
  }
...
bool laghu_http_administrative_plan_build(laghu_http_administrative_plan *plan, laghu_buffer method, laghu_buffer target,
  ...
  if (!laghu_http_administrative_target_split_query(target, &path, &query) || !laghu_http_administrative_view_valid(path) ||
  ...
  if (plan == NULL || options == NULL || !laghu_http_administrative_view_valid(method)) {
    return false;
  }
  ...
  if (!laghu_http_administrative_normalize_legacy_path(path, mapped_target, sizeof(mapped_target))) {
    return true;
  }
  path = (laghu_buffer){(const unsigned char *)mapped_target, strlen(mapped_target)};
  normalized = laghu_http_administrative_target_copy(path, raw_target, sizeof(raw_target));
  if (normalized) {
    normalized = laghu_cache_source_normalize(raw_target, plan->normalized_path, sizeof(plan->normalized_path), &purge_control);
  }
  if (!normalized) {
    if (!method_purge && !laghu_http_administrative_target_mentions_purge(target)) {
      return true;
    }
  ...
  if (plan->route == LAGHU_HTTP_ADMINISTRATIVE_ROUTE_METRICS && !options->metrics_enabled) {
    plan->status = 404U;
    return true;
  }
  if (plan->route == LAGHU_HTTP_ADMINISTRATIVE_ROUTE_READINESS && !options->readiness_enabled) {
    plan->status = 404U;
    return true;
  }
  ...
  switch (plan->route) {
    case LAGHU_HTTP_ADMINISTRATIVE_ROUTE_STATS:
      ...
      if (!options->statistics_enabled || (!laghu_http_administrative_method_is(method, "GET") && !plan->head)) {
        plan->status = 405U;
      }
      break;
    ...
    case LAGHU_HTTP_ADMINISTRATIVE_ROUTE_CONSOLE:
      ...
      if (!laghu_http_administrative_query_parse_console_view(query, &plan->console_view)) {
        plan->status = 400U;
        break;
      }
      if (!laghu_http_administrative_query_parse_format(query, &plan->output_json)) {
        plan->status = 400U;
        break;
      }
      if (!laghu_http_administrative_query_parse_path_target(query, plan->console_query.path, sizeof(plan->console_query.path), false) ||
          !laghu_http_administrative_query_parse_text(query, "before", plan->console_query.before, sizeof(plan->console_query.before)) ||
          !laghu_http_administrative_query_parse_text(query, "after", plan->console_query.after, sizeof(plan->console_query.after)) ||
          !laghu_http_administrative_query_parse_text(query, "laghuFilters", plan->console_query.filter, sizeof(plan->console_query.filter))) {
        plan->status = 400U;
        break;
      }
      if (plan->console_query.path[0] == '\0') {
        (void)snprintf(plan->console_query.path, sizeof(plan->console_query.path), "%s", "/");
      }
      ...
      if (canonicalize_alias) {
        (void)snprintf(plan->normalized_path, sizeof(plan->normalized_path), "%s", "/.laghu/console");
      }
      if (plan->console_view == LAGHU_HTTP_ADMINISTRATIVE_CONSOLE_VIEW_EXPLAIN) {
        ...
        if (!laghu_http_administrative_query_parse_path(query, &plan->explain_query) ||
            !laghu_http_administrative_query_parse_text(query, "before", plan->explain_query.before, sizeof(plan->explain_query.before)) ||
            !laghu_http_administrative_query_parse_text(query, "after", plan->explain_query.after, sizeof(plan->explain_query.after)) ||
            !laghu_http_administrative_query_parse_text(query, "laghuFilters", plan->explain_query.filter, sizeof(plan->explain_query.filter))) {
          plan->status = 400U;
          break;
        }
      } else if (plan->console_view == LAGHU_HTTP_ADMINISTRATIVE_CONSOLE_VIEW_HISTORY) {
        ...
        if (history_limit > LAGHU_OPERATIONAL_HISTORY_SIZE) history_limit = LAGHU_OPERATIONAL_HISTORY_SIZE;
        plan->history_query.limit = history_limit;
      }
      if (!options->statistics_enabled || (!laghu_http_administrative_method_is(method, "GET") && !plan->head)) plan->status = 405U;
      break;
    case LAGHU_HTTP_ADMINISTRATIVE_ROUTE_PURGE:
      ...
      (void)snprintf(plan->purge_target, sizeof(plan->purge_target), "%s", plan->normalized_path);
      if (explicit_purge_route && !laghu_http_administrative_query_parse_path_target(query, plan->purge_target, sizeof(plan->purge_target), true)) {
        plan->status = 400U;
        break;
      }
      if (!explicit_purge_route && plan->purge_by_query) {
        ...
        if (!laghu_http_administrative_query_parse_path_target(query, requested_target, sizeof(requested_target), false)) {
          plan->status = 400U;
          break;
        }
        if (requested_target[0] != '\0') (void)snprintf(plan->purge_target, sizeof(plan->purge_target), "%s", requested_target);
      ...
      if ((method_purge && !options->purge_method_enabled) || (purge_control && !options->purge_query_enabled) ||
          (purge_control && options->purge_query_get_only && !laghu_http_administrative_method_is(method, "GET"))) {
        plan->status = 405U;
      }
      break;
...
bool laghu_http_administrative_render_operational(const laghu_http_administrative_plan *plan, const laghu_operational_snapshot *snapshot,
  ...
  size_t length = 0U;
  if (plan == NULL || snapshot == NULL || output == NULL || response == NULL || plan->status != 0U ||
      (plan->action != LAGHU_HTTP_ADMINISTRATIVE_ACTION_METRICS && plan->action != LAGHU_HTTP_ADMINISTRATIVE_ACTION_READINESS)) {
    return false;
  }
  memset(response, 0, sizeof(*response));
  if (plan->action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_METRICS) {
    if (!laghu_operational_render_prometheus(snapshot, now, output, capacity, &length)) {
      return false;
    }
  ...
  if (!laghu_operational_readiness_evaluate(snapshot, now, runtime_ready, cache_ready, strict_workers, &readiness) ||
      !laghu_operational_render_readiness(&readiness, strict_workers, output, capacity, &length)) {
    return false;
  }
  response->status = readiness.runtime_ready && readiness.cache_ready && readiness.workers_ready ? 200U : 503U;
...
bool laghu_http_administrative_render_purge(laghu_cache_purge_result purge_result, uint64_t matched_artifacts, char *output, size_t capacity,
  ...
  written = snprintf(output, capacity, "{\"status\":\"%s\",\"matched_artifacts\":%llu}", status == 202U ? "accepted" : "rejected",
...
static uint64_t laghu_http_administrative_hit_ratio_ppm(const laghu_cache_stats *stats) {
...
bool laghu_http_administrative_build_console_model(const laghu_cache_stats *stats, const laghu_operational_readiness *ready,
  ...
  (void)snprintf(model->runtime, sizeof(model->runtime), "%s", ready->runtime_ready ? "ready" : "degraded");
  (void)snprintf(model->cache, sizeof(model->cache), "%s", ready->cache_ready ? "ready" : "unavailable");
  (void)snprintf(model->workers, sizeof(model->workers), "%s", ready->workers_ready ? "ready" : "unavailable");
  model->hits = stats->hits;
  model->misses = stats->misses;
  model->bytes = stats->bytes;
  (void)snprintf(model->explain_path, sizeof(model->explain_path), "%s", query->path[0] == '\0' ? "/" : query->path);
  (void)snprintf(model->compare_before, sizeof(model->compare_before), "%s", query->before);
  (void)snprintf(model->compare_after, sizeof(model->compare_after), "%s", query->after);
...
bool laghu_http_administrative_render_console_model(const laghu_http_administrative_console_model *model, char *output, size_t capacity,
  ...
  if (!laghu_http_administrative_append(output, capacity, &length,
                                        ...
                                        "</ul></section>",
                                        model->runtime, model->cache, model->workers, (unsigned long long)model->hits,
                                        (unsigned long long)model->misses, (unsigned long long)model->bytes, model->explain_path, model->filter,
                                        model->explain_path, model->compare_before, model->compare_after, model->filter)) {
    return false;
  }
  response->status = 200U;
...
bool laghu_http_administrative_build_console_page_model(const laghu_http_administrative_plan *plan, const laghu_cache_stats *stats,
  ...
  if (plan == NULL || stats == NULL || ready == NULL || snapshot == NULL || model == NULL) {
    return false;
  }
  memset(model, 0, sizeof(*model));
  model->output_json = plan->output_json;
  model->action = plan->action;
  if (!laghu_http_administrative_build_console_model(stats, ready, &plan->console_query, &model->console)) {
    return false;
  }
  switch (plan->action) {
    case LAGHU_HTTP_ADMINISTRATIVE_ACTION_HISTORY:
      model->has_history = true;
      if (!laghu_http_administrative_build_history_model(snapshot, &plan->history_query, &model->history)) {
        return false;
      }
      break;
    case LAGHU_HTTP_ADMINISTRATIVE_ACTION_EXPLAIN:
      model->has_explain = true;
      if (!laghu_http_administrative_build_explain_model(&plan->explain_query, ready, stats, plan->output_json, &model->explain)) {
        return false;
      }
      break;
...
bool laghu_http_administrative_render_console_page(const laghu_http_administrative_plan *plan,
  ...
  size_t length = 0U;
  int written;
  if (plan == NULL || model == NULL || output == NULL || response == NULL || capacity == 0U) {
    return false;
  }
  memset(response, 0, sizeof(*response));
  if (plan->action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_CONSOLE && plan->output_json) {
    written = snprintf(
        ...
        "\"bytes\":%llu,\"ready\":%s}",
        model->console.runtime, model->console.cache, model->console.workers, (unsigned long long)model->console.hits,
        (unsigned long long)model->console.misses, (unsigned long long)model->console.bytes,
        (strcmp(model->console.runtime, "ready") == 0 && strcmp(model->console.cache, "ready") == 0 && strcmp(model->console.workers, "ready") == 0)
            ? "true"
            : "false");
  ...
  if (plan->action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_HISTORY && plan->output_json) {
    if (!model->has_history) return false;
    return laghu_http_administrative_render_history(&model->history, true, output, capacity, response);
  }
  if (plan->action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_EXPLAIN && plan->output_json) {
    if (!model->has_explain) return false;
    return laghu_http_administrative_render_explain(&model->explain, output, capacity, response);
  }
  if (!laghu_http_administrative_render_console_model(&model->console, output, capacity, response)) {
    return false;
  }
  ...
  if (plan->action == LAGHU_HTTP_ADMINISTRATIVE_ACTION_HISTORY) {
    ...
    if (!laghu_http_administrative_render_history_fragment(&model->history, output, capacity, &length)) {
      return false;
    }
    ...
    if (!laghu_http_administrative_render_explain_fragment(&model->explain, output, capacity, &length)) {
      return false;
    }
  } else {
    return false;
...
bool laghu_http_administrative_render_stats(const laghu_cache_limits *limits, const laghu_cache_stats *stats, char *output, size_t capacity,
                                            laghu_http_administrative_response *response) {
  int written;
  if (limits == NULL || stats == NULL || output == NULL || response == NULL || capacity == 0U) {
    return false;
  }
  written =
      snprintf(output, capacity,
               ...
               (unsigned long long)limits->size_limit, (unsigned long long)limits->inode_limit, (unsigned long long)stats->bytes,
               (unsigned long long)stats->files, (unsigned long long)stats->hits, (unsigned long long)stats->misses,
               (unsigned long long)laghu_http_administrative_hit_ratio_ppm(stats), (unsigned long long)stats->publications,
               (unsigned long long)stats->rejected_publications, (unsigned long long)stats->evictions, (unsigned long long)stats->url_purges,
               (unsigned long long)stats->full_purges, (unsigned long long)stats->invalidated_artifacts, (unsigned long long)stats->invalidated_bytes,
               (unsigned long long)stats->cache_generation, (unsigned long long)stats->last_purge, (unsigned long long)stats->corrupt_removals,
               stats->cleaner_active ? "true" : "false", stats->rebuilding ? "true" : "false", (unsigned long long)stats->last_cleanup);
  if (written < 0 || (size_t)written >= capacity) {
    return false;
  }
  response->status = 200U;
...
bool laghu_http_administrative_build_history_model(const laghu_operational_snapshot *snapshot, const laghu_http_administrative_history_query *query,
  ...
  for (index = 0U; index < count; ++index) {
    ...
    (void)snprintf(record->surface, sizeof(record->surface), "%s", laghu_http_administrative_surface_name(slot->surface));
    (void)snprintf(record->process, sizeof(record->process), "%s", laghu_http_administrative_process_name(slot->process_kind));
    ++model->count;
  }
  return true;
}
...
bool laghu_http_administrative_render_history(const laghu_http_administrative_history_model *model, bool as_json, char *output, size_t capacity,
  ...
  if (as_json) {
    ...
    for (index = 0U; index < model->count; ++index) {
      const laghu_http_administrative_history_record *record = &model->records[index];
      uint64_t saved = record->selected_bytes > record->original_bytes ? 0ULL : record->original_bytes - record->selected_bytes;
      unsigned int saved_ppm;
      if (record->original_bytes == 0U)
        saved_ppm = 0U;
      else
        saved_ppm = (unsigned int)((saved * 10000ULL) / record->original_bytes);
      json_written = snprintf(row, sizeof(row),
                              ...
                              "\"saved_ratio\":%u.%02u}",
                              index == 0U ? "" : ",", record->index, record->surface, record->process, record->active ? "true" : "false",
                              record->healthy ? "true" : "false", record->required ? "true" : "false", (unsigned long long)record->requests,
                              (unsigned long long)record->original_bytes, (unsigned long long)record->selected_bytes, (unsigned long long)saved,
...
bool laghu_http_administrative_build_explain_model(const laghu_http_administrative_explain_query *query, const laghu_operational_readiness *ready,
  ...
  if (query == NULL || model == NULL || query->target[0] == '\0' || stats == NULL || ready == NULL) {
    return false;
  }
  memset(model, 0, sizeof(*model));
  ...
  if (model->has_before && !laghu_cache_source_hash(model->before, model->before_hash)) {
  ...
  (void)snprintf(model->status, sizeof(model->status), "%s",
                 ...
                 : model->cache_ready ? "cache-ready"
                                      : "not-ready");
  (void)snprintf(model->recommendation, sizeof(model->recommendation),
...
bool laghu_http_administrative_render_explain(const laghu_http_administrative_explain_model *model, char *output, size_t capacity,
  ...
  written = snprintf(output, capacity,
                     ...
                     model->target, model->status, model->source_hash, model->runtime_ready ? "ready" : "unavailable",
                     model->cache_ready ? "ready" : "unavailable", model->workers_ready ? "ready" : "unavailable",
                     (unsigned long long)model->hits_ratio_ppm, model->recommendation, comparison_block);
  if (written < 0 || (size_t)written >= capacity) return false;
  response->status = 200U;
  response->content = LAGHU_HTTP_ADMINISTRATIVE_CONTENT_HTML;
  response->length = (size_t)written;
...
bool laghu_http_administrative_render_console(const laghu_cache_stats *stats, const laghu_operational_readiness *ready, bool as_json, char *output,
  ...
  if (as_json) {
    written = snprintf(output, capacity,
                       ...
                       "\"workers\":\"%s\",\"hits\":%llu,\"misses\":%llu,"
                       "\"bytes\":%llu,\"ready\":%s}",
                       model.runtime, model.cache, model.workers, (unsigned long long)model.hits, (unsigned long long)model.misses,
                       (unsigned long long)model.bytes, ready->runtime_ready && ready->cache_ready && ready->workers_ready ? "true" : "false");
    if (written < 0 || (size_t)written >= capacity) return false;
    response->status = 200U;
    response->content = LAGHU_HTTP_ADMINISTRATIVE_CONTENT_JSON;
    response->length = (size_t)written;
  ...
  if (!laghu_http_administrative_render_console_model(&model, output, capacity, response)) return false;
  return true;
}
