...
static bool laghu_http_beacon_view_valid(laghu_buffer value) {
  return value.length == 0U || value.data != NULL;
}

static bool laghu_http_beacon_view_equal(laghu_buffer value,
                                         const char *expected) {
  size_t length = strlen(expected);
  return laghu_http_beacon_view_valid(value) && value.length == length &&
...
bool laghu_http_beacon_plan_build(laghu_http_beacon_plan *plan,
  ...
  if (plan == NULL || options == NULL ||
      !laghu_http_beacon_view_valid(method) ||
      !laghu_http_beacon_view_valid(target)) {
    return false;
  }
  ...
  if (laghu_http_beacon_view_equal(target, "/.laghu/beacon/images.js")) {
    ...
    enabled = options->image_enabled;
  } else if (laghu_http_beacon_view_equal(target,
                                          "/.laghu/beacon/critical-css.js")) {
    plan->route = LAGHU_HTTP_BEACON_ROUTE_CRITICAL_CSS_SCRIPT;
    plan->action = LAGHU_HTTP_BEACON_ACTION_SERVE_SCRIPT;
    enabled = options->critical_css_enabled;
  } else if (laghu_http_beacon_view_equal(target,
                                          "/.laghu/beacon/critical-css")) {
    plan->route = LAGHU_HTTP_BEACON_ROUTE_CRITICAL_CSS_REPORT;
    plan->action = LAGHU_HTTP_BEACON_ACTION_ACCEPT_REPORT;
    enabled = options->critical_css_enabled;
  } else if (laghu_http_beacon_view_equal(
                 target, "/.laghu/beacon/instrumentation.js")) {
  ...
  if (!enabled) {
    plan->status = 404U;
  } else if ((plan->action == LAGHU_HTTP_BEACON_ACTION_SERVE_SCRIPT &&
              !laghu_http_beacon_view_equal(method, "GET")) ||
             (plan->action == LAGHU_HTTP_BEACON_ACTION_ACCEPT_REPORT &&
              !laghu_http_beacon_view_equal(method, "POST"))) {
    plan->status = 405U;
  }
  return true;
}
