...
int main(void) {
  ...
  CHECK(laghu_base_ascii_equal(
      (laghu_buffer){(const unsigned char *)"HeAd", 4U}, "head"));
  ...
  CHECK(laghu_base_url_resolve_same_origin(
            "/pages/index.html", "https://example.test",
            (laghu_buffer){(const unsigned char *)"../hero.webp", 12U}, 0U,
            path, sizeof(path)) &&
        strcmp(path, "/pages/../hero.webp") == 0);
  CHECK(!laghu_base_url_resolve_same_origin(
      "/pages/index.html", "https://example.test",
      (laghu_buffer){(const unsigned char *)"../hero.webp", 12U},
      LAGHU_URL_REJECT_TRAVERSAL, path, sizeof(path)));
  CHECK(laghu_base_hex_value('f') == 15 && laghu_base_hex_value('G') < 0);
  CHECK(laghu_sha256_hex((laghu_buffer){(const unsigned char *)"abc", 3U},
                         digest));
  CHECK(
      strcmp(
          digest,
