...
unsigned char laghu_base_ascii_lower(unsigned char value) { return value >= 'A' && value <= 'Z' ? (unsigned char)(value + ('a' - 'A')) : value; }

bool laghu_base_ascii_equal(laghu_buffer value, const char *expected) {
  size_t index;
  if (expected == NULL || strlen(expected) != value.length || (value.data == NULL && value.length != 0U)) return false;
  for (index = 0U; index < value.length; ++index)
    if (laghu_base_ascii_lower(value.data[index]) != laghu_base_ascii_lower((unsigned char)expected[index])) return false;
  return true;
}
...
bool laghu_base_builder_append(laghu_builder *builder, const void *data, size_t length) {
  if (builder == NULL || (data == NULL && length != 0U) || builder->length > builder->capacity || length > builder->capacity - builder->length)
    return false;
...
bool laghu_base_parse_u64(const char *value, uint64_t minimum, uint64_t maximum, uint64_t *output) {
  ...
  if (value == NULL || *value == '\0' || output == NULL || minimum > maximum) return false;
  while (*cursor != '\0') {
    uint64_t digit;
...
bool laghu_base_url_resolve_same_origin(const char *page_path, const char *page_origin, laghu_buffer value, uint32_t flags, char *output,
  ...
  if (output == NULL || capacity == 0U || url == NULL || length == 0U || length >= capacity || url[0] == '#' ||
      (length >= 2U && url[0] == '/' && url[1] == '/'))
    return false;
  if ((length >= 7U && memcmp(url, "http://", 7U) == 0) || (length >= 8U && memcmp(url, "https://", 8U) == 0)) {
    if (origin_length == 0U || length <= origin_length || memcmp(url, page_origin, origin_length) != 0 || url[origin_length] != '/') return false;
    url += origin_length;
  ...
  if (((flags & LAGHU_URL_REJECT_TRAVERSAL) != 0U && strstr(output, "..") != NULL) ||
      ((flags & LAGHU_URL_REJECT_QUERY) != 0U && strchr(output, '?') != NULL) ||
      ((flags & LAGHU_URL_REJECT_FRAGMENT) != 0U && strchr(output, '#') != NULL) ||
      ((flags & LAGHU_URL_REJECT_API) != 0U && ((strncmp(output, "/api", 4U) == 0 && (output[4] == '\0' || output[4] == '/')) ||
  ...
  if ((flags & LAGHU_URL_REJECT_UNSAFE_BYTES) != 0U)
    for (index = 0U; output[index] != '\0'; ++index)
      if ((unsigned char)output[index] < 0x21U || output[index] == '>' || output[index] == '"' || output[index] == '\\') return false;
  return true;
}
...
static uint32_t laghu_rotate_right(uint32_t value, unsigned int count) { return (value >> count) | (value << (32U - count)); }
...
static void laghu_sha256_transform(laghu_sha256_context *context) {
  ...
  size_t index;
  for (index = 0U; index < 16U; ++index) words[index] = laghu_load_u32_be(&context->block[index * 4U]);
  for (index = 16U; index < 64U; ++index) {
    uint32_t s0 = laghu_rotate_right(words[index - 15U], 7U) ^ laghu_rotate_right(words[index - 15U], 18U) ^ (words[index - 15U] >> 3U);
    uint32_t s1 = laghu_rotate_right(words[index - 2U], 17U) ^ laghu_rotate_right(words[index - 2U], 19U) ^ (words[index - 2U] >> 10U);
  ...
  for (index = 0U; index < 64U; ++index) {
    uint32_t sum1 = laghu_rotate_right(e, 6U) ^ laghu_rotate_right(e, 11U) ^ laghu_rotate_right(e, 25U);
    uint32_t choice = (e & f) ^ ((~e) & g);
    uint32_t temporary1 = h + sum1 + choice + laghu_sha256_round_constants[index] + words[index];
    uint32_t sum0 = laghu_rotate_right(a, 2U) ^ laghu_rotate_right(a, 13U) ^ laghu_rotate_right(a, 22U);
...
void laghu_sha256_init(laghu_sha256_context *context) {
  static const uint32_t initial[8] = {UINT32_C(0x6a09e667), UINT32_C(0xbb67ae85), UINT32_C(0x3c6ef372), UINT32_C(0xa54ff53a),
...
void laghu_sha256_final(laghu_sha256_context *context, unsigned char digest[LAGHU_SHA256_DIGEST_SIZE]) {
  ...
  laghu_sha256_transform(context);
  for (index = 0U; index < 8U; ++index) laghu_store_u32_be(&digest[index * 4U], context->state[index]);
}
